/**
 * @output includes/js/custom-header.js
 */
...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'custom-header-video-button' );
			button.setAttribute( 'class', 'custom-header-video-button custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'custom-header-video-button custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window. ) {
					window..a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'custom-header-video-button custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window. ) {
					window..a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	 ...
	 * @class
	 */
	NativeHandler = BaseHandler.extend(/** @lends .NativeHandler.prototype */{
		/**
	 ...
	 */
	YouTubeHandler = BaseHandler.extend(/** @lends .YouTubeHandler.prototype */{
		/**
	...
	document.addEventListener( 'DOMContentLoaded', window..customHeader.initialize.bind( window..customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window. ) {
		window..customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window..customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window..customHeader.initialize();
			}
		});
	}

})( window, window._CustomHeaderSettings || {} );
