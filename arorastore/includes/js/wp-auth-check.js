 ...
 *
 * @output includes/js/auth-check.js
 */
...
( function( $ ) {
	 ...
	 */
	function show() {
		var parent = $( '#auth-check' ),
			form = $( '#auth-check-form' ),
			noframe = wrap.find( '.auth-fallback-expired' ),
			frame, loaded = false;

		if ( form.length ) {
			// Add unload confirmation to counter (frame-busting) JS redirects.
			$( window ).on( 'beforeunload.auth-check', function( event ) {
				event.originalEvent.returnValue = window..i18n.__( 'Your session has expired. You can log in again from this page or go to the login page.' );
			});

			frame = $( '<iframe id="auth-check-frame" frameborder="0">' ).attr( 'title', noframe.text() );
	...
	$( function() {
		 ...
		 * @since 3.6.0
		 */
		wrap = $( '#auth-check-wrap' );
		wrap.find( '.auth-check-close' ).on( 'click', function() {
			hide();
			setShowTimeout();
		});
	}).on( 'heartbeat-tick.auth-check', function( e, data ) {
		if ( 'auth-check' in data ) {
			if ( ! data['auth-check'] && wrap.hasClass( 'hidden' ) && ! tempHidden ) {
				show();
			} else if ( data['auth-check'] && ! wrap.hasClass( 'hidden' ) ) {
				hide();
			}
		}
	});
