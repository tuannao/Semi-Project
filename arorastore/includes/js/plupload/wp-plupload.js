...
( function( exports, $ ) {
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-upload-attachment-id:\s*(\d+)/i );
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.uploader, drop.uploader', function() {
				/*
				 ...
				 *
				 * @see https://core.trac..org/ticket/21705
				 */
	...
	$.extend( Uploader.prototype, /** @lends wp.Uploader.prototype */{
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'uploader-browser-' + this.uploader.id ).appendTo('body');
					}
