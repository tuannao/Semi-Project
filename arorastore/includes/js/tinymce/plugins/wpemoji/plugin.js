( function( tinymce ) {
	tinymce.PluginManager.add( 'emoji', function( editor ) {
		var typing,
			 = window.,
			settings = window._emojiSettings,
			env = tinymce.Env,
		...
		if ( !  || ! .emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after .emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-emoji="[^>]+>/g, function( img ) {
