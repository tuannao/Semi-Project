( function( tinymce ) {
	tinymce.ui.Factory.add( 'LinkPreview', tinymce.ui.Control.extend( {
		url: '#',
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="link-preview">' +
	...
	tinymce.ui.Factory.add( 'LinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="link-input">' +
	...
	tinymce.PluginManager.add( 'link', function( editor ) {
		...
		var speak = ( typeof window. !== 'undefined' && window..a11y && window..a11y.speak ) ? window..a11y.speak : function() {};
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '__link_placeholder' ) {
					editor.dom.remove( element, true );
				} else if ( $element.attr( 'data-link-edit' ) ) {
					$element.attr( 'data-link-edit', null );
				}
			});
		}
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="__link_placeholder"' ) > -1 ) {
					return text;
				}

				if ( dataAttr ) {
					tag = tag.replace( / data-link-edit="true"/g, '' );
				}

				tag = tag.replace( / data-link-url-error="true"/g, '' );
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-autocomplete-' + ui.item.ID );
							/*
						...
						return $( '<li role="option" id="mce-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
			...
			if ( linkNode ) {
				...
				if ( href === '_wp_link_placeholder' || edit ) {
					...
					if ( $linkNode.attr( 'data-wplink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.link-preview a' ).addClass( 'wplink-url-error' );
					} else {
						toolbar.$el.find( '.link-preview a' ).removeClass( 'wplink-url-error' );
						hasLinkError = false;
					}
				}
