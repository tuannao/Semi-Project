	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.heartbeat', function() {
			...
			$(window).on( 'unload.heartbeat', function() {
		...
		function connect() {
			...
			settings.xhr = $.ajax({
				...
				// Update the Rest API nonce if set and api loaded.
		...
		function userIsActive() {
			...
			$document.off( '.heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.heartbeat-active keyup.heartbeat-active touchend.heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.heartbeat-active keyup.heartbeat-active touchend.heartbeat-active', function() {
							userIsActive();
						});
					}
				});
