...
(function( exports, $ ){
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		if ( /\/(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}
		...
		if ( /\/(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		// Ignore links with class playlist-caption.
		if ( $element.hasClass( 'playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'custom-logo', !! attachmentId );
		},
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
