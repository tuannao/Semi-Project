/**
 * @output includes/js/pointer.js
 */

/**
 * Initializes the pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;

	$.widget('.pointer',/** @lends $.widget..pointer.prototype */{
		options: {
			...
			buttons: function( event, t ) {
				var button = $('<a class="close" href="#"></a>').text( .i18n.__( 'Dismiss' ) );
		 ...
		 * @private
		 *
		 * @constructs $.widget..pointer
		 */
		_create: function() {
			var positioning,
				family;

			this.content = $('<div class="pointer-content"></div>');
			this.arrow   = $('<div class="pointer-arrow"><div class="pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'pointer-' + edge );
		},
