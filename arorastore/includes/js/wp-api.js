...
(function( window, undefined ) {
	...
	/** @namespace .api */
	.api               = .api || new _API();
	.api.versionString = .api.versionString || '/v2/';
...
(function( window, undefined ) {
	 ...
	 */
	.api.getModelByRoute = function( route ) {
	 ...
	 */
	.api.getCollectionByRoute = function( route ) {
		return _.find( .api.collections, function( collection ) {
	 ...
	 * @param {Date} date.
	 */
	.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	.api.utils.capitalize = function( str ) {
	 ...
	 */
	.api.utils.capitalizeAndCamelCaseDashes = function( str ) {
		if ( _.isUndefined( str ) ) {
			return str;
		}
		str = .api.utils.capitalize( str );

		return .api.utils.camelCaseDashes( str );
	};

	/**
	 * Helper function to camel case the letter after dashes, removing the dashes.
	 */
	.api.utils.camelCaseDashes = function( str ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `.api.versionString`.
	 * @param {boolean} [reverse]       Whether to reverse the order when extracting the route part. Optional, default false.
	 */
	.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		var routeParts;

		part = part || 1;
		versionString = versionString || .api.versionString;
	 ...
	 */
	.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object.  expects dates in UTC.
				...
				getDate: function( field ) {
					...
					return new Date( .api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new .api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a .api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new .api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new .api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new .api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new .api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new .api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new .api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new .api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new .api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', ':featuredmedia', 'source_url' );
				}
			};
...
(function() {
	...
	var ApiSettings = window.ApiSettings || {},
	trashableTypes    = [ 'Comment', 'Media', 'Comment', 'Post', 'Page', 'Status', 'Taxonomy', 'Type' ];

	/**
	 * Backbone base model for all models.
	 */
	.api.ApiBaseModel = Backbone.Model.extend(
		/** @lends ApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-Nonce' );
	 ...
	 */
	.api.models.Schema = .api.ApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				.api.ApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || ApiSettings.root;
				model.versionString = options.versionString || ApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	.api.ApiBaseCollection = Backbone.Collection.extend(
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-total' ), 10 );
						}
...
( function() {
	...
	// If ApiSettings is unavailable, try the default.
	if ( _.isEmpty( ApiSettings ) ) {
		ApiSettings.root = window.location.origin + '/json/';
	}

	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		defaults: {
			apiRoot: ApiSettings.root,
		...
		initialize: function() {
			...
			model.schemaModel = new .api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( ApiSettings.cacheSchema ) || ApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( ApiSettings.cacheSchema ) || ApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable ApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = ApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( .api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
					routeName  = .api.utils.extractRoutePart( modelRoute.index, 2, routeModel.get( 'versionString' ), true ),
					parentName = .api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), false ),
					routeEnd   = .api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), true );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = .api.utils.capitalizeAndCamelCaseDashes( parentName ) + .api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = .api.ApiBaseModel.extend( {
					...
					modelClassName = .api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = .api.ApiBaseModel.extend( {
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = .api.utils.extractRoutePart( collectionRoute.index, 1, routeModel.get( 'versionString' ), false );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {

					collectionClassName = .api.utils.capitalizeAndCamelCaseDashes( parentName ) + .api.utils.capitalizeAndCamelCaseDashes( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = .api.ApiBaseCollection.extend( {
					...
					collectionClassName = .api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = .api.ApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = .api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	.api.endpoints = new Backbone.Collection();

	/**
	 * Initialize the api, optionally passing the API root.
	 *
	 * @param {Object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to ApiSettings.nonce.
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to ApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to ApiSettings.root.
	...
	.api.init = function( args ) {
		...
		args                      = args || {};
		attributes.nonce          = _.isString( args.nonce ) ? args.nonce : ( ApiSettings.nonce || '' );
		attributes.apiRoot        = args.apiRoot || ApiSettings.root || '/json';
		attributes.versionString  = args.versionString || ApiSettings.versionString || '/v2/';
		attributes.schema         = args.schema || null;
		attributes.modelEndpoints = args.modelEndpoints || [ 'me', 'settings' ];
		if ( ! attributes.schema && attributes.apiRoot === ApiSettings.root && attributes.versionString === ApiSettings.versionString ) {
			attributes.schema = ApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {

			// Look for an existing copy of this endpoint.
			endpoint = .api.endpoints.findWhere( { 'apiRoot': attributes.apiRoot, 'versionString': attributes.versionString } );
			if ( ! endpoint ) {
			...
			endpoint.schemaConstructed.done( function( resolvedEndpoint ) {
				.api.endpoints.add( resolvedEndpoint );

				// Map the default endpoints, extending any already present items (including Schema model).
				.api.models      = _.extend( .api.models, resolvedEndpoint.get( 'models' ) );
				.api.collections = _.extend( .api.collections, resolvedEndpoint.get( 'collections' ) );
	...
	// The .api.init function returns a promise that will resolve with the endpoint once it is ready.
	.api.loadPromise = .api.init();
