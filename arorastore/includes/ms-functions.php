 ...
 * @return _Site|void The blog object
 */
 ...
 * @return _Post|null _Post object on success, null on failure
 */
 ...
 * @return true|_Error True on success or a _Error object if the user doesn't exist
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new _Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param true|_Error $retval  True if the user should be added to the site, error
	 *                               object otherwise.
	...
	if ( true !== $can_add_user ) {
		...
		if ( is__error( $can_add_user ) ) {
			return $can_add_user;
		}

		return new _Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
	...
	_cache_delete( $blog_id . '_user_count', 'blog-details' );
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new _Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
	...
	if ( $reassign ) {
		...
		$post_ids = $db->get_col( $db->prepare( "SELECT ID FROM $db->posts WHERE post_author = %d", $user_id ) );
		$link_ids = $db->get_col( $db->prepare( "SELECT link_id FROM $db->links WHERE link_owner = %d", $user_id ) );

		if ( ! empty( $post_ids ) ) {
			$db->query( $db->prepare( "UPDATE $db->posts SET post_author = %d WHERE post_author = %d", $reassign, $user_id ) );
			array_walk( $post_ids, 'clean_post_cache' );
		}

		if ( ! empty( $link_ids ) ) {
			$db->query( $db->prepare( "UPDATE $db->links SET link_owner = %d WHERE link_owner = %d", $reassign, $user_id ) );
...
function get_blog_id_from_url( $domain, $path = '/' ) {
	...
	$id     = _cache_get( md5( $domain . $path ), 'blog-id-cache' );
	...
	if ( ! $id ) {
		_cache_set( md5( $domain . $path ), -1, 'blog-id-cache' );
		return 0;
	}

	_cache_set( md5( $domain . $path ), $id, 'blog-id-cache' );
 ...
 * administrators.
 *
 * The {@see 'mu_validate_user_signup'} hook provides an easy way to modify the sign-up
 * process. The value $result, which is passed to the hook, contains both the user-provided
 * info and the error messages created by the function. {@see 'mu_validate_user_signup'}
 ...
 *     @type _Error $errors        _Error object containing any errors found.
 * }
 */
function mu_validate_user_signup( $user_name, $user_email ) {
	global $db;
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from . Please use another email provider.' ) );
	}
	...
	$signup = $db->get_row( $db->prepare( "SELECT * FROM $db->signups WHERE user_login = %s", $user_name ) );
	if ( $signup instanceof stdClass ) {
		...
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$db->delete( $db->signups, array( 'user_login' => $user_name ) );
		} else {
			$errors->add( 'user_name', __( 'That username is currently reserved but may be available in a couple of days.' ) );
		}
	}

	$signup = $db->get_row( $db->prepare( "SELECT * FROM $db->signups WHERE user_email = %s", $user_email ) );
	if ( $signup instanceof stdClass ) {
		$diff = time() - mysql2date( 'U', $signup->registered );
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$db->delete( $db->signups, array( 'user_email' => $user_email ) );
		} else {
	 ...
	 * }
	 */
	return apply_filters( 'mu_validate_user_signup', $result );
}
 ...
 *
 * Filter {@see 'mu_validate_blog_signup'} if you want to modify
 ...
 * @param _User|string $user       Optional. The user object to check against the new site name.
 * @return array {
 ...
 *     @type string|_User $user       By default, an empty string. A user object if provided.
 *     @type _Error       $errors     _Error containing any errors found.
 * }
 */
function mu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	global $db, $domain;
	...
	if ( ! is_subdomain_install() && $db->get_var( $db->prepare( 'SELECT post_name FROM ' . $db->get_blog_prefix( $current_network->site_id ) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname ) ) ) {
	...
	$blog_title = _unslash( $blog_title );
	...
	$signup = $db->get_row( $db->prepare( "SELECT * FROM $db->signups WHERE domain = %s AND path = %s", $mydomain, $path ) );
	if ( $signup instanceof stdClass ) {
		$diff = time() - mysql2date( 'U', $signup->registered );
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
	 ...
	 * }
	 */
	return apply_filters( 'mu_validate_blog_signup', $result );
}
 ...
 */
function mu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	global $db;

	$key = substr( md5( time() . _rand() . $domain ), 0, 16 );
 ...
 */
function mu_signup_user( $user, $user_email, $meta = array() ) {
	global $db;

	// Format data.
	$user       = preg_replace( '/\s+/', '', sanitize_user( $user, true ) );
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . _rand() . $user_email ), 0, 16 );
 ...
 * is enabled.
 *
 * Filter {@see 'mu_signup_blog_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'mu_signup_blog_notification_email'} and
 * {@see 'mu_signup_blog_notification_subject'} to change the content
 ...
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in mu_signup_blog()
 * @param array  $meta       Optional. Signup meta data. By default, contains the requested privacy setting and lang_id.
 * @return bool
 */
function mu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string       $user_email User email address.
	 * @param string       $key        Activation key created in mu_signup_blog().
	 * @param array        $meta       Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'mu_signup_blog_notification', $domain, $path, $title, $user_login, $user_email, $key, $meta ) ) {
		return false;
	}
	...
	if ( '' === $admin_email ) {
		$admin_email = 'support@' . _parse_url( network_home_url(), PHP_URL_HOST );
	}
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be formatted for transmission via _mail().
		 *
		 * @since MU (3.0.0)
		 *
		 ...
		 */
		apply_filters(
			'mu_signup_blog_notification_email',
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'mu_signup_blog_notification_subject',
	...
	_mail( $user_email, _specialchars_decode( $subject ), $message, $message_headers );
 ...
 * been requested.
 *
 * Filter {@see 'mu_signup_user_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'mu_signup_user_notification_email'} and
 * {@see 'mu_signup_user_notification_subject'} to change the content
 * and subject line of the email sent to newly registered users.
 *
 * @since MU (3.0.0)
 *
 * @param string $user_login The user's login name.
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in mu_signup_user()
 * @param array  $meta       Optional. Signup meta data. Default empty array.
 * @return bool
 */
function mu_signup_user_notification( $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string $user_login User login name.
	 * @param string $user_email User email address.
	 * @param string $key        Activation key created in mu_signup_user().
	 * @param array  $meta       Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'mu_signup_user_notification', $user_login, $user_email, $key, $meta ) ) {
		return false;
	}
	...
	$message         = sprintf(
		 ...
		 */
		apply_filters(
			'mu_signup_user_notification_email',
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'mu_signup_user_notification_subject',
 ...
 * Activates a signup.
 *
 * Hook to {@see 'mu_activate_user'} or {@see 'mu_activate_blog'} for events
 ...
 * @return array|_Error An array containing information about the activated user and/or blog
 */
function mu_activate_signup( $key ) {
	global $db;

	$signup = $db->get_row( $db->prepare( "SELECT * FROM $db->signups WHERE activation_key = %s", $key ) );

	if ( empty( $signup ) ) {
		return new _Error( 'invalid_key', __( 'Invalid activation key.' ) );
	}

	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new _Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new _Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}

	$meta     = maybe_unserialize( $signup->meta );
	$password = _generate_password( 12, false );

	$user_id = username_exists( $signup->user_login );

	if ( ! $user_id ) {
		$user_id = mu_create_user( $signup->user_login, $password, $signup->user_email );
	} else {
		$user_already_exists = true;
	}

	if ( ! $user_id ) {
		return new _Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new _Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
		 ...
		 */
		do_action( 'mu_activate_user', $user_id, $password, $meta );
	...
	$blog_id = mu_create_blog( $signup->domain, $signup->path, $signup->title, $user_id, $meta, get_current_network_id() );

	// TODO: What to do if we create a user but cannot create a blog?
	if ( is__error( $blog_id ) ) {
		/*
	 ...
	 * @param array  $meta          Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	do_action( 'mu_activate_blog', $blog_id, $user_id, $password, $signup->title, $meta );
 ...
 */
function _delete_signup_on_user_delete( $id, $reassign, $user ) {
	global $db;

	$db->delete( $db->signups, array( 'user_login' => $user->user_login ) );
}
 ...
 * a Super Admin creates a new user. Hook to {@see 'mu_new_user'} for events
 ...
 */
function mu_create_user( $user_name, $password, $email ) {
	$user_name = preg_replace( '/\s+/', '', sanitize_user( $user_name, true ) );

	$user_id = _create_user( $user_name, $password, $email );
 ...
 * as when a Super Admin creates a new site. Hook to {@see 'mu_new_blog'}
 ...
 * @return int|_Error Returns _Error object on failure, the new site ID on success.
 */
function mu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	$defaults = array(
		'public' => 0,
	);
	$options  = _parse_args( $options, $defaults );
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new _Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
	...
	// Data to pass to _initialize_site().
	...
	$blog_id = _insert_site( array_merge( $site_data, $site_initialization_data ) );

	if ( is__error( $blog_id ) ) {
		return $blog_id;
	}

	_cache_set( 'last_changed', microtime(), 'sites' );
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 $blog_id now supports input from the {@see '_initialize_site'} action.
 *
 * @param _Site|int $blog_id    The new site's object or ID.
...
Disable these notifications: %4$s'
	...
	_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
...
Disable these notifications: %3$s'
	 ...
	 *
	 * @param string  $msg  Email body.
	 * @param _User $user _User instance of the new user.
	 */
	$msg = apply_filters( 'newuser_notify_siteadmin', $msg, $user );

	/* translators: New user notification email subject. %s: User login. */
	_mail( $email, sprintf( __( 'New User Registration: %s' ), $user->user_login ), $msg );
 ...
 *
 * Filter {@see 'mu_welcome_notification'} to disable or bypass.
 *
 ...
 */
function mu_welcome_notification( $blog_id, $user_id, $password, $title, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array     $meta     Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'mu_welcome_notification', $blog_id, $user_id, $password, $title, $meta ) ) {
		return false;
	}
...
--The Team @ SITE_NAME'
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, _unslash( $title ) ) );

	_mail( $user->user_email, _specialchars_decode( $subject ), $message, $message_headers );
 ...
 * @return bool Whether the email notification was sent.
 */
function mu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param _Site $site Site object of the new site.
	 * @param _User $user User object of the administrator of the new site.
	 */
...
Name: %3$s'
	...
	_mail(
		$new_site_email['to'],
		_specialchars_decode( $new_site_email['subject'] ),
 ...
 *
 * Filter {@see 'mu_welcome_user_notification'} to disable or bypass.
 *
 ...
 * @return bool
 */
function mu_welcome_user_notification( $user_id, $password, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array  $meta     Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'mu_welcome_user_notification', $user_id, $password, $meta ) ) {
		return false;
	}
	...
	$welcome_email = str_replace( 'USERNAME', $user->user_login, $welcome_email );
	$welcome_email = str_replace( 'PASSWORD', $password, $welcome_email );
	$welcome_email = str_replace( 'LOGINLINK', _login_url(), $welcome_email );
...
function get_most_recent_post_of_user( $user_id ) {
	...
	foreach ( (array) $user_blogs as $blog ) {
		$prefix      = $db->get_blog_prefix( $blog->userblog_id );
		$recent_post = $db->get_row( $db->prepare( "SELECT ID, post_date_gmt FROM {$prefix}posts WHERE post_author = %d AND post_type = 'post' AND post_status = 'publish' ORDER BY post_date_gmt DESC LIMIT 1", $user_id ), ARRAY_A );
...
function update_posts_count( $deprecated = '' ) {
	global $db;
	update_option( 'post_count', (int) $db->get_var( "SELECT COUNT(ID) FROM {$db->posts} WHERE post_status = 'publish' and post_type = 'post'" ) );
}
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 Parameters now support input from the {@see '_initialize_site'} action.
 *
 * @global db $db  database abstraction object.
 *
 * @param _Site|int $blog_id The new site's object or ID.
 ...
 */
function mu_log_new_registrations( $blog_id, $user_id ) {
	global $db;
	...
	if ( $user ) {
		$db->insert(
			...
			array(
				...
				'IP'              => preg_replace( '/[^0-9., ]/', '', _unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
...
function global_terms( $term_id, $deprecated = '' ) {
	...
	$term_id = (int) $term_id;
	$c       = $db->get_row( $db->prepare( "SELECT * FROM $db->terms WHERE term_id = %d", $term_id ) );

	$global_id = $db->get_var( $db->prepare( "SELECT cat_ID FROM $db->sitecategories WHERE category_nicename = %s", $c->slug ) );
	if ( null == $global_id ) {
		$used_global_id = $db->get_var( $db->prepare( "SELECT cat_ID FROM $db->sitecategories WHERE cat_ID = %d", $c->term_id ) );
		if ( null == $used_global_id ) {
			...
			$global_id = $db->insert_id;
			if ( empty( $global_id ) ) {
				return $term_id;
			}
		} else {
			$max_global_id = $db->get_var( "SELECT MAX(cat_ID) FROM $db->sitecategories" );
			$max_local_id  = $db->get_var( "SELECT MAX(term_id) FROM $db->terms" );
		...
		$local_id = $db->get_var( $db->prepare( "SELECT term_id FROM $db->terms WHERE term_id = %d", $global_id ) );
		if ( null != $local_id ) {
	...
	if ( $global_id != $term_id ) {
		...
		$db->update( $db->terms, array( 'term_id' => $global_id ), array( 'term_id' => $term_id ) );
		$db->update( $db->term_taxonomy, array( 'term_id' => $global_id ), array( 'term_id' => $term_id ) );
		$db->update( $db->term_taxonomy, array( 'parent' => $global_id ), array( 'parent' => $term_id ) );
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( '_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
...
function signup_nonce_fields() {
	...
	_nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	...
	if ( ! _verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
 ...
 * added, as when a user is invited through the regular  Add User interface.
 *
 * @since MU (3.0.0)
 */
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is__error( add_existing_user_to_blog( $details ) ) ) {
		_die(
			sprintf(
 ...
 * }
 * @return true|_Error|void True on success or a _Error object if the user doesn't exist
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int           $user_id User ID.
		 * @param true|_Error $result  True on success or a _Error object if the user doesn't exist
		 *                               or could not be added.
		 */
 ...
 * is specifically hooked into the {@see 'mu_activate_user'} action.
 *
 * @since MU (3.0.0)
 *
 ...
 * @since MU (3.0.0)
 *
 * @param string|_User $user Optional. Defaults to current user. _User object,
 ...
 */
function filter_SSL( $url ) {  // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
 ...
 * @since 3.1.0
 */
function _schedule_update_network_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! _next_scheduled( 'update_network_counts' ) && ! _installing() ) {
		_schedule_event( time(), 'twicedaily', 'update_network_counts' );
	}
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function _update_network_counts( $network_id = null ) {
	_update_network_user_counts( $network_id );
	_update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function _maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! _is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function _maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! _is_large_network( 'users', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function _update_network_site_counts( $network_id = null ) {
 ...
 * @since 3.7.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 * @since 6.0.0 This function is now a wrapper for _update_user_counts().
 *
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function _update_network_user_counts( $network_id = null ) {
	_update_user_counts( $network_id );
}
 ...
 * Plugins can alter this criteria using the {@see '_is_large_network'} filter.
 *
 * @since 3.3.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 *
 ...
 */
function _is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	if ( 'users' === $using ) {
		...
		$is_large_network = _is_large_user_count( $network_id );
		 ...
		 */
		return apply_filters( '_is_large_network', $is_large_network, 'users', $count, $network_id );
	}

	$count = get_blog_count( $network_id );

	/** This filter is documented in includes/ms-functions.php */
	return apply_filters( '_is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'admin', 'content', 'includes', 'json', and 'embed' were added
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', _specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
	...
	_mail(
		...
		sprintf(
			...
			_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES )
		),
		$content
	);
 ...
 */
function _network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// Get network name.
	$network_name = _specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );
