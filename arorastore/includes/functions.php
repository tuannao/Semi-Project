...
function date_i18n( $format, $timestamp_with_offset = false, $gmt = false ) {
	...
	if ( ! is_numeric( $timestamp ) ) {
		// phpcs:ignore .DateTime.CurrentTimeTimestamp.Requested
...
function maybe_serialize( $data ) {
	 ...
	 * See https://core.trac..org/ticket/12930
 ...
 * @global WP_Query $wp_the_query Copy of the  Query object.
 *
...
/**
 * Determines whether  is already installed.
 *
...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		 ...
		 */
		if ( $is_image ) {
			/** This filter is documented in includes/class-image-editor.php */
...
/**
 * Kills  execution and displays HTML page with an error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when  is still
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( wp_doing_ajax() ) {
		...
		/**
		 * Filters the callback for killing  execution for JSON requests.
		 *
		 * @since 5.1.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing  execution for JSONP REST requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing  execution for XML requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing  execution for all non-Ajax, non-JSON, non-XML requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $callback Callback function name.
...
/**
 * Kills  execution and displays Ajax response with an error message.
 *
...
/**
 * Kills  execution and displays JSON response with an error message.
 *
...
/**
 * Kills  execution and displays JSONP response with an error message.
 *
...
/**
 * Kills  execution and displays XML response with an error message.
 *
...
/**
 * Kills  execution and displays an error message.
 *
...
function _wp_to_kebab_case( $string ) {
	...
	//phpcs:enable .NamingConventions.ValidVariableName.VariableNotSnakeCase
}
 ...
 * @param string $version     The version of  that deprecated the hook.
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'Please see <a href="%s">Debugging in </a> for more information.' ),
				__( 'https://.org/support/article/debugging-in-/' )
			);

			trigger_error(
				sprintf(
					/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3:  version number. */
			...
			$message .= sprintf(
				' Please see <a href="%s">Debugging in </a> for more information.',
				'https://.org/support/article/debugging-in-/'
			);
...
function wp_timezone_choice( $selected_zone, $locale = null ) {
	...
	foreach ( timezone_identifiers_list() as $zone ) {
		...
		// phpcs:disable .WP.I18n.LowLevelTranslationFunction,.WP.I18n.NonSingularStringLiteralText
		$zonen[] = array(
 ...
 * @access private
 *
 * @see https://core.trac..org/ticket/8497
 *
 ...
 *
 * @link https://codex..org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 * @return int 0.
 */
function __return_zero() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return 0;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_null() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return null;
}
 ...
 */
function __return_empty_string() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
 ...
 * @since 3.4.0
 *
 * @see https://core.trac..org/ticket/19589
 *
...
function wp_auth_check_load() {
	...
	if ( apply_filters( 'wp_auth_check_load', $show, $screen ) ) {
		wp_enqueue_style( 'auth-check' );
		wp_enqueue_script( 'auth-check' );
...
/**
 * Output the HTML that shows the login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
function wp_auth_check_html() {
	...
	?>
	<div id="auth-check-wrap" class="<?php echo $wrap_class; ?>">
	<div id="auth-check-bg"></div>
	<div id="auth-check">
	<button type="button" class="auth-check-close button-link"><span class="screen-reader-text"><?php _e( 'Close dialog' ); ?></span></button>
	<?php
	...
	if ( $same_domain ) {
		...
		?>
		<div id="auth-check-form" class="loading" data-src="<?php echo esc_url( $login_src ); ?>"></div>
		<?php
	}

	?>
	<div class="auth-fallback">
		<p><b class="auth-fallback-expired" tabindex="0"><?php _e( 'Session expired' ); ?></b></p>
 ...
 * @return array The Heartbeat response with 'auth-check' value set.
 */
function wp_auth_check( $response ) {
	$response['auth-check'] = is_user_logged_in() && empty( $GLOBALS['login_grace_period'] );
	return $response;
}
 ...
 * @access private
 *
 * @see https://core.trac..org/ticket/23688
 *
...
function wp_privacy_exports_dir() {
	...
	$exports_dir = trailingslashit( $upload_dir['basedir'] ) . 'personal-data-exports/';
...
function wp_privacy_exports_url() {
	$upload_dir  = wp_upload_dir();
	$exports_url = trailingslashit( $upload_dir['baseurl'] ) . 'personal-data-exports/';
...
function wp_get_default_update_php_url() {
	return _x( 'https://.org/support/update-php/', 'localized PHP upgrade information page' );
}
...
function wp_get_update_php_annotation() {
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official  documentation</a>.' ),
		esc_url( $default_url )
	);
...
function wp_get_default_update_https_url() {
	...
	return __( 'https://.org/support/article/why-should-i-use-https/' );
}
 ...
 *                                   The timeout is global and is measured from the moment  started to load.
