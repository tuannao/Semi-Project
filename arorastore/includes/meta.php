...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	_cache_delete( $object_id, $meta_type . '_meta' );
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	if ( $delete_all ) {
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$object_ids = $db->get_col( $db->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s", $meta_key, $meta_value ) );
		} else {
	...
	_cache_delete_multiple( $data, $meta_type . '_meta' );
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = _cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! _is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $db->get_row( $db->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		$result = $db->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result ) {
			return false;
		}
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		$result = (bool) $db->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$cache          = array();
	$cache_values   = _cache_get_multiple( $object_ids, $cache_key );
	...
	_cache_add_multiple( $data, $cache_key );
 ...
 * @since 4.5.0
 *
 * @return _Metadata_Lazyloader Metadata lazyloader queue.
 */
function _metadata_lazyloader() {
	static $_metadata_lazyloader;

	if ( null === $_metadata_lazyloader ) {
		$_metadata_lazyloader = new _Metadata_Lazyloader();
	}

	return $_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new _Meta_Query( $meta_query );
 ...
 *
 * @since 3.3.0
 * @since 4.6.0 {@link https://core.trac..org/ticket/35658 Modified
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $_meta_keys ) || ! isset( $_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $_meta_keys[ $object_type ] ) ) {
		unset( $_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $_meta_keys ) || ! isset( $_meta_keys[ $object_type ] ) || ! isset( $_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $_meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function __register_meta_args_allowed_list( $args, $default_args ) {
