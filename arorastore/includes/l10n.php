...
function get_locale() {
	...
	if ( isset( $_local_package ) ) {
		$locale = $_local_package;
	}

	// LANG was defined in config.
	if ( defined( 'LANG' ) ) {
		$locale = LANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( _installing() ) {
			$ms_locale = get_site_option( 'LANG' );
		} else {
			$ms_locale = get_option( 'LANG' );
			if ( false === $ms_locale ) {
				$ms_locale = get_site_option( 'LANG' );
			}
		}
 ...
 * @since 4.7.0
 *
 * @param int|_User $user_id User's ID or a _User object. Defaults to current user.
 * @return string The locale of the user.
 */
function get_user_locale( $user_id = 0 ) {
	$user = false;
	if ( 0 === $user_id && function_exists( '_get_current_user' ) ) {
		$user = _get_current_user();
	} elseif ( $user_id instanceof _User ) {
		$user = $user_id;
...
function determine_locale() {
	...
	if ( isset( $_GET['_locale'] ) && 'user' === $_GET['_locale'] && _is_json_request() ) {
		$determined_locale = get_user_locale();
	}
	...
	if ( ! empty( $_GET['_lang'] ) ) {
		$_lang = sanitize_text_field( $_GET['_lang'] );
	} elseif ( ! empty( $_COOKIE['_lang'] ) ) {
		$_lang = sanitize_text_field( $_COOKIE['_lang'] );
	}

	if ( ! empty( $_lang ) && ! empty( $GLOBALS['pagenow'] ) && 'login.php' === $GLOBALS['pagenow'] ) {
		$determined_locale = $_lang;
	}
 ...
 *
 * Loads the .mo file in _LANG_DIR constant path from  root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', _LANG_DIR . "/$locale.mo" );

	if ( ( is_multisite() || ( defined( '_INSTALLING_NETWORK' ) && _INSTALLING_NETWORK ) ) && ! file_exists( _LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', _LANG_DIR . "/ms-$locale.mo" );
		return $return;
	}

	if ( is_admin() || _installing() || ( defined( '_REPAIRING' ) && _REPAIRING ) ) {
		load_textdomain( 'default', _LANG_DIR . "/admin-$locale.mo" );
	}

	if ( is_network_admin() || ( defined( '_INSTALLING_NETWORK' ) && _INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', _LANG_DIR . "/admin-network-$locale.mo" );
	}
 ...
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to _PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( load_textdomain( $domain, _LANG_DIR . '/plugins/' . $mofile ) ) {
		return true;
	}

	if ( false !== $plugin_rel_path ) {
		$path = _PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `MU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = MU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	// Try to load from the languages directory first.
	if ( load_textdomain( $domain, _LANG_DIR . '/themes/' . $mofile ) ) {
		return true;
	}
...
function load_script_textdomain( $handle, $domain = 'default', $path = null ) {
	...
	if ( ! isset( $_scripts->registered[ $handle ] ) ) {
		return false;
	}
	...
	$src = $_scripts->registered[ $handle ]->src;

	if ( ! preg_match( '|^(https?:)?//|', $src ) && ! ( $_scripts->content_url && 0 === strpos( $src, $_scripts->content_url ) ) ) {
		$src = $_scripts->base_url . $src;
	}

	$relative       = false;
	$languages_path = _LANG_DIR;

	$src_url     = _parse_url( $src );
	$content_url = _parse_url( content_url() );
	$plugins_url = _parse_url( plugins_url() );
	$site_url    = _parse_url( site_url() );
	...
	if (
		...
		$languages_path = _LANG_DIR . '/' . $relative[0];
		...
		$languages_path = _LANG_DIR . '/plugins';
		...
		if ( ! isset( $site_url['path'] ) ) {
			...
			// Make the src relative to the  root.
...
function _get_path_to_translation_from_lang_dir( $domain ) {
	...
	$path = _LANG_DIR . '/plugins/' . $mofile;
	if ( in_array( $path, $cached_mofiles, true ) ) {
		return $path;
	}

	$path = _LANG_DIR . '/themes/' . $mofile;
	if ( in_array( $path, $cached_mofiles, true ) ) {
		return $path;
	}
 ...
 *
 * The default directory is _LANG_DIR.
 *
 * @since 3.0.0
...
function get_available_languages( $dir = null ) {
	...
	$lang_files = glob( ( is_null( $dir ) ? _LANG_DIR : $dir ) . '/*.mo' );
	if ( $lang_files ) {
 ...
 */
function _get_installed_translations( $type ) {
	...
	if ( $dir && ! is_dir( _LANG_DIR . $dir ) ) {
		return array();
	}

	$files = scandir( _LANG_DIR . $dir );
	if ( ! $files ) {
		return array();
	}
	...
	foreach ( $files as $file ) {
		if ( '.' === $file[0] || is_dir( _LANG_DIR . "$dir/$file" ) ) {
			continue;
		}
		if ( substr( $file, -3 ) !== '.po' ) {
			continue;
		...
		$language_data[ $textdomain ][ $language ] = _get_pomo_file_data( _LANG_DIR . "$dir/$file" );
	}
 ...
 */
function _get_pomo_file_data( $po_file ) {
 ...
 * @see _get_available_translations()
 *
 ...
 */
function _dropdown_languages( $args = array() ) {
	...
	if ( empty( $translations ) ) {
		...
		$translations = _get_available_translations();
	}
...
function is_rtl() {
	global $_locale;
	if ( ! ( $_locale instanceof _Locale ) ) {
		return false;
	}
	return $_locale->is_rtl();
}
...
function switch_to_locale( $locale ) {
	/* @var _Locale_Switcher $_locale_switcher */
	global $_locale_switcher;

	return $_locale_switcher->switch_to_locale( $locale );
}
...
function restore_previous_locale() {
	...
	return $_locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $_locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $_locale_switcher->is_switched();
}
 ...
 */
function _get_list_item_separator() {
	global $_locale;

	return $_locale->get_list_item_separator();
}
