 ...
 * By default,  uses this class to instantiate the global $db object, providing
 * access to the  database.
 *
 * It is possible to replace this class with your own by setting the $db global variable
 * in content/db.php file to your class. The db class will still be included, so you can
 * extend it or simply use your own.
 *
 * @link https://developer..org/reference/classes/db/
 *
 * @since 0.71
 */
class db {
	 ...
	 *
	 * Default is to show errors if both _DEBUG and _DEBUG_DISPLAY evaluate to true.
	 *
	 * @since 0.71
	 *
	 * @var bool
	 ...
	 *  table prefix.
	 *
	 * You can set this to have multiple  installations in a single database.
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		// Use the `mysqli` extension if it exists unless `_USE_EXT_MYSQL` is defined as true.
		if ( function_exists( 'mysqli_connect' ) ) {
			$this->use_mysqli = true;

			if ( defined( '_USE_EXT_MYSQL' ) ) {
				$this->use_mysqli = ! _USE_EXT_MYSQL;
			}
		}
	 ...
	 * @return string|_Error Old prefix or _Error on error.
	 */
	public function set_prefix( $prefix, $set_table_names = true ) {

		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new _Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	 ...
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the  users
	 ...
	 * Examples:
	 *
	 *     $db->prepare( "SELECT * FROM `table` WHERE `column` = %s AND `field` = %d OR `other_field` LIKE %s", array( 'foo', 1337, '%bar' ) );
	 ...
	 *                Call db::prepare() or db::_real_escape() next.
	 */
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			...
			if ( $this->dbh->connect_errno ) {
				 ...
				 *  - _USE_EXT_MYSQL isn't set to false, and
				 *  - ext/mysql is loaded.
				 */
				...
				} elseif ( defined( '_USE_EXT_MYSQL' ) && ! _USE_EXT_MYSQL ) {
					$attempt_fallback = false;
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			if ( file_exists( _CONTENT_DIR . '/db-error.php' ) ) {
				require_once _CONTENT_DIR . '/db-error.php';
				die();
			}
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			...
			if ( $this->reconnect_retries === $tries && _DEBUG ) {
				error_reporting( $error_reporting );
			}
		...
		// If template_redirect has already happened, it's too late for _die()/dead_db().
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( ' database error: Could not perform query because it contains invalid data.' );
	...
	public function placeholder_escape() {
		...
		if ( ! $placeholder ) {
			...
			// Old  installs may not have AUTH_SALT defined.
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( ' database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( ' database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|_Error Table character set, _Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|_Error|null $charset The character set to use, _Error object
		...
		if ( ! $results ) {
			return new _Error( 'db_get_table_charset_failure', __( 'Could not retrieve table charset.' ) );
		}
	 ...
	 * @return string|false|_Error Column character set as a string. False if the column has
	 *                               no character set. _Error object if there was an error.
	 */
	 ...
	 *     example, numeric column), _Error object if there was an error.
	 *
	 ...
	 * @return array|_Error The $data parameter, with invalid characters removed from each value.
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return new _Error( 'db_strip_invalid_text_failure', __( 'Could not strip invalid text.' ) );
			}
	 ...
	 * @return string|_Error The converted query, or a _Error object if the conversion fails.
	 */
	 ...
	 * @return string|_Error The converted string, or a _Error object if the conversion fails.
	 */
	...
	protected function get_table_from_query( $query ) {
		...
		// SHOW TABLE STATUS and SHOW TABLES WHERE Name = '_posts'
		 ...
		 * strip the trailing % and unescape the _ to get '_123_'
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( '_Error', false ) ) {
				$this->error = new _Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	...
	public function check_database_version() {
		global $_version, $required_mysql_version;
		// Make sure the server has the required MySQL version.
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1:  version number, 2: Minimum required MySQL version number. */
			return new _Error( 'database_version', sprintf( __( '<strong>Error</strong>:  %1$s requires MySQL %2$s or higher' ), $_version, $required_mysql_version ) );
		}
	}
	...
	/**
	 * Retrieves a comma-separated list of the names of the functions that called db.
	 *
	 * @since 2.5.0
	 *
	...
	public function get_caller() {
		return _debug_backtrace_summary( __CLASS__ );
	}
