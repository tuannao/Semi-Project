...
class _REST_Server {
	 ...
	 * @since 4.4.0
	 *
	 * @return _Error|null _Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a _Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A _Error instance can be returned if an error occurs, and this should
		 ...
		 * @since 4.4.0
		 *
		 * @param _Error|null|true $errors _Error if authentication error, null if authentication
	 ...
	 *
	 * Note: This should only be used in _REST_Server::serve_request(), as it
	 * cannot handle _Error internally. All callbacks and other internal methods
	 * should instead return a _Error with the data set to an array that includes
	 * a 'status' key, with the value being the HTTP status to send.
	 *
	 * @since 4.4.0
	 *
	 * @param string $code    _Error-style code.
	 ...
	 * @since 4.4.0
	 *
	 * @see _REST_Server::dispatch()
	 *
	 * @global _User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var _User|null $current_user */
		global $current_user;

		if ( $current_user instanceof _User && ! $current_user->exists() ) {
			/*
		...
		$expose_headers = array( 'X-Total', 'X-TotalPages', 'Link' );
		...
		if ( $send_no_cache_headers ) {
			foreach ( _get_nocache_headers() as $header => $header_value ) {
		...
		$request = new _REST_Request( $_SERVER['REQUEST_METHOD'], $path );

		$request->set_query_params( _unslash( $_GET ) );
		$request->set_body_params( _unslash( $_POST ) );
		$request->set_file_params( $_FILES );
		$request->set_headers( $this->get_headers( _unslash( $_SERVER ) ) );
		...
		// Normalize to either _Error or _REST_Response...
		$result = rest_ensure_response( $result );

		// ...then convert _Error across.
		if ( is__error( $result ) ) {
		 ...
		 *
		 * @param _HTTP_Response $result  Result to send to the client. Usually a `_REST_Response`.
		 * @param _REST_Server   $server  Server instance.
		...
		if ( ! $served ) {
			...
			$result = _json_encode( $result );

			$json_error_message = $this->get_json_last_error();

			if ( $json_error_message ) {
				$this->set_status( 500 );
				$json_error_obj = new _Error(
					'rest_encode_error',
					$json_error_message,
					array( 'status' => 500 )
				);
				...
				$result = _json_encode( $result->data );
			}
	 ...
	 *
	 * @param _REST_Response $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param _REST_Response $response Response to extract links from.
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = _REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
	 ...
	 * @param _REST_Response $response Response object.
	 * @param bool|string[]    $embed    Whether to embed all links, a filtered list of link relations, or no links.
	 * @return _REST_Response New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param _REST_Response $response Original response data.
		 */
	...
	public function get_routes( $namespace = '' ) {
		...
		if ( $namespace ) {
			$endpoints = _list_filter( $endpoints, array( 'namespace' => $namespace ) );
		}
		...
		foreach ( $endpoints as $route => &$handlers ) {
			...
			foreach ( $handlers as $key => &$handler ) {
				...
				$handler = _parse_args( $handler, $defaults );
	 ...
	 * @since 4.4.0
	 *
	 * @param _REST_Request $request Request to attempt dispatching.
	 * @return _REST_Response Response returned by the callback.
	 */
	public function dispatch( $request ) {
		...
		if ( ! is__error( $error ) ) {
			...
			if ( is__error( $check_required ) ) {
				$error = $check_required;
			} else {
				$check_sanitized = $request->sanitize_params();
				if ( is__error( $check_sanitized ) ) {
	 ...
	 * @return array|_Error The route and request handler on success or a _Error instance if no handler was found.
	 */
	 ...
	 * @param _Error|null   $response The current error object if any.
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param _REST_Response|_HTTP_Response|_Error|mixed $response Result to send to the client.
		 *                                                                   Usually a _REST_Response or _Error.
		...
		if ( ! is__error( $response ) && ! empty( $handler['permission_callback'] ) ) {
			$permission = call_user_func( $handler['permission_callback'], $request );

			if ( is__error( $permission ) ) {
				$response = $permission;
	 ...
	 * }
	 * @return _REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new _REST_Response( $available );
		$response->add_link( 'help', 'https://developer..org/rest-api/' );
		 ...
		 *
		 * @param _REST_Response $response Response data.
		 * @param _REST_Request  $request  Request data.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param _REST_Response $response REST API response.
	 */
	protected function add_active_theme_link_to_index( _REST_Response $response ) {
		...
		if ( $should_add ) {
			$theme = _get_theme();
			$response->add_link( 'https://api.w.org/active-theme', rest_url( '/v2/themes/' . $theme->get_stylesheet() ) );
		}
	}

	/**
	 * Exposes the site logo through the  REST API.
	 *
	 ...
	 * @param _REST_Response $response REST API response.
	 */
	protected function add_site_logo_to_index( _REST_Response $response ) {
	...
	/**
	 * Exposes the site icon through the  REST API.
	 *
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 ...
	 * @param _REST_Response $response REST API response.
	 */
	protected function add_site_icon_to_index( _REST_Response $response ) {
	...
	/**
	 * Exposes an image through the  REST API.
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 * @since 5.9.0
	 ...
	 */
	protected function add_image_to_index( _REST_Response $response, $image_id, $type ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param _REST_Request $request REST request instance.
	 * @return _REST_Response|_Error _REST_Response instance if the index was found,
	 *                                   _Error if the namespace isn't set.
	 */
	public function get_namespace_index( $request ) {
		 ...
		 *
		 * @param _REST_Response $response Response data.
		 * @param _REST_Request  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
	 ...
	 * @since 5.6.0
	 *
	 * @param _REST_Request $batch_request The batch request object.
	 * @return _REST_Response The generated response object.
	 */
	public function serve_batch_request_v1( _REST_Request $batch_request ) {
		$requests = array();

		foreach ( $batch_request['requests'] as $args ) {
			$parsed_url = _parse_url( $args['path'] );

			if ( false === $parsed_url ) {
				$requests[] = new _Error( 'parse_path_failed', __( 'Could not parse the path.' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new _REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );

			if ( ! empty( $parsed_url['query'] ) ) {
				$query_args = null; // Satisfy linter.
				_parse_str( $parsed_url['query'], $query_args );
		...
		return new _REST_Response( array( 'responses' => $responses ), _Http::MULTI_STATUS );
	}
