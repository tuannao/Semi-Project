<?php
/**
 * REST API: _REST_Menus_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class _REST_Menus_Controller extends _REST_Terms_Controller {
	 ...
	 * @return bool|_Error True if the request has read access, otherwise false or _Error object.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return bool|_Error True if the request has read access for the item, otherwise false or _Error object.
	 */
	...
	protected function get_term( $id ) {
		...
		$nav_term           = _get_nav_menu_object( $term );
	...
	public function prepare_item_for_response( $term, $request ) {
		$nav_menu = _get_nav_menu_object( $term );
	...
	protected function prepare_links( $term ) {
		...
		foreach ( $locations as $location ) {
			$url = rest_url( sprintf( '/v2/menu-locations/%s', $location ) );
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new _Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = _get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new _Error( 'rest_term_invalid', __( 'Parent term does not exist.' ), array( 'status' => 400 ) );
			}
		}

		$prepared_term = $this->prepare_item_for_database( $request );

		$term = _update_nav_menu_object( 0, _slash( (array) $prepared_term ) );
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			if ( ! isset( $prepared_term->{'menu-name'} ) ) {
				// _update_nav_menu_object() requires that the menu-name is always passed.
				$prepared_term->{'menu-name'} = $term->name;
			}

			$update = _update_nav_menu_object( $term->term_id, _slash( (array) $prepared_term ) );
	...
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new _Error( 'rest_trash_not_supported', sprintf( __( "Menus do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		$result = _delete_nav_menu( $term );

		if ( ! $result || is__error( $result ) ) {
			return new _Error( 'rest_cannot_delete', __( 'The menu cannot be deleted.' ), array( 'status' => 500 ) );
		}
	...
	protected function handle_auto_add( $menu_id, $request ) {
		...
		do_action( '_update_nav_menu', $menu_id );
	 ...
	 * @param int             $menu_id The menu id to update.
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True on success, a _Error on an error updating any of the locations.
	 */
