<?php
/**
 * REST API: _REST_Search_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.0.0
...
/**
 * Core class to search through all  content via the REST API.
 *
 * @since 5.0.0
 *
 * @see _REST_Controller
 */
class _REST_Search_Controller extends _REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof _REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), '_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|_Error True if the request has search access, _Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( is__error( $handler ) ) {
			return $handler;
		}

		$result = $handler->search_items( $request );

		if ( ! isset( $result[ _REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ _REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ _REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new _Error(
		...
		$ids = $result[ _REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ _REST_Search_Handler::RESULT_TOTAL ];
	 ...
	 * @return array|_Error List of valid subtypes, or _Error object on failure.
	 */
	public function sanitize_subtypes( $subtypes, $request, $parameter ) {
		$subtypes = _parse_slug_list( $subtypes );

		$subtypes = rest_parse_request_arg( $subtypes, $request, $parameter );
		if ( is__error( $subtypes ) ) {
			return $subtypes;
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Search_Handler|_Error Search handler for the request type, or _Error object on failure.
	 */
