<?php
/**
 * REST API: _REST_Posts_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class _REST_Posts_Controller extends _REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function __construct( $post_type ) {
		...
		$this->namespace = ! empty( $obj->rest_namespace ) ? $obj->rest_namespace : '/v2';

		$this->meta = new _REST_Post_Meta_Fields( $this->post_type );
	}
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( _REST_Server::CREATABLE ),
				),
	 ...
	 * @param _Post $post     The post been password checked.
	...
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal _Query parameter
		 ...
		 */
		foreach ( $parameter_mappings as $api_param => $_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$args[ $_param ] = $request[ $api_param ];
			}
		}
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * _Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters _Query arguments when querying posts via the REST API.
		 *
		 ...
		 *
		 * @param array           $args    Array of arguments for _Query.
		 * @param _REST_Request $request The REST API request.
		 */
		$args       = apply_filters( "rest_{$this->post_type}_query", $args, $request );
		$query_args = $this->prepare_items_query( $args, $request );

		$posts_query  = new _Query();
	 ...
	 * @return _Post|_Error Post object if ID is valid, _Error otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param _Post         $post    Post to check against.
	 * @param _REST_Request $request Request data to check.
	...
	public function create_item( $request ) {
		...
		if ( is__error( $prepared_post ) ) {
			return $prepared_post;
		}

		$prepared_post->post_type = $this->post_type;

		$post_id = _insert_post( _slash( (array) $prepared_post ), true, false );

		if ( is__error( $post_id ) ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param _Post         $post     Inserted or updated post object.
		...
		_after_insert_post( $post, false, null );
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to update the item, _Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		// Convert the post object to an array, otherwise _update_post() will expect non-escaped input.
		$post_id = _update_post( _slash( (array) $post ), true, false );
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-posts-controller.php */
		...
		// Filter is fired in _REST_Attachments_Controller subclass.
	...
	public function delete_item( $request ) {
		 ...
		 * @param _Post $post           The Post object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			// (Note that internally this falls through to `_delete_post()`
			// if the Trash is disabled.)
			$result   = _trash_post( $id );
			$post     = get_post( $id );
			$response = $this->prepare_item_for_response( $post, $request );
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param _Post          $post     The deleted or trashed post.
		 * @param _REST_Response $response The response data.
		 * @param _REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared _Query arguments. Default empty array.
	 * @param _REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$query_args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
		}
		...
		// Map to proper _Query orderby param.
	 ...
	 *
	 * @param _REST_Request $request Request object.
	 * @return stdClass|_Error Post object or _Error.
	 */
	 ...
	 * @return true|_Error True if the status is valid, or _Error if not.
	 */
	public function check_status( $status, $request, $param ) {
		if ( $request['id'] ) {
			$post = $this->get_post( $request['id'] );

			if ( ! is__error( $post ) && $post->post_status === $status ) {
				return true;
			}
		}
	 ...
	 * @param _Post_Type $post_type   Post type.
	 * @return string|_Error Post status or _Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|_Error Whether the post thumbnail was successfully deleted, otherwise _Error.
	 */
	 ...
	 * @param _REST_Request $request  Request.
	 * @return bool|_Error True if template is still valid or if the same as existing value, or false if template not supported.
	 */
	public function check_template( $template, $request ) {
		...
		// If this is a create request, get_post() will return null and  theme will fallback to the passed post type.
		$allowed_templates = _get_theme()->get_page_templates( $post, $this->post_type );
	...
	public function handle_template( $template, $post_id, $validate = false ) {
		...
		if ( $validate && ! array_key_exists( $template, _get_theme()->get_page_templates( get_post( $post_id ) ) ) ) {
			$template = '';
		}

		update_post_meta( $post_id, '__page_template', $template );
	}
	 ...
	 * @param _REST_Request $request The request object with post and terms data.
	 * @return null|_Error _Error on an error assigning any of the terms, otherwise null.
	 */
	protected function handle_terms( $post_id, $request ) {
		$taxonomies = _list_filter( get_object_taxonomies( $this->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$result = _set_object_terms( $post_id, $request[ $base ], $taxonomy->name );
	 ...
	 * @since 4.7.0
	 *
	 * @param _Post_Type|string $post_type Post type name or object.
	 ...
	 *
	 * @param _Post         $item    Post object.
	 * @param _REST_Request $request Request object.
	 * @return _REST_Response Response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$data[ $base ] = $terms ? array_values( _list_pluck( $terms, 'term_id' ) ) : array();
			}
		}
	 ...
	 *
	 * By default,  will show password protected posts with a title of
	...
	protected function prepare_links( $post ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( '/v2/types/' . $this->post_type ),
			),
		);
		...
		if ( in_array( $post->post_type, array( 'post', 'page' ), true ) || post_type_supports( $post->post_type, 'revisions' ) ) {
			$revisions       = _get_post_revisions( $post->ID, array( 'fields' => 'ids' ) );
	...
	public function get_collection_params() {
		...
		$query_params['slug'] = array(
			...
			'sanitize_callback' => '_parse_slug_list',
		);
		 ...
		 * collection parameter to an internal _Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set _Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|_Error A list of valid statuses, otherwise _Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = _parse_slug_list( $statuses );

		// The default status is different in _REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    _Query arguments.
	 * @param _REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, _REST_Request $request ) {
