<?php
/**
 * REST API: _REST_Comments_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class _REST_Comments_Controller extends _REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var _REST_Comment_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new _REST_Comment_Meta_Fields();
	}
	 ...
	 * @return true|_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response|_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		foreach ( $parameter_mappings as $api_param => $_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$prepared_args[ $_param ] = $request[ $api_param ];
			}
		}
		...
		/**
		 * Filters _Comment_Query arguments when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		 * @link https://developer..org/reference/classes/_comment_query/
		 *
		 * @param array           $prepared_args Array of arguments for _Comment_Query.
		 * @param _REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_comment_query', $prepared_args, $request );

		$query        = new _Comment_Query;
		...
		$response->header( 'X-Total', $total_comments );
		$response->header( 'X-TotalPages', $max_pages );
	 ...
	 * @return _Comment|_Error Comment object if ID is valid, _Error otherwise.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has read access for the item, error object otherwise.
	 */
	public function get_item_permissions_check( $request ) {
		$comment = $this->get_comment( $request['id'] );
		if ( is__error( $comment ) ) {
			return $comment;
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param _REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		...
		// Setting remaining values before _insert_comment so we can use _allow_comment().
		...
		$check_comment_lengths = _check_comment_data_max_lengths( $prepared_comment );

		if ( is__error( $check_comment_lengths ) ) {
		...
		$prepared_comment['comment_approved'] = _allow_comment( $prepared_comment, true );

		if ( is__error( $prepared_comment['comment_approved'] ) ) {
		 ...
		 *
		 * Allows modification of the comment right before it is inserted via _insert_comment().
		 * Returning a _Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a _Error to short-circuit insertion.
		 *
		 * @param array|_Error  $prepared_comment The prepared comment data for _insert_comment().
		 * @param _REST_Request $request          Request used to insert the comment.
		 */
		...
		$comment_id = _insert_comment( _filter_comment( _slash( (array) $prepared_comment ) ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param _Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to update the item, error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			$check_comment_lengths = _check_comment_data_max_lengths( $prepared_args );
			...
			$updated = _update_comment( _slash( (array) $prepared_args ), true );

			if ( is__error( $updated ) ) {
				return new _Error(
					'rest_comment_failed_edit',
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-comments-controller.php */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param _Comment $comment        The comment object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			$result   = _delete_comment( $comment->comment_ID, true );
			...
			$result   = _trash_comment( $comment->comment_ID );
		 ...
		 * @since 4.7.0
		 *
		 * @param _Comment       $comment  The deleted comment data.
		 * @param _REST_Response $response The response returned from the API.
	 ...
	 *
	 * @param _Comment      $item    Comment object.
	 * @param _REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param _Comment        $comment  The original comment object.
		 * @param _REST_Request   $request  Request used to generate the response.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param _Comment $comment Comment object.
	...
	protected function prepare_links( $comment ) {
		...
		if ( 0 !== (int) $comment->user_id ) {
			$links['author'] = array(
				'href'       => rest_url( '/v2/users/' . $comment->user_id ),
				'embeddable' => true,
			);
		}
	 ...
	 *
	 * @param _REST_Request $request Request object.
	 * @return array|_Error Prepared comment, otherwise _Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_comment The prepared comment data for `_insert_comment`.
		 * @param _REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal _Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set _Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function handle_status_param( $new_status, $comment_id ) {
		$old_status = _get_comment_status( $comment_id );
		...
		switch ( $new_status ) {
			...
			case 'approve':
			case '1':
				$changed = _set_comment_status( $comment_id, 'approve' );
				break;
			case 'hold':
			case '0':
				$changed = _set_comment_status( $comment_id, 'hold' );
				break;
			case 'spam':
				$changed = _spam_comment( $comment_id );
				break;
			case 'unspam':
				$changed = _unspam_comment( $comment_id );
				break;
			case 'trash':
				$changed = _trash_comment( $comment_id );
				break;
			case 'untrash':
				$changed = _untrash_comment( $comment_id );
				break;
			default:
				$changed = false;
				break;
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		// Ensure the posts controller is specifically a _REST_Posts_Controller instance
		// before using methods specific to that controller.
		if ( ! $posts_controller instanceof _REST_Posts_Controller ) {
			$posts_controller = new _REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
	...
	protected function check_is_comment_content_allowed( $prepared_comment ) {
		 ...
		 * comment_content. See _handle_comment_submission().
		 */
