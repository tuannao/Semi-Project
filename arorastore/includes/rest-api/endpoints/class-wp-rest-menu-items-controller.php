<?php
/**
 * REST API: _REST_Menu_Items_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class _REST_Menu_Items_Controller extends _REST_Posts_Controller {
	 ...
	 * @return object|_Error Post object if ID is valid, _Error otherwise.
	 */
	protected function get_nav_menu_item( $id ) {
		...
		return _setup_nav_menu_item( $post );
	}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return bool|_Error True if the request has read access for the item, _Error object otherwise.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return bool|_Error Whether the current user has permission.
	 */
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new _Error( 'rest_post_exists', __( 'Cannot create existing post.' ), array( 'status' => 400 ) );
		}

		$prepared_nav_item = $this->prepare_item_for_database( $request );

		if ( is__error( $prepared_nav_item ) ) {
			return $prepared_nav_item;
		}
		$prepared_nav_item = (array) $prepared_nav_item;

		$nav_menu_item_id = _update_nav_menu_item( $prepared_nav_item['menu-id'], $prepared_nav_item['menu-item-db-id'], _slash( $prepared_nav_item ), false );
		if ( is__error( $nav_menu_item_id ) ) {
	...
	public function update_item( $request ) {
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-menu-items-controller.php */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response|_Error True on success, or _Error object on failure.
	 */
	public function delete_item( $request ) {
		$menu_item = $this->get_nav_menu_item( $request['id'] );
		if ( is__error( $menu_item ) ) {
			return $menu_item;
		}
		...
		if ( ! $request['force'] ) {
			...
			return new _Error( 'rest_trash_not_supported', sprintf( __( "Menu items do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}

		$previous = $this->prepare_item_for_response( get_post( $request['id'] ), $request );

		$result = _delete_post( $request['id'], true );

		if ( ! $result ) {
			return new _Error( 'rest_cannot_delete', __( 'The post cannot be deleted.' ), array( 'status' => 500 ) );
		}
	...
	protected function prepare_item_for_database( $request ) {
		...
		// Need to persist the menu item data. See https://core.trac..org/ticket/28138
		if ( ! is__error( $menu_item_obj ) ) {
			// Correct the menu position if this was the first item. See https://core.trac..org/ticket/28140
		...
		if ( ! $prepared_nav_item['menu-item-object'] ) {
			...
			if ( 'taxonomy' === $prepared_nav_item['menu-item-type'] ) {
				...
				if ( empty( $original ) || is__error( $original ) ) {
		...
		// The xfn and classes properties are arrays, but passed to _update_nav_menu_item as a string.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$term_ids = $terms ? array_values( _list_pluck( $terms, 'term_id' ) ) : array();
		 ...
		 * @param object           $menu_item Menu item setup by {@see _setup_nav_menu_item()}.
		 * @param _REST_Request  $request   Request object.
		 */
		...
		$schema['properties']['classes'] = array(
			...
			'arg_options' => array(
				'sanitize_callback' => function ( $value ) {
					return array_map( 'sanitize_html_class', _parse_list( $value ) );
				},
			),
		);
	...
	protected function get_menu_id( $menu_item_id ) {
		$menu_ids = _get_post_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
		$menu_id  = 0;
		if ( $menu_ids && ! is__error( $menu_ids ) ) {
