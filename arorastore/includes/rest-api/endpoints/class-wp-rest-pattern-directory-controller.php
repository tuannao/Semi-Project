<?php
/**
 * Block Pattern Directory REST API: _REST_Pattern_Directory_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.8.0
 ...
 *
 * This simply proxies the endpoint at http://api..org/patterns/1.0/. That isn't necessary for
 * functionality, but is desired for privacy. It prevents api..org from knowing the user's IP address.
 *
 * @since 5.8.0
 *
 * @see _REST_Controller
 */
class _REST_Pattern_Directory_Controller extends _REST_Controller {
	 ...
	 * @return true|_Error True if the request has permission, _Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		/*
		 * Include an unmodified `$_version`, so the API can craft a response that's tailored to
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://api..org/patterns/1.0/?' . build_query( $query_args );
			if ( _http_supports( array( 'ssl' ) ) ) {
			 ...
			 */
			$cache_ttl      = 5;
			$org_response = _remote_get( $api_url );
			$raw_patterns   = json_decode( _remote_retrieve_body( $org_response ) );

			if ( is__error( $org_response ) ) {
				$raw_patterns = $org_response;

			} elseif ( ! is_array( $raw_patterns ) ) {
				// HTTP request succeeded, but response data is invalid.
				$raw_patterns = new _Error(
					'pattern_api_failed',
					sprintf(
					...
					array(
						'response' => _remote_retrieve_body( $org_response ),
					)
				);
	 ...
	 *
	 * @param object          $item    Raw pattern from api..org, before any changes.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$prepared_pattern = array(
			...
			'content'        => _kses_post( $raw_pattern->pattern_content ),
			'categories'     => array_map( 'sanitize_title', $raw_pattern->category_slugs ),
			'keywords'       => array_map( 'sanitize_text_field', explode( ',', $raw_pattern->meta->op_keywords ) ),
			'description'    => sanitize_text_field( $raw_pattern->meta->op_description ),
			'viewport_width' => absint( $raw_pattern->meta->op_viewport_width ),
		);

		$prepared_pattern = $this->add_additional_fields_to_object( $prepared_pattern, $request );

		$response = new _REST_Response( $prepared_pattern );
	...
	protected function get_transient_key( $query_args ) {
		...
		if ( isset( $query_args['slug'] ) ) {
			...
			$query_args['slug'] = _parse_list( $query_args['slug'] );
		...
		return '_remote_block_patterns_' . md5( serialize( $query_args ) );
	}
}
