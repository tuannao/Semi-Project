<?php
/**
 * REST API: _REST_Users_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class _REST_Users_Controller extends _REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var _REST_User_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new _REST_User_Meta_Fields();
	}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has read access, otherwise _Error object.
	 */
	...
	public function get_items( $request ) {
		...
		/**
		 * Filters _User_Query arguments when querying users via the REST API.
		 *
		 * @link https://developer..org/reference/classes/_user_query/
		 *
		 * @since 4.7.0
		 *
		 * @param array           $prepared_args Array of arguments for _User_Query.
		 * @param _REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_user_query', $prepared_args, $request );

		$query = new _User_Query( $prepared_args );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new _User_Query( $prepared_args );
	 ...
	 * @return _User|_Error True if ID is valid, _Error otherwise.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has read access for the item, otherwise _Error object.
	 */
	...
	public function create_item( $request ) {
		...
		if ( is_multisite() ) {
			$ret = mu_validate_user_signup( $user->user_login, $user->user_email );

			if ( is__error( $ret['errors'] ) && $ret['errors']->has_errors() ) {
				$error = new _Error(
					'rest_invalid_param',
					__( 'Invalid user parameter(s).' ),
					array( 'status' => 400 )
		...
		if ( is_multisite() ) {
			$user_id = mu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			...
			$user->ID = $user_id;
			$user_id  = _update_user( _slash( (array) $user ) );
			...
			$user_id = _insert_user( _slash( (array) $user ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param _User         $user     Inserted or updated user object.
	...
	public function update_item( $request ) {
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-users-controller.php */
	...
	public function delete_item( $request ) {
		...
		// Include user admin functions to get access to _delete_user().
		require_once ABSPATH . 'admin/includes/user.php';

		$result = _delete_user( $id, $reassign );
		 ...
		 * @since 4.7.0
		 *
		 * @param _User          $user     The user data.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param _User          $user     User object used to create response.
		 * @param _REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|_Error True if the current user is allowed to make the role change,
	 *                       otherwise a _Error object.
	 */
	protected function check_role_update( $user_id, $roles ) {
		...
		foreach ( $roles as $role ) {
			...
			if ( ! isset( $_roles->role_objects[ $role ] ) ) {
				return new _Error(
			...
			$potential_role = $_roles->role_objects[ $role ];
	 ...
	 * @return string|_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param _REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal _User_Query parameter.  Use the
		 * `rest_user_query` filter to set _User_Query arguments.
		 *
		 * @since 4.7.0
		 *
