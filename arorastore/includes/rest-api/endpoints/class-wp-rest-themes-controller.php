<?php
/**
 * REST API: _REST_Themes_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class _REST_Themes_Controller extends _REST_Controller {
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return bool|_Error True if the request has read access for the item, otherwise _Error object.
	 */
	public function get_item_permissions_check( $request ) {
		...
		$_theme      = _get_theme( $request['stylesheet'] );
		$current_theme = _get_theme();

		if ( $this->is_same_theme( $_theme, $current_theme ) ) {
			return $this->check_read_active_theme_permission();
		}
	 ...
	 * @since 5.7.0
	 *
	 * @return bool|_Error Whether the theme can be read.
	 */
	...
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $_theme, $request );
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-Total', count( $themes ) );
		$response->header( 'X-TotalPages', 1 );
	 ...
	 *
	 * @param _Theme        $item    Theme object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $theme->get_stylesheet() === _get_theme()->get_stylesheet() ) {
			// This creates a record for the active theme if not existent.
			$id = _Theme_JSON_Resolver::get_user_global_styles_post_id();
		} else {
			$user_cpt = _Theme_JSON_Resolver::get_user_data_from__global_styles( $theme );
		 ...
		 * @param _Theme         $theme    Theme object used to create response.
		 * @param _REST_Request  $request  Request object.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param _Theme $theme_a First theme to compare.
	 * @param _Theme $theme_b Second theme to compare.
	 * @return bool
	 */
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'requires_'    => array(
					'description' => __( 'The minimum  version required for the theme to work.' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'screenshot'     => array(
