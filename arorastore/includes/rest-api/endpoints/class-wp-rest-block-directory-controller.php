<?php
/**
 * REST API: _REST_Block_Directory_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class _REST_Block_Directory_Controller extends _REST_Controller {
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$block = array(
			...
			'description'         => _trim_words( $plugin['short_description'], 30, '...' ),
			'id'                  => $plugin['slug'],
			...
			'author'              => _strip_all_tags( $plugin['author'] ),
		...
		$response = new _REST_Response( $block );
	 ...
	 * @since 5.5.0
	 *
	 * @param array $plugin The plugin data from .org.
	 * @return array
	 */
	protected function prepare_links( $plugin ) {
		$links = array(
			'https://api.w.org/install-plugin' => array(
				'href' => add_query_arg( 'slug', urlencode( $plugin['slug'] ), rest_url( '/v2/plugins' ) ),
			),
		);
		...
		if ( $plugin_file ) {
			$links['https://api.w.org/plugin'] = array(
				'href'       => rest_url( '/v2/plugins/' . substr( $plugin_file, 0, - 4 ) ),
				'embeddable' => true,
			);
		}
	 ...
	 * @since 5.5.0
	 *
	 * @param string $slug The .org directory slug for a plugin.
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'author'              => array(
					'description' => __( 'The .org username of the block author.' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'icon'                => array(
