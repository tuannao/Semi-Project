<?php
/**
 * REST API: _REST_Revisions_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class _REST_Revisions_Controller extends _REST_Controller {
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $this->parent_controller ) {
			$this->parent_controller = new _REST_Posts_Controller( $parent_post_type );
		}
	}
	...
	public function get_items( $request ) {
		...
		if ( _revisions_enabled( $parent ) ) {
			...
			/** This filter is documented in includes/rest-api/endpoints/class-rest-posts-controller.php */
			$args       = apply_filters( 'rest_revision_query', $args, $request );
			$query_args = $this->prepare_items_query( $args, $request );

			$revisions_query = new _Query();
		...
		$response->header( 'X-Total', (int) $total_revisions );
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $response || is__error( $response ) ) {
			return $response;
		}
	...
	public function delete_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param _Post|false|null $result The revision object (if it was deleted or moved to the Trash successfully)
	 ...
	 * @param _Post $post    Post revision object.
