<?php
/**
 * REST API: _REST_Terms_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class _REST_Terms_Controller extends _REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->namespace = ! empty( $tax_obj->rest_namespace ) ? $tax_obj->rest_namespace : '/v2';

		$this->meta = new _REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 * @return true|_Error True if the request has read access, otherwise false or _Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @link https://developer..org/reference/functions/get_terms/
		 *
		...
		if ( ! empty( $prepared_args['post'] ) ) {
			$query_result = _get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling _count_terms() below.
		...
		$total_terms = _count_terms( $count_args );

		// _count_terms() can return a falsey value when the term has no children.
		if ( ! $total_terms ) {
			$total_terms = 0;
		}
	 ...
	 * @return _Term|_Error Term object if ID is valid, _Error otherwise.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has read access for the item, otherwise false or _Error object.
	 */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to create items, false or _Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		$term = _insert_term( _slash( $prepared_term->name ), $this->taxonomy, _slash( (array) $prepared_term ) );
		if ( is__error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param _Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to update the item, false or _Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = _update_term( $term->term_id, $term->taxonomy, _slash( (array) $prepared_term ) );
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-terms-controller.php */
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to delete the item, otherwise false or _Error object.
	 */
	...
	public function delete_item( $request ) {
		...
		$retval = _delete_term( $term->term_id, $term->taxonomy );
		 ...
		 * @since 4.7.0
		 *
		 * @param _Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof _Term ) {
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param _Term           $item      The original term object.
	...
	protected function prepare_links( $term ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( sprintf( '/v2/taxonomies/%s', $this->taxonomy ) ),
			),
		);
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal _Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set _Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
		 * @param array       $query_params JSON Schema-formatted collection parameters.
		 * @param _Taxonomy $taxonomy     Taxonomy object.
		 */
