<?php
/**
 * REST API: _REST_Templates_Controller class
 *
 * @package    
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class _REST_Templates_Controller extends _REST_Controller {
	 ...
	 * requires using a path like //v2/templates/twentytwentytwo//home. There
	 * are special cases when  routing corrects the name to contain
	 ...
	 *
	 * @since 5.9.0
	 * @see https://core.trac..org/ticket/54507
	 *
	 ...
	 * @since 5.8.0
	 *
	 * @param _REST_Request $request The request instance.
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new _Error( 'rest_template_not_found', __( 'No templates exist with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has write access for the item, _Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		$post_before = get_post( $template->_id );

		if ( isset( $request['source'] ) && 'theme' === $request['source'] ) {
			_delete_post( $template->_id, true );
		...
		if ( is__error( $changes ) ) {
			return $changes;
		}

		if ( 'custom' === $template->source ) {
			$update = true;
			$result = _update_post( _slash( (array) $changes ), false );
		} else {
			$update      = false;
			$post_before = null;
			$result      = _insert_post( _slash( (array) $changes ), false );
		}
	...
	public function create_item( $request ) {
		...
		$post_id                  = _insert_post( _slash( (array) $prepared_post ), true );
		...
		$posts = get_block_templates( array( '_id' => $post_id ), $this->post_type );
		if ( ! count( $posts ) ) {
			return new _Error( 'rest_template_insert_error', __( 'No templates exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has delete access for the item, _Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		...
		if ( 'custom' !== $template->source ) {
			return new _Error( 'rest_invalid_template', __( 'Templates based on theme files can\'t be removed.' ), array( 'status' => 400 ) );
		}
	 ...
	 * @return stdClass Changes to pass to _update_post.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( null === $template ) {
			...
			$changes->tax_input   = array(
				'_theme' => isset( $request['theme'] ) ? $request['theme'] : _get_theme()->get_stylesheet(),
			);
		...
		if ( '_template_part' === $this->post_type ) {
			if ( isset( $request['area'] ) ) {
				$changes->tax_input['_template_part_area'] = _filter_block_template_part_area( $request['area'] );
			} elseif ( null !== $template && 'custom' !== $template->source && $template->area ) {
				$changes->tax_input['_template_part_area'] = _filter_block_template_part_area( $template->area );
			} elseif ( ! $template->area ) {
				$changes->tax_input['_template_part_area'] = _TEMPLATE_PART_AREA_UNCATEGORIZED;
			}
		}
	 ...
	 *
	 * @param _Block_Template $item    Template instance.
	 * @param _REST_Request   $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( rest_is_field_included( 'title.rendered', $fields ) ) {
			if ( $template->_id ) {
				...
				$data['title']['rendered'] = apply_filters( 'the_title', $template->title, $template->_id );
			} else {
		...
		if ( rest_is_field_included( '_id', $fields ) ) {
			$data['_id'] = (int) $template->_id;
		}
		...
		if ( rest_is_field_included( 'is_custom', $fields ) && '_template' === $template->type ) {
		...
		if ( rest_is_field_included( 'area', $fields ) && '_template_part' === $template->type ) {
