<?php
/**
 * REST API: _REST_Plugins_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class _REST_Plugins_Controller extends _REST_Controller {
				...
				array(
					...
					'args'                => array(
						'slug'   => array(
							'type'        => 'string',
							'required'    => true,
							'description' => __( '.org plugin directory slug.' ),
							'pattern'     => '[\w\-]+',
						),
						'status' => array(
	...
	public function get_items( $request ) {
		...
		foreach ( get_plugins() as $file => $data ) {
			if ( is__error( $this->check_read_permission( $file ) ) ) {
				continue;
			}
		...
		return new _REST_Response( $plugins );
	}
	 ...
	 * @return true|_Error True if can read, a _Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new _Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	 ...
	 * @since 5.5.0
	 *
	 * @global _Filesystem_Base $_filesystem  filesystem subclass.
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response|_Error Response object on success, or _Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( is__error( $filesystem_available ) ) {
		...
		if ( is_null( $result ) ) {
			// Pass through the error from _Filesystem if one was raised.
			if ( $_filesystem instanceof _Filesystem_Base
				&& is__error( $_filesystem->errors ) && $_filesystem->errors->has_errors()
			) {
				return new _Error(
					'unable_to_connect_to_filesystem',
					$_filesystem->errors->get_error_message(),
					array( 'status' => 500 )
				);
			}
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new _REST_Response( $data );
	 ...
	 * @return array|_Error The plugin data, or a _Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param _REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( _PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|_Error True if filesystem is available, _Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new _Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'requires_'  => array(
					'description' => __( 'Minimum required version of .' ),
					'type'        => 'string',
					'readonly'    => true,
					'context'     => array( 'view', 'edit', 'embed' ),
				),
