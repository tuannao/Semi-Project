<?php
/**
 * REST API: _REST_Widget_Types_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class _REST_Widget_Types_Controller extends _REST_Controller {
	 ...
	 * @since 5.8.0
	 *
	 * @return true|_Error True if the widget type is visible, _Error otherwise.
	 */
	 ...
	 * @return array|_Error The array of widget data if the name is valid, _Error otherwise.
	 */
	public function get_widget( $id ) {
		...
		return new _Error( 'rest_widget_type_invalid', __( 'Invalid widget type.' ), array( 'status' => 404 ) );
	}
	 ...
	 * @global array             $_registered_widgets The list of registered widgets.
	 *
	...
	protected function get_widgets() {
		...
		foreach ( $_registered_widgets as $widget ) {
			$parsed_id     = _parse_widget_id( $widget['id'] );
	 ...
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response Widget type data.
	 */
	...
	public function encode_form_data( $request ) {
		...
		$id            = $request['id'];
		$widget_object = $_widget_factory->get_widget_object( $id );

		if ( ! $widget_object ) {
			return new _Error(
				'rest_invalid_widget',
				__( 'Cannot preview a widget that does not extend _Widget.' ),
				array( 'status' => 400 )
			);
		}
		...
		$widget_key          = $_widget_factory->get_widget_key( $id );
		...
		$response = array(
			...
			'instance' => array(
				...
				'hash'    => _hash( $serialized_instance ),
			),
		);
	...
	/**
	 * Returns the output of _Widget::widget() when called with the provided
	...
	/**
	 * Returns the output of _Widget::form() when called with the provided
	 * instance. Used by encode_form_data() to preview a widget's form.
	 *
	 * @since 5.8.0
	 *
	 * @param _Widget $widget_object Widget object to call widget() on.
	 * @param array     $instance Widget instance settings.
	 * @return string
	 */
	...
	private function render_legacy_widget_preview_iframe( $id_base, $instance ) {
		...
		<div id="page" class="site">
			<div id="content" class="site-content">
				<?php
				$registry = _Block_Type_Registry::get_instance();
