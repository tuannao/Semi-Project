<?php
/**
 * REST API: _REST_Sidebars_Controller class
 *
 * Original code from {@link https://github.com/martin-pettersson/rest-api-sidebars Martin Pettersson (martin_pettersson@outlook.com)}.
 *
 * @package 
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class _REST_Sidebars_Controller extends _REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		foreach ( _get_sidebars_widgets() as $id => $widgets ) {
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response Response object on success.
	 */
	public function get_items( $request ) {
		...
		foreach ( _get_sidebars_widgets() as $id => $widgets ) {
			...
			if ( is__error( $permissions_check ) && ! $this->check_read_permission( $sidebar ) ) {
				continue;
			}
	...
	public function get_item( $request ) {
		...
		$sidebar = $this->get_sidebar( $request['id'] );
		if ( ! $sidebar ) {
			return new _Error( 'rest_sidebar_not_found', __( 'No sidebar exists with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response Response object on success, or _Error object on failure.
	 */
		...
		if ( isset( $request['widgets'] ) ) {
			...
			foreach ( $sidebars as $sidebar_id => $widgets ) {
				foreach ( $widgets as $i => $widget_id ) {
					...
					if ( $sidebar_id === $request['id'] && ! in_array( $widget_id, $request['widgets'], true ) ) {
						$sidebars['_inactive_widgets'][] = $widget_id;
					}
				}
			}
	 ...
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response Prepared response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( isset( $_registered_sidebars[ $id ] ) ) {
			$registered_sidebar = $_registered_sidebars[ $id ];

			$sidebar['status']        = 'active';
			$sidebar['name']          = isset( $registered_sidebar['name'] ) ? $registered_sidebar['name'] : '';
			$sidebar['description']   = isset( $registered_sidebar['description'] ) ? _sidebar_description( $id ) : '';
		...
		if ( rest_is_field_included( 'widgets', $fields ) ) {
			...
			$widgets  = array_filter(
				...
				static function ( $widget_id ) use ( $_registered_widgets ) {
					return isset( $_registered_widgets[ $widget_id ] );
				}
			);
	...
	protected function prepare_links( $sidebar ) {
		return array(
			...
			'https://api.w.org/widget' => array(
				'href'       => add_query_arg( 'sidebar', $sidebar['id'], rest_url( '//v2/widgets' ) ),
				'embeddable' => true,
			),
		);
	}
