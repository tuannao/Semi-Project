<?php
/**
 * REST API: _REST_Block_Types_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class _REST_Block_Types_Controller extends _REST_Controller {

	/**
	 * Instance of _Block_Type_Registry.
	 *
	 * @since 5.5.0
	 * @var _Block_Type_Registry
	 */
	protected $block_registry;

	/**
	 * Instance of _Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var _Block_Styles_Registry
	 */
	...
	public function __construct() {
		...
		$this->block_registry = _Block_Type_Registry::get_instance();
		$this->style_registry = _Block_Styles_Registry::get_instance();
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|_Error True if the block type is visible, _Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new _Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return _Block_Type|_Error Block type object if name is valid, _Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new _Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param _Block_Type   $item    Block type data.
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response Block type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( rest_is_field_included( 'styles', $fields ) ) {
			...
			$data['styles'] = _parse_args( $styles, $data['styles'] );
		 ...
		 * @param _Block_Type    $block_type The original block type object.
	 ...
	 * @since 5.5.0
	 *
	 * @param _Block_Type $block_type Block type data.
	...
	protected function prepare_links( $block_type ) {
		...
		if ( $block_type->is_dynamic() ) {
			$links['https://api.w.org/render-block'] = array(
				'href' => add_query_arg( 'context', 'edit', rest_url( sprintf( '%s/%s/%s', '/v2', 'block-renderer', $block_type->name ) ) ),
			);
		}
