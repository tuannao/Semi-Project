<?php
/**
 * REST API: _REST_Autosaves_Controller class.
 *
 * @package 
 * @subpackage REST_API
 * @since 5.0.0
 ...
 * @since 5.0.0
 *
 * @see _REST_Revisions_Controller
 * @see _REST_Controller
 */
class _REST_Autosaves_Controller extends _REST_Revisions_Controller {
	 ...
	 *
	 * @since 5.0.0
	 * @var _REST_Revisions_Controller
	 */
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $parent_controller ) {
			$parent_controller = new _REST_Posts_Controller( $parent_post_type );
		}

		$this->parent_controller    = $parent_controller;
		$this->revisions_controller = new _REST_Revisions_Controller( $parent_post_type );
		$this->rest_base            = 'autosaves';
		$this->namespace            = ! empty( $post_type_object->rest_namespace ) ? $post_type_object->rest_namespace : '/v2';
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->parent_controller->get_endpoint_args_for_item_schema( _REST_Server::EDITABLE ),
				),
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return true|_Error True if the request has access to create the item, _Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ( 'draft' === $post->post_status || 'auto-draft' === $post->post_status ) && $post->post_author == $user_id ) {
			...
			// Convert the post object to an array and add slashes, _update_post() expects escaped array.
			$autosave_id = _update_post( _slash( (array) $prepared_post ), true );
		} else {
			// Non-draft posts: create or update the post autosave.
			$autosave_id = $this->create_post_autosave( (array) $prepared_post );
		}

		if ( is__error( $autosave_id ) ) {
			return $autosave_id;
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _Post|_Error Revision post object if ID is valid, _Error otherwise.
	 */
	public function get_item( $request ) {
		...
		$autosave = _get_post_autosave( $parent_id );
	...
	/**
	 * Gets a collection of autosaves using _get_post_autosave.
	 *
	...
	public function get_items( $request ) {
		...
		$response  = array();
		$parent_id = $parent->ID;
		$revisions = _get_post_revisions( $parent_id, array( 'check_enabled' => false ) );
	 ...
	 * @return mixed The autosave revision ID or _Error.
	 */
	public function create_post_autosave( $post_data ) {
		...
		$old_autosave = _get_post_autosave( $post_id, $user_id );

		if ( $old_autosave ) {
			$new_autosave                = __post_revision_data( $post_data, true );
			...
			foreach ( array_intersect( array_keys( $new_autosave ), array_keys( __post_revision_fields( $post ) ) ) as $field ) {
			...
			if ( ! $autosave_is_different ) {
				_delete_post_revision( $old_autosave->ID );
				return new _Error(
			...
			do_action( '_creating_autosave', $new_autosave );

			// _update_post() expects escaped array.
			return _update_post( _slash( $new_autosave ) );
		}

		// Create the new autosave as a special post revision.
		return __put_post_revision( $post_data, true );
	}
	 ...
	 *
	 * @param _Post         $item    Post revision object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'preview_link', $fields, true ) ) {
			$parent_id          = _is_post_autosave( $post );
			$preview_post_id    = false === $parent_id ? $post->ID : $parent_id;
			$preview_query_args = array();

			if ( false !== $parent_id ) {
				$preview_query_args['preview_id']    = $parent_id;
				$preview_query_args['preview_nonce'] = _create_nonce( 'post_preview_' . $parent_id );
			}
		 ...
		 * @param _Post          $post     The original revision object.
