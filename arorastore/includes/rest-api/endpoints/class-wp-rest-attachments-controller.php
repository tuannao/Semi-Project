<?php
/**
 * REST API: _REST_Attachments_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see _REST_Posts_Controller
 */
class _REST_Attachments_Controller extends _REST_Posts_Controller {
	 ...
	 * @param _REST_Request $request       Optional. Request to prepare items for.
	 ...
	 * @return true|_Error Boolean true if the attachment may be created, or a _Error if not.
	 */
	public function create_item_permissions_check( $request ) {
		$ret = parent::create_item_permissions_check( $request );

		if ( ! $ret || is__error( $ret ) ) {
			return $ret;
		}
	 ...
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response|_Error Response object on success, _Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $attachment_id, '__attachment_image_alt', sanitize_text_field( $request['alt_text'] ) );
		}
		 ...
		 * @since 5.0.0
		 *
		 * @param _Post         $attachment Inserted or updated attachment object.
		 * @param _REST_Request $request    Request object.
		 * @param bool            $creating   True when creating an attachment, false when updating.
		 */
		do_action( 'rest_after_insert_attachment', $attachment, $request, true );

		_after_insert_post( $attachment, false, null );

		if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
			// Set a custom header with the attachment_id.
			// Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			header( 'X-Upload-Attachment-ID: ' . $attachment_id );
		}

		// Include media and image functions to get access to _generate_attachment_metadata().
	...
	protected function insert_attachment( $request ) {
		...
		// Include image functions to get access to _read_image_metadata().
		require_once ABSPATH . 'admin/includes/image.php';

		// Use image exif/iptc data for title and caption defaults if possible.
		$image_meta = _read_image_metadata( $file );
		...
		if ( empty( $attachment->post_title ) ) {
			$attachment->post_title = preg_replace( '/\.[^.]+$/', '', _basename( $file ) );
		}

		// $post_parent is inherited from $attachment['post_parent'].
		$id = _insert_attachment( _slash( (array) $attachment ), $file, 0, true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param _Post         $attachment Inserted or updated attachment
		 *                                    object.
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $data['id'], '__attachment_image_alt', $request['alt_text'] );
		}
		...
		/** This action is documented in includes/rest-api/endpoints/class-rest-attachments-controller.php */
		do_action( 'rest_after_insert_attachment', $attachment, $request, false );

		_after_insert_post( $attachment, true, $attachment_before );
	...
	public function post_process_item( $request ) {
		switch ( $request['action'] ) {
			case 'create-image-subsizes':
				require_once ABSPATH . 'admin/includes/image.php';
				_update_image_subsizes( $request['id'] );
				break;
		}
	...
	public function edit_media_item( $request ) {
		...
		$image_file = _get_original_image_path( $attachment_id );
		$image_meta = _get_attachment_metadata( $attachment_id );

		if (
			! $image_meta ||
			! $image_file ||
			! _image_file_matches_image_meta( $request['src'], $image_meta, $attachment_id )
		) {
			return new _Error(
		...
		$image_editor = _get_image_editor( $image_file_to_edit );

		if ( is__error( $image_editor ) ) {
			return new _Error(
		...
		$image_name = _basename( $image_file, ".{$image_ext}" );

		// Do not append multiple `-edited` to the file name.
		// The user may be editing a previously edited image.
		if ( preg_match( '/-edited(-\d+)?$/', $image_name ) ) {
			// Remove any `-1`, `-2`, etc. `_unique_filename()` will add the proper number.
		...
		$filename = _unique_filename( $uploads['path'], $filename );
		...
		$new_attachment_id = _insert_attachment( _slash( $new_attachment_post ), $saved['path'], 0, true );

		if ( is__error( $new_attachment_id ) ) {
		...
		$image_alt = get_post_meta( $attachment_id, '__attachment_image_alt', true );

		if ( ! empty( $image_alt ) ) {
			// update_post_meta() expects slashed.
			update_post_meta( $new_attachment_id, '__attachment_image_alt', _slash( $image_alt ) );
		}

		if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
			// Set a custom header with the attachment_id.
			// Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			header( 'X-Upload-Attachment-ID: ' . $new_attachment_id );
		}

		// Generate image sub-sizes and meta.
		$new_image_meta = _generate_attachment_metadata( $new_attachment_id, $saved['path'] );
		...
		$new_image_meta['parent_image'] = array(
			...
			'file'          => __relative_upload_path( $image_file ),
		);
		 ...
		 */
		$new_image_meta = apply_filters( '_edited_image_metadata', $new_image_meta, $new_attachment_id, $attachment_id );

		_update_attachment_metadata( $new_attachment_id, $new_image_meta );
	 ...
	 *
	 * @param _REST_Request $request Request object.
	 * @return stdClass|_Error Post object.
	 */
	 ...
	 *
	 * @param _Post         $item    Attachment object.
	 * @param _REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'alt_text', $fields, true ) ) {
			$data['alt_text'] = get_post_meta( $post->ID, '__attachment_image_alt', true );
		}

		if ( in_array( 'media_type', $fields, true ) ) {
			$data['media_type'] = _attachment_is_image( $post->ID ) ? 'image' : 'file';
		}
		...
		if ( in_array( 'media_details', $fields, true ) ) {
			$data['media_details'] = _get_attachment_metadata( $post->ID );
			...
			if ( empty( $data['media_details'] ) ) {
				...
				foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
					...
					$image_src = _get_attachment_image_src( $post->ID, $size );
					if ( ! $image_src ) {
						continue;
					}
				...
				$full_src = _get_attachment_image_src( $post->ID, 'full' );

				if ( ! empty( $full_src ) ) {
					$data['media_details']['sizes']['full'] = array(
						'file'       => _basename( $full_src[0] ),
						'width'      => $full_src[1],
						'height'     => $full_src[2],
		...
		if ( in_array( 'source_url', $fields, true ) ) {
			$data['source_url'] = _get_attachment_url( $post->ID );
		}

		if ( in_array( 'missing_image_sizes', $fields, true ) ) {
			require_once ABSPATH . 'admin/includes/image.php';
			$data['missing_image_sizes'] = array_keys( _get_missing_image_subsizes( $post->ID ) );
		}
		 ...
		 * @param _Post          $post     The original attachment post.
	 ...
	 * @return array|_Error Data from _handle_sideload().
	 */
	protected function upload_from_data( $data, $headers ) {
		...
		// Include filesystem functions to get access to _tempnam() and _handle_sideload().
		require_once ABSPATH . 'admin/includes/file.php';

		// Save the file.
		$tmpfname = _tempnam( $filename );
		...
		$sideloaded = _handle_sideload( $file_data, $overrides );
	 ...
	 * @param array $headers HTTP headers from the request.
	 * @return array|_Error Data from _handle_upload().
	 */
	protected function upload_from_file( $files, $headers ) {
		...
		// Pass off to  to handle the actual upload.
		$overrides = array(
			'test_form' => false,
		);
		...
		if ( defined( 'DIR_TESTDATA' ) && DIR_TESTDATA ) {
			$overrides['action'] = '_handle_mock_upload';
		}
		...
		// Include filesystem functions to get access to _handle_upload().
		require_once ABSPATH . 'admin/includes/file.php';

		$file = _handle_upload( $files['file'], $overrides );
	 ...
	 * @return true|_Error True if can upload, error for errors.
	 */
