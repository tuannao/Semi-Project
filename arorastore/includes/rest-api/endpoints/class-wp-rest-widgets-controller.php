<?php
/**
 * REST API: _REST_Widgets_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class _REST_Widgets_Controller extends _REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( _get_sidebars_widgets() as $sidebar_id => $widget_ids ) {
			...
			if ( is__error( $permissions_check ) && ! $this->check_read_sidebar_permission( $sidebar_id ) ) {
				continue;
			}
		...
		return new _REST_Response( $prepared );
	}
	...
	public function get_item_permissions_check( $request ) {
		...
		$sidebar_id = _find_widgets_sidebar( $widget_id );
	...
	protected function check_read_sidebar_permission( $sidebar_id ) {
		$sidebar = _get_sidebar( $sidebar_id );
	...
	public function create_item( $request ) {
		...
		if ( is__error( $widget_id ) ) {
			return $widget_id;
		}

		_assign_widget_to_sidebar( $widget_id, $sidebar_id );
	...
	public function update_item( $request ) {
		 ...
		 * _inactive_widgets sidebar based on the contents of the $sidebars_widgets global.
		 *
		 * When batch requests are processed, this global is not properly updated by previous
		 * calls, resulting in widgets incorrectly being moved to the _inactive_widgets
		 * sidebar.
		 *
		 * See https://core.trac..org/ticket/53657.
		 */
		_get_sidebars_widgets();
		$this->retrieve_widgets();

		$widget_id  = $request['id'];
		$sidebar_id = _find_widgets_sidebar( $widget_id );

		// Allow sidebar to be unset or missing when widget is not a _Widget.
		$parsed_id     = _parse_widget_id( $widget_id );
		$widget_object = $_widget_factory->get_widget_object( $parsed_id['id_base'] );
	 ...
	 *
	 * @global _Widget_Factory $_widget_factory
	 * @global array             $_registered_widget_updates The registered widget update functions.
	 *
	 * @param _REST_Request $request Full details about the request.
	 * @return _REST_Response|_Error Response object on success, or _Error object on failure.
	 */
	public function delete_item( $request ) {
		global $_widget_factory, $_registered_widget_updates;
		...
		if ( $request['force'] ) {
			...
			$callback = $_registered_widget_updates[ $id_base ]['callback'];
			$params   = $_registered_widget_updates[ $id_base ]['params'];
			...
			$widget_object = $_widget_factory->get_widget_object( $id_base );

			if ( $widget_object ) {
				/*
				 * _Widget sets `updated = true` after an update to prevent more than one widget
			...
			_assign_widget_to_sidebar( $widget_id, '' );
			...
			_assign_widget_to_sidebar( $widget_id, '_inactive_widgets' );

			$response = $this->prepare_item_for_response(
				array(
					'sidebar_id' => '_inactive_widgets',
					'widget_id'  => $widget_id,
				),
				$request
			);
		 ...
		 * @param _REST_Response|_Error $response   The response data, or _Error object on failure.
	 ...
	 * @return string|_Error The saved widget ID.
	 */
	protected function save_widget( $request, $sidebar_id ) {
		...
		if ( isset( $request['instance'] ) ) {
			if ( ! $widget_object ) {
				return new _Error(
					'rest_invalid_widget',
					__( 'Cannot set instance on a widget that does not extend _Widget.' ),
					array( 'status' => 400 )
				);
			}
			...
			if ( isset( $request['instance']['raw'] ) ) {
				...
				if ( ! hash_equals( _hash( $serialized_instance ), $request['instance']['hash'] ) ) {
					return new _Error(
						'rest_invalid_widget',
	 ...
	 *
	 * @global _Widget_Factory $_widget_factory
	 * @global array             $_registered_widgets The registered widgets.
	 *
	...
	public function prepare_item_for_response( $item, $request ) {
		global $_widget_factory, $_registered_widgets;
		...
		$widget    = $_registered_widgets[ $widget_id ];
		$parsed_id = _parse_widget_id( $widget_id );
		...
		if (
			...
			'_inactive_widgets' !== $sidebar_id
		) {
			$prepared['rendered'] = trim( _render_widget( $widget_id, $sidebar_id ) );
		}

		if ( rest_is_field_included( 'rendered_form', $fields ) ) {
			$rendered_form = _render_widget_control( $widget_id );
		...
		if ( rest_is_field_included( 'instance', $fields ) ) {
			...
			if ( $widget_object && isset( $parsed_id['number'] ) ) {
				...
				$prepared['instance']['hash']    = _hash( $serialized_instance );
		 ...
		 * @since 5.8.0
		 *
		 * @param _REST_Response|_Error $response The response object, or _Error object on failure.
	...
	protected function prepare_links( $prepared ) {
		...
		return array(
			...
			'about'                     => array(
				'href'       => rest_url( sprintf( '/v2/widget-types/%s', $id_base ) ),
				'embeddable' => true,
			),
			'https://api.w.org/sidebar' => array(
				'href' => rest_url( sprintf( '/v2/sidebars/%s/', $prepared['sidebar'] ) ),
			),
		);
	}
