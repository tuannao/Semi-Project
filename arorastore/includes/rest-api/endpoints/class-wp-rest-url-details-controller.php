<?php
/**
 * REST API: _REST_URL_Details_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class _REST_URL_Details_Controller extends _REST_Controller {
	...
	public function __construct() {
		$this->namespace = 'block-editor/v1';
				...
				array(
					...
					'args'                => array(
						'url' => array(
							...
							'validate_callback' => '_http_validate_url',
	 ...
	 * @return _REST_Response|_Error The parsed details as a response object. _Error if there are errors.
	 */
	public function parse_url_details( $request ) {
		$url = untrailingslashit( $request['url'] );

		if ( empty( $url ) ) {
			return new _Error( 'rest_invalid_url', __( 'Invalid URL' ), array( 'status' => 404 ) );
		}
		...
		if ( ! empty( $cached_response ) ) {
			...
			if ( is__error( $remote_url_response ) || empty( $remote_url_response ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return _Error|bool True if the request has permission, else _Error.
	 */
	 ...
	 * @return string|_Error The HTTP response from the remote URL on success.
	 *                         _Error if no response or no content.
	 */
	private function get_remote_url( $url ) {

		/*
		 * Provide a modified UA string to workaround web properties which block  "Pingbacks".
		 * Why? The UA string used for pingback requests contains `/` which is very similar
		 * to that used as the default UA string by the  HTTP API. Therefore requests from this
		 * REST endpoint are being unintentionally blocked as they are misidentified as pingback requests.
		 * By slightly modifying the UA string, but still retaining the "" identification (via "")
		 * we are able to work around this issue.
		 * Example UA string: `URLDetails/5.9-alpha-51389 (+http://localhost:8888)`.
		*/
		$modified_user_agent = 'URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		 ...
		 *
		 * Can be used to adjust response size limit and other _Http::request() args.
		 *
		 * @since 5.9.0
		 *
		...
		$response = _safe_remote_get( $url, $args );

		if ( _Http::OK !== _remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new _Error(
				'no_response',
				__( 'URL not found. Response returned a non-200 status code for this URL.' ),
				array( 'status' => _Http::NOT_FOUND )
			);
		}

		$remote_body = _remote_retrieve_body( $response );
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = _Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = _Http::make_absolute_url( $image, $root_url );
		}
	...
	private function prepare_metadata_for_output( $metadata ) {
		...
		$metadata = _strip_all_tags( $metadata );
		return $metadata;
	}
