<?php
/**
 * REST API: _REST_Global_Styles_Controller class
 *
 * @package    
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class _REST_Global_Styles_Controller extends _REST_Controller {
	 ...
	 * For example, `/v2/global-styles/twentytwentytwo%200.4.0`
	...
	public function update_item( $request ) {
		...
		if ( is__error( $post_before ) ) {
			return $post_before;
		}

		$changes = $this->prepare_item_for_database( $request );
		$result  = _update_post( _slash( (array) $changes ), true, false );
		if ( is__error( $result ) ) {
			return $result;
		}
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			$config['version']                     = _Theme_JSON::LATEST_SCHEMA;
			$changes->post_content                 = _json_encode( $config );
		}
	 ...
	 * @since 5.9.0
	 *
	 * @param _Post         $post    Global Styles post object.
	 * @param _REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( $is_global_styles_user_theme_json ) {
			$config = ( new _Theme_JSON( $raw_config, 'custom' ) )->get_raw_data();
		}
	...
	public function get_theme_item( $request ) {
		if ( _get_theme()->get_stylesheet() !== $request['stylesheet'] ) {
		...
		$theme  = _Theme_JSON_Resolver::get_merged_data( 'theme' );
		$data   = array();
		$fields = $this->get_fields_for_response( $request );
