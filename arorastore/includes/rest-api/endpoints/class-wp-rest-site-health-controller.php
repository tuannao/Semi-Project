<?php
/**
 * REST API: _REST_Site_Health_Controller class
 *
 * @package 
 * @subpackage REST_API
 * @since 5.6.0
 ...
 */
class _REST_Site_Health_Controller extends _REST_Controller {
	 ...
	 * @since 5.6.0
	 *
	 * @param _Site_Health $site_health An instance of the site health class.
	 */
	public function __construct( $site_health ) {
		$this->namespace = 'site-health/v1';
	...
	/**
	 * Checks that the site can reach the .org API.
	 *
	 * @since 5.6.0
	 *
	 * @return array
	...
	public function get_directory_sizes() {
		if ( ! class_exists( '_Debug_Data' ) ) {
			require_once ABSPATH . 'admin/includes/class-debug-data.php';
		}

		$this->load_admin_textdomain();

		$sizes_data = _Debug_Data::get_sizes();
		...
		if ( isset( $all_sizes['total_size']['debug'] ) && 'not available' === $all_sizes['total_size']['debug'] ) {
			return new _Error( 'not_available', __( 'Directory sizes could not be returned.' ), array( 'status' => 500 ) );
		}
	 ...
	 *
	 * The {@see _Site_Health} class is defined in Admin, while the REST API operates in a front-end context.
