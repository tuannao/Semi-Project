 ...
 * true.
 *
 * Note: _signon() doesn't handle setting the current user. This means that if the
 * function is called before the {@see 'init'} hook is fired, is_user_logged_in() will
 * evaluate as false until that point. If is_user_logged_in() is needed in conjunction
 * with _signon(), _set_current_user() should be called explicitly.
 *
 * @since 2.5.0
 *
 ...
 * @return _User|_Error _User on success, _Error on failure.
 */
function _signon( $credentials = array(), $secure_cookie = '' ) {
	if ( empty( $credentials ) ) {
		$credentials = array(); // Back-compat for plugins passing an empty string.

		if ( ! empty( $_POST['log'] ) ) {
			$credentials['user_login'] = _unslash( $_POST['log'] );
		}
		if ( ! empty( $_POST['pwd'] ) ) {
	 ...
	 * @since 1.5.1
	 *
	 * @todo Decide whether to deprecate the _authenticate action.
	 *
	 * @param string $user_login    Username (passed by reference).
	 * @param string $user_password User password (passed by reference).
	 */
	do_action_ref_array( '_authenticate', array( &$credentials['user_login'], &$credentials['user_password'] ) );
	...
	global $auth_secure_cookie; // XXX ugly hack to pass this to _authenticate_cookie().
	$auth_secure_cookie = $secure_cookie;

	add_filter( 'authenticate', '_authenticate_cookie', 30, 3 );

	$user = _authenticate( $credentials['user_login'], $credentials['user_password'] );

	if ( is__error( $user ) ) {
		return $user;
	}

	_set_auth_cookie( $user->ID, $credentials['remember'], $secure_cookie );
	/**
	 ...
	 * @param _User $user       _User object of the logged-in user.
	 */
	do_action( '_login', $user->user_login, $user );
	return $user;
}
 ...
 * @since 2.8.0
 *
 * @param _User|_Error|null $user     _User or _Error object from a previous callback. Default null.
 * @param string                $username Username for authentication.
 * @param string                $password Password for authentication.
 * @return _User|_Error _User on success, _Error on failure.
 */
function _authenticate_username_password( $user, $username, $password ) {
	if ( $user instanceof _User ) {
		return $user;
	}
	 ...
	 * @since 2.5.0
	 *
	 * @param _User|_Error $user     _User or _Error object if a previous
	 *                                   callback failed authentication.
	 * @param string           $password Password to check against the user.
	 */
	$user = apply_filters( '_authenticate_user', $user, $password );
	if ( is__error( $user ) ) {
		return $user;
	}

	if ( ! _check_password( $password, $user->user_pass, $user->ID ) ) {
		return new _Error(
			'incorrect_password',
			sprintf(
				/* translators: %s: User name. */
 ...
 * @since 4.5.0
 *
 * @param _User|_Error|null $user     _User or _Error object if a previous
 *                                        callback failed authentication.
 * @param string                $email    Email address for authentication.
 * @param string                $password Password for authentication.
 * @return _User|_Error _User on success, _Error on failure.
 */
function _authenticate_email_password( $user, $email, $password ) {
	if ( $user instanceof _User ) {
		return $user;
	}
	...
	if ( empty( $email ) || empty( $password ) ) {
		...
		if ( empty( $email ) ) {
			// Uses 'empty_username' for back-compat with _signon().
...
/**
 * Authenticates the user using the  auth cookie.
 *
 * @since 2.8.0
 *
 * @global string $auth_secure_cookie
 ...
 * @return _User|_Error _User on success, _Error on failure.
 */
function _authenticate_cookie( $user, $username, $password ) {
	if ( $user instanceof _User ) {
		return $user;
	}

	if ( empty( $username ) && empty( $password ) ) {
		$user_id = _validate_auth_cookie();
		if ( $user_id ) {
			return new _User( $user_id );
		}
		...
		if ( ! empty( $_COOKIE[ $auth_cookie ] ) ) {
			return new _Error( 'expired_session', __( 'Please log in again.' ) );
		}
 ...
 * @since 5.6.0
 *
 * @param _User|_Error|null $input_user _User or _Error object if a previous
 *                                          callback failed authentication.
 * @param string                $username   Username for authentication.
 * @param string                $password   Password for authentication.
 * @return _User|_Error|null _User on success, _Error on failure, null if
 *                               null is passed in and this isn't an API request.
 */
function _authenticate_application_password( $input_user, $username, $password ) {
	if ( $input_user instanceof _User ) {
		return $input_user;
	}

	if ( ! _Application_Passwords::is_in_use() ) {
		return $input_user;
	}
	...
	} elseif ( ! _is_application_passwords_available() ) {
		$error = new _Error(
			'application_passwords_disabled',
			__( 'Application passwords are not available.' )
		);
	} elseif ( ! _is_application_passwords_available_for_user( $user ) ) {
		$error = new _Error(
	...
	if ( $error ) {
		 ...
		 * @since 5.6.0
		 *
		 * @param _Error $error The authentication error.
		 */
	...
	$hashed_passwords = _Application_Passwords::get_user_application_passwords( $user->ID );

	foreach ( $hashed_passwords as $key => $item ) {
		if ( ! _check_password( $password, $item['password'], $user->ID ) ) {
			continue;
		}
		 ...
		 * @since 5.6.0
		 *
		 * @param _Error $error    The error object.
		 * @param _User  $user     The user authenticating.
		 * @param array    $item     The details about the application password.
		 * @param string   $password The raw supplied password.
		 */
		do_action( '_authenticate_application_password_errors', $error, $user, $item, $password );

		if ( is__error( $error ) && $error->has_errors() ) {
		...
		_Application_Passwords::record_application_password_usage( $user->ID, $item['uuid'] );
		 ...
		 * @since 5.6.0
		 *
		 * @param _User $user The user who was authenticated.
 ...
 */
function _validate_application_password( $input_user ) {
	...
	$authenticated = _authenticate_application_password( null, $_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'] );

	if ( $authenticated instanceof _User ) {
 ...
 *
 * @param _User|_Error|null $user _User or _Error object from a previous callback. Default null.
 * @return _User|_Error _User on success, _Error if the user is considered a spammer.
 */
function _authenticate_spam_check( $user ) {
	if ( $user instanceof _User && is_multisite() ) {
		/**
		 ...
		 * @param _User $user    User to check against.
		 */
		$spammed = apply_filters( 'check_is_user_spammed', is_user_spammy( $user ), $user );

		if ( $spammed ) {
			return new _Error( 'spammer_account', __( '<strong>Error</strong>: Your account has been marked as a spammer.' ) );
		}
	}
	return $user;
}
 ...
 */
function _validate_logged_in_cookie( $user_id ) {
	if ( $user_id ) {
		return $user_id;
	}
	...
	return _validate_auth_cookie( $_COOKIE[ LOGGED_IN_COOKIE ], 'logged_in' );
}
...
function count_user_posts( $userid, $post_type = 'post', $public_only = false ) {
	...
	$count = $db->get_var( "SELECT COUNT(*) FROM $db->posts $where" );
...
function count_many_users_posts( $users, $post_type = 'post', $public_only = false ) {
	...
	$result = $db->get_results( "SELECT post_author, COUNT(*) FROM $db->posts $where AND post_author IN ($userlist) GROUP BY post_author", ARRAY_N );
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	 ...
	 * @param _User $user   _User object of the user whose option is being retrieved.
	 */
...
function update_user_option( $user_id, $option_name, $newvalue, $global = false ) {
	...
	if ( ! $global ) {
		$option_name = $db->get_blog_prefix() . $option_name;
	}
 ...
 * @see _User_Query
 *
 * @param array $args Optional. Arguments to retrieve users. See _User_Query::prepare_query()
...
function get_users( $args = array() ) {
	...
	$user_search = new _User_Query( $args );
 ...
 */
function _list_users( $args = array() ) {
	$defaults = array(
		'orderby'       => 'name',
		'order'         => 'ASC',
		'number'        => '',
	...
	$query_args           = _array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
...
function get_blogs_of_user( $user_id, $all = false ) {
	...
	if ( isset( $keys[ $db->base_prefix . 'capabilities' ] ) && defined( 'MULTISITE' ) ) {
		$site_ids[] = 1;
		unset( $keys[ $db->base_prefix . 'capabilities' ] );
	}
	...
	foreach ( $keys as $key ) {
		...
		if ( $db->base_prefix && 0 !== strpos( $key, $db->base_prefix ) ) {
			continue;
		}
		$site_id = str_replace( array( $db->base_prefix, '_capabilities' ), '', $key );
...
function is_user_member_of_blog( $user_id = 0, $blog_id = 0 ) {
	...
	$base_capabilities_key = $db->base_prefix . 'capabilities';
	$site_capabilities_key = $db->base_prefix . $blog_id . '_capabilities';
 ...
 * @since 3.0.0
 *
 * @link https://developer..org/reference/functions/delete_user_meta/
 *
 * @param int    $user_id    User ID
 * @param string $meta_key   Metadata name.
 ...
 * @since 3.0.0
 *
 * @link https://developer..org/reference/functions/get_user_meta/
 *
 * @param int    $user_id User ID.
 ...
 * @since 3.0.0
 *
 * @link https://developer..org/reference/functions/update_user_meta/
 *
 * @param int    $user_id    User ID.
 ...
 * Assumes role names are unique phrases. Same assumption made by _User_Query::prepare_query()
 * Using $strategy = 'time' this is CPU-intensive and should handle around 10^7 users.
 * Using $strategy = 'memory' this is memory-intensive and should handle around 10^5 users, but see  Bug #12257.
 *
 * @since 3.0.0
...
function count_users( $strategy = 'time', $site_id = null ) {
	...
	$blog_prefix = $db->get_blog_prefix( $site_id );
	$result      = array();

	if ( 'time' === $strategy ) {
		if ( is_multisite() && get_current_blog_id() != $site_id ) {
			switch_to_blog( $site_id );
			$avail_roles = _roles()->get_names();
		...
		foreach ( $avail_roles as $this_role => $name ) {
			$select_count[] = $db->prepare( 'COUNT(NULLIF(`meta_value` LIKE %s, false))', '%' . $db->esc_like( '"' . $this_role . '"' ) . '%' );
		}
		...
		$row = $db->get_row(
			...
			FROM {$db->usermeta}
			INNER JOIN {$db->users} ON user_id = ID
 ...
 */
function _maybe_update_user_counts( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$is_small_network = ! _is_large_user_count( $network_id );
	/** This filter is documented in includes/ms-functions.php */
	if ( ! apply_filters( 'enable_live_network_counts', $is_small_network, 'users' ) ) {
		return false;
	}

	return _update_user_counts( $network_id );
}
 ...
 * @return bool Whether the update was successful.
 */
function _update_user_counts( $network_id = null ) {
	global $db;
	...
	$query = "SELECT COUNT(ID) as c FROM $db->users";
	if ( is_multisite() ) {
 ...
 * @since 6.0.0
 */
function _schedule_update_user_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! _next_scheduled( '_update_user_counts' ) && ! _installing() ) {
		_schedule_event( time(), 'twicedaily', '_update_user_counts' );
	}
}
 ...
 */
function _is_large_user_count( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	 ...
	 */
	return apply_filters( '_is_large_user_count', $count > 10000, $count, $network_id );
}
 ...
 *
 * Used by _set_current_user() for back compat. Might be deprecated in the future.
 *
 * @since 2.0.4
 *
 * @global string  $user_login    The user username for logging in
 * @global _User $userdata      User data.
 ...
 *     See _User_Query::prepare_query() for additional available arguments.
 *
...
function _dropdown_users( $args = '' ) {
	 ...
	 * @param array $parsed_args The arguments passed to _dropdown_users() combined with the defaults.
	 */
	$query_args = apply_filters( '_dropdown_users_args', $query_args, $parsed_args );
	...
	/**
	 * Filters the _dropdown_users() HTML output.
	 *
	 * @since 2.3.0
	 *
	 * @param string $output HTML output generated by _dropdown_users().
	 */
	$html = apply_filters( '_dropdown_users', $output );
 ...
 * @since 3.0.0
 *
 * @param object|_User $user User object or database row to be cached
...
function update_user_caches( $user ) {
	...
	_cache_add( $user->ID, $user, 'users' );
	_cache_add( $user->user_login, $user->ID, 'userlogins' );
	_cache_add( $user->user_email, $user->ID, 'useremail' );
	_cache_add( $user->user_nicename, $user->ID, 'userslugs' );
}
 ...
 *
 * @global _User $current_user The current user object which holds the user data.
 *
 * @param _User|int $user User object or ID to be cleaned from the cache
 */
function clean_user_cache( $user ) {
	...
	_cache_delete( $user->ID, 'users' );
	_cache_delete( $user->user_login, 'userlogins' );
	_cache_delete( $user->user_email, 'useremail' );
	_cache_delete( $user->user_nicename, 'userslugs' );
 ...
 *              methods for new installations. See _get_user_contact_methods().
 ...
 * @global db $db  database abstraction object.
 *
 * @param array|object|_User $userdata {
 *     An array, object, or _User object of user data arguments.
 *
 ...
 *                                        Default empty.
 * }
 * @return int|_Error The newly created user's ID or a _Error object if the user could not
 *                      be created.
 */
function _insert_user( $userdata ) {
	...
	} elseif ( $userdata instanceof _User ) {
	...
	if ( ! empty( $userdata['ID'] ) ) {
		...
		if ( ! $old_user_data ) {
			return new _Error( 'invalid_user_id', __( 'Invalid user ID.' ) );
		}

		// Hashed in _update_user(), plaintext if called directly.
		$user_pass = ! empty( $userdata['user_pass'] ) ? $userdata['user_pass'] : $old_user_data->user_pass;
	} else {
		$update = false;
		// Hash the password.
		$user_pass = _hash_password( $userdata['user_pass'] );
	}
	...
	if ( empty( $user_login ) ) {
		return new _Error( 'empty_user_login', __( 'Cannot create a user with an empty login name.' ) );
	} elseif ( mb_strlen( $user_login ) > 60 ) {
		return new _Error( 'user_login_too_long', __( 'Username may not be longer than 60 characters.' ) );
	}

	if ( ! $update && username_exists( $user_login ) ) {
		return new _Error( 'existing_user_login', __( 'Sorry, that username already exists!' ) );
	}
	...
	if ( in_array( strtolower( $user_login ), array_map( 'strtolower', $illegal_logins ), true ) ) {
		return new _Error( 'invalid_username', __( 'Sorry, that username is not allowed.' ) );
	}
	...
	if ( mb_strlen( $user_nicename ) > 50 ) {
		return new _Error( 'user_nicename_too_long', __( 'Nicename may not be longer than 50 characters.' ) );
	}

	$user_nicename_check = $db->get_var( $db->prepare( "SELECT ID FROM $db->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $user_nicename, $user_login ) );
	...
	if ( $user_nicename_check ) {
		...
		while ( $user_nicename_check ) {
			...
			$user_nicename_check = $db->get_var( $db->prepare( "SELECT ID FROM $db->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $alt_user_nicename, $user_login ) );
			$suffix++;
		}
	...
	if ( ( ! $update || ( ! empty( $old_user_data ) && 0 !== strcasecmp( $user_email, $old_user_data->user_email ) ) )
		...
		return new _Error( 'existing_user_email', __( 'Sorry, that email address is already used!' ) );
	}
	...
	if ( mb_strlen( $user_url ) > 100 ) {
		return new _Error( 'user_url_too_long', __( 'User URL may not be longer than 100 characters.' ) );
	}
	...
	if ( ! empty( $userdata['spam'] ) && ! is_multisite() ) {
		return new _Error( 'no_spam', __( 'Sorry, marking a user as spam is only supported on Multisite.' ) );
	}
	 ...
	 * @param array    $userdata The raw array of data passed to _insert_user().
	 */
	$data = apply_filters( '_pre_insert_user_data', $data, $update, ( $update ? $user_id : null ), $userdata );

	if ( empty( $data ) || ! is_array( $data ) ) {
		return new _Error( 'empty_data', __( 'Not enough data to create this user.' ) );
	}
	...
	if ( $update ) {
		...
		$db->update( $db->users, $data, array( 'ID' => $user_id ) );
	} else {
		$db->insert( $db->users, $data );
		$user_id = (int) $db->insert_id;
	}

	$user = new _User( $user_id );

	/**
	 * Filters a user's meta values and keys immediately after the user is created or updated
	 * and before any user meta is inserted or updated.
	 *
	 * Does not include contact methods. These are added using `_get_user_contact_methods( $user )`.
	 *
	...
	foreach ( _get_user_contact_methods( $user ) as $key => $value ) {
	...
	if ( $update ) {
		 ...
		 *
		 * @param int     $user_id       User ID.
		 * @param _User $old_user_data Object containing user's data prior to update.
		 * @param array   $userdata      The raw array of data passed to _insert_user().
		 */
 ...
 * @since 2.0.0
 *
 * @see _insert_user() For what fields can be set in $userdata.
 *
 * @param array|object|_User $userdata An array of user data or a user object of type stdClass or _User.
 * @return int|_Error The updated user's ID or a _Error object if the user could not be updated.
 */
function _update_user( $userdata ) {
	...
	if ( ! empty( $userdata['user_pass'] ) && $userdata['user_pass'] !== $user_obj->user_pass ) {
		...
		$userdata['user_pass'] = _hash_password( $userdata['user_pass'] );
		 ...
		 * @since 4.3.0
		 *
		 * @see _insert_user() For `$user` and `$userdata` fields.
		 *
	...
	if ( is__error( $user_id ) ) {
		return $user_id;
	}

	$blog_name = _specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		_mail( $pass_change_email['to'], sprintf( $pass_change_email['subject'], $blog_name ), $pass_change_email['message'], $pass_change_email['headers'] );
	}
...
###SITEURL###'
		...
		_mail( $email_change_email['to'], sprintf( $email_change_email['subject'], $blog_name ), $email_change_email['message'], $email_change_email['headers'] );
	}
	...
	if ( $current_user->ID == $user_id ) {
		if ( isset( $plaintext_pass ) ) {
			...
			$logged_in_cookie = _parse_auth_cookie( '', 'logged_in' );
			...
			_set_auth_cookie( $user_id, $remember );
		}
	}
 ...
 * complex user creation use _insert_user() to specify more information.
 *
 * @since 2.0.0
 *
 * @see _insert_user() More complete way to create a new user.
 *
 ...
 *                      be created.
 */
function _create_user( $username, $password, $email = '' ) {
	$user_login = _slash( $username );
	$user_email = _slash( $email );
	$user_pass  = $password;

	$userdata = compact( 'user_login', 'user_email', 'user_pass' );
	return _insert_user( $userdata );
}

/**
 * Returns a list of meta keys to be (maybe) populated in _update_user().
 *
 ...
 * @access private
 *
 * @param _User $user _User instance.
 * @return string[] List of user keys to be populated in _update_user().
 */
function _get_additional_user_keys( $user ) {
	...
	return array_merge( $keys, array_keys( _get_user_contact_methods( $user ) ) );
}
 ...
 * @since 3.7.0
 *
 * @param _User|null $user Optional. _User object.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function _get_user_contact_methods( $user = null ) {
	$methods = array();
	 ...
	 * @param _User|null $user    _User object or null if none was provided.
	 */
 ...
 *
 * Use _get_user_contact_methods() instead.
 *
 * @since 2.9.0
 * @access private
 *
 * @param _User|null $user Optional. _User object. Default null.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function __get_user_contactmethods( $user = null ) {
	return _get_user_contact_methods( $user );
}
 ...
 * @since 4.4.0
 *
 * @global PasswordHash $_hasher Portable PHP password hashing framework.
 *
 * @param _User $user User to retrieve password reset key for.
 * @return string|_Error Password reset key on success. _Error on error.
 */
function get_password_reset_key( $user ) {
	global $_hasher;

	if ( ! ( $user instanceof _User ) ) {
		return new _Error( 'invalidcombo', __( '<strong>Error</strong>: There is no account with that username or email address.' ) );
	}
	...
	if ( ! $allow ) {
		return new _Error( 'no_password_reset', __( 'Password reset is not allowed for this user' ) );
	} elseif ( is__error( $allow ) ) {
		return $allow;
	}

	// Generate something random for a password reset key.
	$key = _generate_password( 20, false );
	...
	$hashed = time() . ':' . $_hasher->HashPassword( $key );

	$key_saved = _update_user(
		array(
			'ID'                  => $user->ID,
 ...
 * but have a different _Error code so good user feedback can be provided.
 *
 * @since 3.1.0
 *
 * @global db         $db       database object for queries.
 * @global PasswordHash $_hasher Portable PHP password hashing framework instance.
 *
 * @param string $key       Hash to validate sending user's password.
 * @param string $login     The user login.
 * @return _User|_Error _User object on success, _Error object for invalid or expired keys.
 */
function check_password_reset_key( $key, $login ) {
	...
	if ( empty( $key ) || ! is_string( $key ) ) {
		return new _Error( 'invalid_key', __( 'Invalid key.' ) );
	}
	...
	$hash_is_correct = $_hasher->CheckPassword( $key, $pass_key );

	if ( $hash_is_correct && $expiration_time && time() < $expiration_time ) {
		return $user;
	} elseif ( $hash_is_correct && $expiration_time ) {
		// Key has an expiration time that's passed.
		return new _Error( 'expired_key', __( 'Invalid key.' ) );
	}
	...
	if ( hash_equals( $user->user_activation_key, $key ) || ( $hash_is_correct && ! $expiration_time ) ) {
		 ...
		 *
		 * @param _Error $return  A _Error object denoting an expired key.
		 *                          Return a _User object to validate the key.
 ...
 * @return true|_Error True when finished, _Error object on error.
 */
function retrieve_password( $user_login = null ) {
	...
	if ( empty( $user_login ) ) {
		...
		$user_data = get_user_by( 'email', trim( _unslash( $user_login ) ) );
		if ( empty( $user_data ) ) {
			$errors->add( 'invalid_email', __( '<strong>Error</strong>: There is no account with that username or email address.' ) );
		}
	} else {
		$user_data = get_user_by( 'login', trim( _unslash( $user_login ) ) );
	}
	 ...
	 * @since 5.7.0
	 *
	 * @param _User|false $user_data _User object if found, false if the user does not exist.
	 * @param _Error      $errors    A _Error object containing any errors generated
	 ...
	 *
	 * The filtered _Error object may, for example, contain errors for an invalid
	 * username or email address. A _Error object should always be returned,
	 ...
	 * @param bool    $send       Whether to send the email.
	 * @param string  $user_login The username for the user.
	 * @param _User $user_data  _User object.
	 */
	...
	if ( is_multisite() ) {
		 ...
		 */
		$site_name = _specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
	}
	...
	$message .= network_site_url( "login.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . '&_lang=' . $locale . "\r\n\r\n";
	 ...
	 *     The default notification email arguments. Used to build _mail().
	 *
	 ...
	 * @type _User $user_data  _User object.
	 */
	...
	$subject = _specialchars_decode( $subject );
	...
	if ( ! _mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			...
			sprintf(
				...
				esc_url( __( 'https://.org/support/article/resetting-your-password/' ) )
			)
		);
		return $errors;
	}
...
function reset_password( $user, $new_pass ) {
	...
	_set_password( $new_pass, $user->ID );
 ...
 * @return int|_Error Either user's ID or error on failure.
 */
function register_new_user( $user_login, $user_email ) {
	 ...
	 * @param _Error $errors               Contains any errors with submitted username and email,
	 ...
	 *
	 * The filtered _Error object may, for example, contain errors for an invalid
	 * or existing username or email address. A _Error object should always be returned,
	 * but may or may not contain errors.
	 *
	 ...
	 * @since 2.1.0
	 *
	 * @param _Error $errors               A _Error object containing any errors encountered
	 *                                       during registration.
	...
	$user_pass = _generate_password( 12, false );
	$user_id   = _create_user( $sanitized_user_login, $user_pass, $user_email );
	if ( ! $user_id || is__error( $user_id ) ) {
		$errors->add(
			'registerfail',
			sprintf(
	...
	if ( ! empty( $_COOKIE['_lang'] ) ) {
		$_lang = sanitize_text_field( $_COOKIE['_lang'] );
		if ( in_array( $_lang, get_available_languages(), true ) ) {
			update_user_meta( $user_id, 'locale', $_lang ); // Set user locale if defined on registration.
		}
	}
 ...
 *                        Default 'both'.
 */
function _send_new_user_notifications( $user_id, $notify = 'both' ) {
	_new_user_notification( $user_id, null, $notify );
}
...
function _get_session_token() {
	$cookie = _parse_auth_cookie( '', 'logged_in' );
...
function _get_all_sessions() {
	$manager = _Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function _destroy_current_session() {
 ...
 * @since 4.0.0
 */
function _destroy_other_sessions() {
	$token = _get_session_token();
	if ( $token ) {
		$manager = _Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function _destroy_all_sessions() {
	$manager = _Session_Tokens::get_instance( get_current_user_id() );
	$manager->destroy_all();
}
 ...
 */
function _get_users_with_no_role( $site_id = null ) {
	global $db;
	...
	$prefix = $db->get_blog_prefix( $site_id );

	if ( is_multisite() && get_current_blog_id() != $site_id ) {
		switch_to_blog( $site_id );
		$role_names = _roles()->get_names();
		restore_current_blog();
	} else {
		$role_names = _roles()->get_names();
	}
 ...
 *
 * This function is used by the pluggable functions _get_current_user() and
 ...
 * @access private
 *
 * @see _get_current_user()
 * @global _User $current_user Checks if the current user is set.
 *
 * @return _User Current _User instance.
 */
function __get_current_user() {
	global $current_user;

	if ( ! empty( $current_user ) ) {
		if ( $current_user instanceof _User ) {
			return $current_user;
		}

		// Upgrade stdClass to _User.
		if ( is_object( $current_user ) && isset( $current_user->ID ) ) {
			$cur_id       = $current_user->ID;
			$current_user = null;
			_set_current_user( $cur_id );
			return $current_user;
		}

		// $current_user has a junk value. Force to _User with ID 0.
		$current_user = null;
		_set_current_user( 0 );
		return $current_user;
	}
 ...
 *
 * @global _Error $errors _Error object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		...
		$hash           = md5( $_POST['email'] . time() . _rand() );
		...
		$sitename = _specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		_mail( $_POST['email'], sprintf( __( '[%s] Email Change Request' ), $sitename ), $content );
 ...
 */
function __privacy_action_request_types() {
	return array(
 ...
 */
function _register_user_personal_data_exporter( $exporters ) {
	$exporters['-user'] = array(
		'exporter_friendly_name' => __( ' User' ),
		'callback'               => '_user_personal_data_exporter',
	);
 ...
 */
function _user_personal_data_exporter( $email_address ) {
	 ...
	 * }
	 * @param _User  $user           The user whose data is being exported.
	 ...
	 */
	$_extra_data = apply_filters( '_privacy_additional_user_profile_data', array(), $user, $reserved_names );
	...
	if ( is_array( $_extra_data ) && ! empty( $_extra_data ) ) {
		...
		if ( count( $extra_data ) !== count( $_extra_data ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				sprintf(
					/* translators: %s: _privacy_additional_user_profile_data */
					__( 'Filter %s returned items with reserved names.' ),
					'<code>_privacy_additional_user_profile_data</code>'
				),
				'5.4.0'
			);
		}
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the  Events and News dashboard widget.' ),
 ...
 */
function __privacy_account_request_confirmed( $request_id ) {
	$request = _get_user_request( $request_id );
	...
	update_post_meta( $request_id, '__user_request_confirmed_timestamp', time() );
	_update_post(
		array(
			'ID'          => $request_id,
 ...
 */
function __privacy_send_request_confirmation_notification( $request_id ) {
	$request = _get_user_request( $request_id );

	if ( ! is_a( $request, '_User_Request' ) || 'request-confirmed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '__admin_notified', true );
	...
	$action_description = _user_request_action_description( $request->action_name );
	 ...
	 * @param _User_Request $request     The request that is initiating the notification.
	 */
	 ...
	 *
	 *     @type _User_Request $request     User request object.
...
###SITEURL###'
	...
	$email_sent = _mail( $email_data['admin_email'], $subject, $content, $headers );

	if ( $email_sent ) {
		update_post_meta( $request_id, '__admin_notified', true );
	}
}
 ...
 */
function __privacy_send_erasure_fulfillment_notification( $request_id ) {
	$request = _get_user_request( $request_id );

	if ( ! is_a( $request, '_User_Request' ) || 'request-completed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '__user_notified', true );
...
###SITEURL###'
	...
	$email_sent = _mail( $user_email, $subject, $content, $headers );
	...
	if ( $email_sent ) {
		update_post_meta( $request_id, '__user_notified', true );
	}
}
 ...
 */
function __privacy_account_request_confirmed_message( $request_id ) {
	$request = _get_user_request( $request_id );
	...
	if ( $request && in_array( $request->action_name, __privacy_action_request_types(), true ) ) {
		if ( 'export_personal_data' === $request->action_name ) {
 ...
 * @return int|_Error                   Returns the request ID if successful, or a _Error object on failure.
 */
function _create_user_request( $email_address = '', $action_name = '', $request_data = array(), $status = 'pending' ) {
	$email_address = sanitize_email( $email_address );
	$action_name   = sanitize_key( $action_name );

	if ( ! is_email( $email_address ) ) {
		return new _Error( 'invalid_email', __( 'Invalid email address.' ) );
	}

	if ( ! in_array( $action_name, __privacy_action_request_types(), true ) ) {
		return new _Error( 'invalid_action', __( 'Invalid action name.' ) );
	}

	if ( ! in_array( $status, array( 'pending', 'confirmed' ), true ) ) {
		return new _Error( 'invalid_status', __( 'Invalid request status.' ) );
	}

	$user    = get_user_by( 'email', $email_address );
	$user_id = $user && ! is__error( $user ) ? $user->ID : 0;

	// Check for duplicates.
	$requests_query = new _Query(
		array(
	...
	if ( $requests_query->found_posts ) {
		return new _Error( 'duplicate_request', __( 'An incomplete personal data request for this email address already exists.' ) );
	}

	$request_id = _insert_post(
		array(
			'post_author'   => $user_id,
			'post_name'     => $action_name,
			'post_title'    => $email_address,
			'post_content'  => _json_encode( $request_data ),
 ...
 */
function _user_request_action_description( $action_name ) {
	switch ( $action_name ) {
 ...
 * @since 4.9.6
 *
 * @param string $request_id ID of the request created via _create_user_request().
 * @return true|_Error True on success, `_Error` on failure.
 */
function _send_user_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = _get_user_request( $request_id );

	if ( ! $request ) {
		return new _Error( 'invalid_request', __( 'Invalid personal data request.' ) );
	}
	...
	$email_data = array(
		'request'     => $request,
		'email'       => $request->email,
		'description' => _user_request_action_description( $request->action_name ),
		'confirm_url' => add_query_arg(
			array(
				'action'      => 'confirmaction',
				'request_id'  => $request_id,
				'confirm_key' => _generate_user_request_key( $request_id ),
			),
			_login_url()
		),
		'sitename'    => _specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ),
...
###SITEURL###'
	...
	$email_sent = _mail( $email_data['email'], $subject, $content, $headers );
	...
	if ( ! $email_sent ) {
		return new _Error( 'privacy_email_error', __( 'Unable to send personal data export confirmation email.' ) );
	}
 ...
 */
function _generate_user_request_key( $request_id ) {
	global $_hasher;
	...
	_update_post(
		array(
			...
			'post_password' => $_hasher->HashPassword( $key ),
		)
	);
 ...
 * @return true|_Error True on success, _Error on failure.
 */
function _validate_user_request_key( $request_id, $key ) {
	global $_hasher;
	...
	if ( ! in_array( $request->status, array( 'request-pending', 'request-failed' ), true ) ) {
		return new _Error( 'expired_request', __( 'This personal data request has expired.' ) );
	}

	if ( empty( $key ) ) {
		return new _Error( 'missing_key', __( 'The confirmation key is missing from this personal data request.' ) );
	}
	...
	if ( ! $_hasher->CheckPassword( $key, $saved_key ) ) {
		return new _Error( 'invalid_key', __( 'The confirmation key is invalid for this personal data request.' ) );
	}

	if ( ! $expiration_time || time() > $expiration_time ) {
		return new _Error( 'expired_key', __( 'The confirmation key has expired for this personal data request.' ) );
	}
 ...
 * @return _User_Request|false
 */
function _get_user_request( $request_id ) {
	$request_id = absint( $request_id );
 ...
 * but may be made available using the {@see '_is_application_passwords_available'} filter.
 *
 * @since 5.9.0
 *
 * @return bool
 */
function _is_application_passwords_supported() {
	return is_ssl() || 'local' === _get_environment_type();
}
 ...
 * Use the {@see '_is_application_passwords_available'} filter to adjust its availability.
 *
 * @since 5.6.0
 *
 * @return bool
 */
function _is_application_passwords_available() {
	/**
	 ...
	 */
	return apply_filters( '_is_application_passwords_available', _is_application_passwords_supported() );
}

/**
 * Checks if Application Passwords is available for a specific user.
 *
 * By default all users can use Application Passwords. Use {@see '_is_application_passwords_available_for_user'}
 * to restrict availability to certain users.
 *
 * @since 5.6.0
 *
 * @param int|_User $user The user to check.
 * @return bool
 */
function _is_application_passwords_available_for_user( $user ) {
	if ( ! _is_application_passwords_available() ) {
		return false;
	}
	 ...
	 *
	 * @param bool    $available True if available, false otherwise.
	 * @param _User $user      The user to check.
	 */
	return apply_filters( '_is_application_passwords_available_for_user', true, $user );
}
