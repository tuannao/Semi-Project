...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $_rewrite;
	$_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see _Rewrite::remove_permastruct()
 * @global _Rewrite $_rewrite  rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $function ) {
	...
	if ( ! in_array( $feedname, $_rewrite->feeds, true ) ) {
		$_rewrite->feeds[] = $feedname;
	}
...
function flush_rewrite_rules( $hard = true ) {
	...
	if ( is_callable( array( $_rewrite, 'flush_rules' ) ) ) {
		$_rewrite->flush_rules( $hard );
	}
}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $_rewrite;
	$_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function __filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to _Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, _unique_post_slug() prevents the creation of post slugs
 ...
 *                          ::parse_request(). Default empty array.
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof _Post ) {
			return (int) $page_on_front;
		}
	}

	// Check to see if we are using rewrite rules.
	$rewrite = $_rewrite->_rewrite_rules();
	...
	if ( ! $_rewrite->using_index_permalinks() ) {
		$url = str_replace( $_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			if ( $_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			$query = addslashes( _MatchesMapRegex::apply( $query, $matches ) );
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $>public_query_vars, true ) ) {
					$query[ $key ] = $value;
