 ...
 * @see get_site()
 *
 * @return _Site Current site object.
 */
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use _generate_password()
 * @see _generate_password()
 *
 * @param int $len Optional. The length of password to generate. Default 8.
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', '_generate_password()' );
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use _die()
 * @see _die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', '_die()' );
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use _get_sites()
 * @see _get_sites()
 *
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	_deprecated_function( __FUNCTION__, '3.0.0', '_get_sites()' );

	global $db;
	$blogs = $db->get_results( $db->prepare( "SELECT blog_id, domain, path FROM $db->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", get_current_network_id() ), ARRAY_A );

	$blog_list = array();
	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $db->get_var( "SELECT COUNT(ID) FROM " . $db->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.3.0 Use _redirect()
 * @see _redirect()
 *
 * @param string $url Optional. Redirect URL. Default empty.
 */
function mu_admin_do_redirect( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0', '_redirect()' );

	$ref = '';
	if ( isset( $_GET['ref'] ) && isset( $_POST['ref'] ) && $_GET['ref'] !== $_POST['ref'] ) {
		_die( __( 'A variable mismatch has been detected.' ), __( 'Sorry, you are not allowed to view this item.' ), 400 );
	...
	if ( $ref ) {
		$ref = mu_admin_redirect_add_updated_param( $ref );
		_redirect( $ref );
		exit;
	}
	...
	$url = mu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) && isset( $_POST['redirect'] ) && $_GET['redirect'] !== $_POST['redirect'] ) {
		...
		$url = mu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	_redirect( $url );
	exit;
}
 ...
 * @return string
 */
function mu_admin_redirect_add_updated_param( $url = '' ) {
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	/*
	 * Need to back up db table names, and create a new _blogs entry for new blog.
	 * Need to get blog_id from _blogs, and create new table names.
...
function get_admin_users_for_domain( $domain = '', $path = '' ) {
	...
	if ( $network_id )
		return $db->get_results( $db->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $db->users AS u, $db->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $network_id ), ARRAY_A );
 ...
 * }
 * @return array[] An empty array if the installation is considered "large" via _is_large_network(). Otherwise,
 *                 an associative array of _Site data as arrays.
 */
function _get_sites( $args = array() ) {
	_deprecated_function( __FUNCTION__, '4.6.0', 'get_sites()' );

	if ( _is_large_network() )
		return array();
...
function is_user_option_local( $key, $user_id = 0, $blog_id = 0 ) {
	...
	$local_key = $db->get_blog_prefix( $blog_id ) . $key;
 ...
 *
 * This function creates a row in the _blogs table and returns
 * the new blog's ID. It is the first step in creating a new blog.
 *
 * @since MU (3.0.0)
 * @deprecated 5.1.0 Use _insert_site()
 * @see _insert_site()
 *
...
function insert_blog($domain, $path, $site_id) {
	_deprecated_function( __FUNCTION__, '5.1.0', '_insert_site()' );
	...
	$site_id = _insert_site( $data );
	if ( is__error( $site_id ) ) {
		return false;
	}
 ...
 * directly, be sure to use switch_to_blog() first, so that $db
 ...
 *
 * @param int    $blog_id    The value returned by _insert_site().
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $db->get_results( "DESCRIBE {$db->posts}" ) ) {
	...
	update_option( 'blogname', _unslash( $blog_title ) );
 ...
 * Set blog defaults.
 *
 * This function creates a row in the _blogs table.
 *
 * @since MU (3.0.0)
 * @deprecated MU
 * @deprecated Use _install_defaults()
 *
 * @global db $db  database abstraction object.
 *
 ...
 *
 * @since 3.0.0
 * @deprecated 5.3.0 Use _update_user()
 * @see _update_user()
 *
 * @global db $db  database abstraction object.
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the _users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	_deprecated_function( __FUNCTION__, '5.3.0', '_update_user()' );

	if ( null !== $deprecated ) {
		_deprecated_argument( __FUNCTION__, '3.0.2' );
	}

	$db->update( $db->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
