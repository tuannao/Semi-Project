 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', INC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', '/' . $GLOBALS['_version']);
...
/**
 * Retrieve URL headers and content using  HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = _safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is__error($resp) ) {
	...
	// Also note, _HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( _remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = _remote_retrieve_response_code( $resp );
	$response->response_code = _remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = _remote_retrieve_body( $resp );
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = _CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 * to display. You can't display all of them like you can with _rss()
 * function.
 *
 * @since 1.5.0
 * @package External
