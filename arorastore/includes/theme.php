...
/**
 * Returns an array of _Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to _get_theme() if possible.
 *
 * @since 3.4.0
 *
 * @global array $_theme_directories
 *
 * @param array $args {
 ...
 * }
 * @return _Theme[] Array of _Theme objects.
 */
function _get_themes( $args = array() ) {
	global $_theme_directories;
	...
	if ( is_array( $_theme_directories ) && count( $_theme_directories ) > 1 ) {
		// Make sure the active theme wins out, in case search_theme_directories() picks the wrong
		// one in the case of a conflict. (Normally, last registered theme root wins.)
		$current_theme = get_stylesheet();
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			$root_of_current_theme = get_raw_theme_root( $current_theme );
			if ( ! in_array( $root_of_current_theme, $_theme_directories, true ) ) {
				$root_of_current_theme = _CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, _Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, _Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, _Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, _Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = new _Theme( $theme, $theme_root['theme_root'] );
	...
	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $_theme ) {
			if ( $_theme->errors() != $args['errors'] ) {
				unset( $themes[ $theme ] );
			}
		}
	}
...
/**
 * Gets a _Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global array $_theme_directories
 ...
 * @return _Theme Theme object. Be sure to check the object's exists() method
 *                  if you need to confirm the theme's existence.
 */
function _get_theme( $stylesheet = '', $theme_root = '' ) {
	global $_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = _CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $_theme_directories, true ) ) {
			$theme_root = _CONTENT_DIR . $theme_root;
		}
	}

	return new _Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and _Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the theme updates cache.
 */
function _clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_themes' );
	}
	search_theme_directories( true );
	foreach ( _get_themes( array( 'errors' => null ) ) as $theme ) {
		$theme->cache_delete();
	}
}
...
function get_locale_stylesheet_uri() {
	...
	} elseif ( ! empty( $_locale->text_direction ) && file_exists( "$dir/{$_locale->text_direction}.css" ) ) {
		$stylesheet_uri = "$stylesheet_dir_uri/{$_locale->text_direction}.css";
	} else {
		$stylesheet_uri = '';
	}
	/**
 ...
 *                          or a folder within _CONTENT_DIR.
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = _CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $_theme_directories ) ) {
		$_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $_theme_directories, true ) ) {
		$_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$_theme_directories = (array) $_theme_directories;
	 ...
	 */
	foreach ( $_theme_directories as $theme_root ) {
		if ( 0 === strpos( $theme_root, _CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( _CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	 ...
	 */
	$cache_expiration = apply_filters( '_cache_themes_persistently', false, 'search_theme_directories' );
	...
	foreach ( $_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				...
				// Return it; _Theme will catch the error.
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
						'theme_root' => $theme_root,
...
function get_theme_root( $stylesheet_or_template = '' ) {
	...
	if ( $stylesheet_or_template ) {
		...
		if ( $theme_root ) {
			// Always prepend _CONTENT_DIR unless the root currently registered as a theme directory.
			// This gives relative theme roots the benefit of the doubt when things go haywire.
			if ( ! in_array( $theme_root, (array) $_theme_directories, true ) ) {
				$theme_root = _CONTENT_DIR . $theme_root;
			}
		}
	}
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $_theme_directories, true ) ) {
			...
			if ( 0 === strpos( $theme_root, _CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( _CONTENT_DIR, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, _PLUGIN_DIR ) || 0 === strpos( $theme_root, MU_PLUGIN_DIR ) ) {
 ...
 *
 * @global array                $_theme_directories
 * @global _Customize_Manager $_customize
 * @global array                $sidebars_widgets
 *
 * @param string $stylesheet Stylesheet name.
 */
function switch_theme( $stylesheet ) {
	global $_theme_directories, $_customize, $sidebars_widgets;

	$requirements = validate_theme_requirements( $stylesheet );
	if ( is__error( $requirements ) ) {
		_die( $requirements );
	}

	$_sidebars_widgets = null;
	if ( '_ajax_customize_save' === current_action() ) {
		$old_sidebars_widgets_data_setting = $_customize->get_setting( 'old_sidebars_widgets_data' );
		if ( $old_sidebars_widgets_data_setting ) {
			$_sidebars_widgets = $_customize->post_value( $old_sidebars_widgets_data_setting );
		}
	...
	$new_theme = _get_theme( $stylesheet );
	 ...
	 * @param _Theme $new_theme _Theme instance of the new theme.
	 * @param _Theme $old_theme _Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( _installing() || ! apply_filters( 'validate_current_theme', true ) ) {
		return true;
	}
	...
	$default = _get_theme( _DEFAULT_THEME );
	if ( $default->exists() ) {
		switch_theme( _DEFAULT_THEME );
		return false;
	}

	/**
	 * If we're in an invalid state but _DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 *
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against _DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = _Theme::get_core_default_theme();
...
/**
 * Validates the theme requirements for  version and PHP version.
 *
 ...
 * @return true|_Error True if requirements are met, _Error on failure.
 */
function validate_theme_requirements( $stylesheet ) {
	$theme = _get_theme( $stylesheet );

	$requirements = array(
		'requires'     => ! empty( $theme->get( 'Requires' ) ) ? $theme->get( 'Requires' ) : '',
		'requires_php' => ! empty( $theme->get( 'RequiresPHP' ) ) ? $theme->get( 'RequiresPHP' ) : '',
	);

	$compatible_  = is__version_compatible( $requirements['requires'] );
	$compatible_php = is_php_version_compatible( $requirements['requires_php'] );

	if ( ! $compatible_ && ! $compatible_php ) {
		return new _Error(
			'theme__php_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current  and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new _Error(
			...
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current  version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name ) {
			$theme_name = _get_theme()->get( 'Name' );
		}
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( ! empty( $header->attachment_id ) ) {
		$image_alt = get_post_meta( $header->attachment_id, '__attachment_image_alt', true );
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '__attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = _calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );

			if ( ! empty( $attr['sizes'] ) ) {
				$sizes = $attr['sizes'];
			} else {
				$sizes = _calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
			}
 ...
 * @access private
 *
 * @global array $__default_headers
 *
 * @return object
 */
function _get_random_header_data() {
	global $__default_headers;
	static $__random_header = null;

	if ( empty( $__random_header ) ) {
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers          = array();

		if ( 'random-uploaded-image' === $header_image_mod ) {
			$headers = get_uploaded_header_images();
		} elseif ( ! empty( $__default_headers ) ) {
		...
		$__random_header = (object) $headers[ array_rand( $headers ) ];

		$__random_header->url = sprintf(
			$__random_header->url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);

		$__random_header->thumbnail_url = sprintf(
			$__random_header->thumbnail_url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);
	}

	return $__random_header;
}
...
function get_uploaded_header_images() {
	...
	$headers = get_posts(
		array(
			...
			'meta_key'   => '__attachment_is_custom_header',
	...
	foreach ( (array) $headers as $header ) {
		$url          = esc_url_raw( _get_attachment_url( $header->ID ) );
		$header_data  = _get_attachment_metadata( $header->ID );
		...
		$header_images[ $header_index ]['alt_text']      = get_post_meta( $header->ID, '__attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $__default_headers ) ) {
				foreach ( (array) $__default_headers as $default_header ) {
	...
	return (object) _parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$__default_headers = array_merge( (array) $__default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $__default_headers[ $header ] ) ) {
		unset( $__default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function get_header_video_settings() {
	...
	$video_type = _check_filetype( $video_url, _get_mime_types() );
...
function get_custom_header_markup() {
	...
	return sprintf(
		'<div id="custom-header" class="custom-header">%s</div>',
...
function the_custom_header_markup() {
	...
	if ( is_header_video_active() && ( has_header_video() || is_customize_preview() ) ) {
		_enqueue_script( 'custom-header' );
		_localize_script( 'custom-header', '_CustomHeaderSettings', get_header_video_settings() );
	}
}
 ...
 * @since 4.7.0
 */
function _custom_css_cb() {
	$styles = _get_custom_css();
	if ( $styles || is_customize_preview() ) :
		$type_attr = current_theme_supports( 'html5', 'style' ) ? '' : ' type="text/css"';
		?>
		<style<?php echo $type_attr; ?> id="custom-css">
			<?php
 ...
 * @return _Post|null The custom_css post or null if none exists.
 */
function _get_custom_css_post( $stylesheet = '' ) {
	if ( empty( $stylesheet ) ) {
		$stylesheet = get_stylesheet();
	}
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new _Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
			 * Cache the lookup. See _update_custom_css_post().
 ...
 */
function _get_custom_css( $stylesheet = '' ) {
	$css = '';
	...
	$post = _get_custom_css_post( $stylesheet );
	if ( $post ) {
	 ...
	 */
	$css = apply_filters( '_get_custom_css', $css, $stylesheet );
 ...
 * }
 * @return _Post|_Error Post on success, error on failure.
 */
function _update_custom_css_post( $css, $args = array() ) {
	$args = _parse_args(
		$args,
		array(
			'preprocessed' => '',
	 ...
	 *     $post = _get_custom_css_post( $setting->stylesheet );
	 ...
	 * }
	 * @param array $args {
	 *     The args passed into `_update_custom_css_post()` merged with defaults.
	 *
	...
	$post = _get_custom_css_post( $args['stylesheet'] );
	if ( $post ) {
		$post_data['ID'] = $post->ID;
		$r               = _update_post( _slash( $post_data ), true );
	} else {
		$r = _insert_post( _slash( $post_data ), true );
		...
		if ( ! is__error( $r ) ) {
			...
			if ( 0 === count( _get_post_revisions( $r ) ) ) {
				_save_post_revision( $r );
			}
		}
	}
			...
			case 'posts':
				foreach ( $config[ $type ] as $id => $item ) {
					if ( is_array( $item ) ) {
						...
						$content[ $type ][ $id ] = _array_slice_assoc(
							$item,
							array(
								'post_type',
								'post_title',
 ...
 *              `editor-styles`, and `block-styles` features were added.
...
function add_theme_support( $feature, ...$args ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( isset( $args[0] ) && is_array( $args[0] ) && isset( $__theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $__theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5':
			...
			if ( isset( $__theme_features['html5'] ) ) {
				$args[0] = array_merge( $__theme_features['html5'][0], $args[0] );
			}
			break;
		...
		case 'custom-logo':
			...
			$args[0]  = _parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header':
			...
			if ( isset( $__theme_features['custom-header'] ) ) {
				$args[0] = _parse_args( $__theme_features['custom-header'][0], $args[0] );
			}

			// Load in the defaults at the end, as we need to insure first one wins.
			// This will cause all constants to be defined, as each arg will then be set to the default.
			if ( $jit ) {
				$args[0] = _parse_args( $args[0], $defaults );
			}
			 ...
			 * Once we get to _loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background':
			...
			$defaults = array(
				...
				'default-color'          => '',
				'head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
			...
			if ( isset( $__theme_features['custom-background'] ) ) {
				$args[0] = _parse_args( $__theme_features['custom-background'][0], $args[0] );
			}
		...
		case 'title-tag':
			// Can be called in functions.php but must happen before _loaded, i.e. not in header.php.
			if ( did_action( '_loaded' ) ) {
				_doing_it_wrong(
					"add_theme_support( 'title-tag' )",
					sprintf(
						/* translators: 1: title-tag, 2: _loaded */
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['head-callback'] ) {
			add_action( '_head', $args[0]['head-callback'] );
		}
...
function get_theme_support( $feature, ...$args ) {
	...
	if ( ! isset( $__theme_features[ $feature ] ) ) {
		return false;
	}

	if ( ! $args ) {
		return $__theme_features[ $feature ];
	}
	...
	switch ( $feature ) {
		...
		case 'custom-header':
		case 'custom-background':
			if ( isset( $__theme_features[ $feature ][0][ $args[0] ] ) ) {
				return $__theme_features[ $feature ][0][ $args[0] ];
			}
			return false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header':
			...
			if ( isset( $support[0]['head-callback'] ) ) {
				remove_action( '_head', $support[0]['head-callback'] );
			}
	...
	unset( $__theme_features[ $feature ] );
...
function current_theme_supports( $feature, ...$args ) {
	...
	if ( ! $args ) {
		/** This filter is documented in includes/theme.php */
		return apply_filters( "current_theme_supports-{$feature}", true, $args, $__theme_features[ $feature ] ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
	}
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 */
			if ( true === $__theme_features[ $feature ] ) {  // Registered for all types.
				return true;
			}
			$content_type = $args[0];
			return in_array( $content_type, $__theme_features[ $feature ][0], true );
		...
		case 'post-formats':
			 ...
			 */
			$type = $args[0];
			return in_array( $type, $__theme_features[ $feature ][0], true );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $__theme_features[ $feature ][0][ $args[0] ] ) && $__theme_features[ $feature ][0][ $args[0] ] );
	}
 ...
 * @see add_theme_support()
 *
 * @global array $__registered_theme_features
 *
 ...
 *      }
 * }
 * @return true|_Error True if the theme feature was successfully registered, a _Error object if not.
 */
function register_theme_feature( $feature, $args = array() ) {
	global $__registered_theme_features;

	if ( ! is_array( $__registered_theme_features ) ) {
		$__registered_theme_features = array();
	}
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		...
		$args['show_in_rest']['schema'] = _parse_args(
	...
	$__registered_theme_features[ $feature ] = $args;
...
function get_registered_theme_features() {
	...
	return $__registered_theme_features;
}
...
function get_registered_theme_feature( $feature ) {
	...
	return isset( $__registered_theme_features[ $feature ] ) ? $__registered_theme_features[ $feature ] : null;
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = _get_attachment_url( $id );
...
/**
 * Checks if a theme has been changed and runs 'after_switch_theme' hook on the next  load.
 *
...
function check_theme_switched() {
	...
	if ( $stylesheet ) {
		$old_theme = _get_theme( $stylesheet );

		// Prevent widget & menu mapping from running since Customizer already called it up front.
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '__menus_changed' );
			remove_action( 'after_switch_theme', '__sidebars_changed' );
			update_option( 'theme_switched_via_customizer', false );
		}

		if ( $old_theme->exists() ) {
			/**
			 * Fires on the first  load after a theme switch if the old theme still exists.
			 *
...
/**
 * Includes and instantiates the _Customize_Manager class.
 *
 * Loads the Customizer at plugins_loaded when accessing the customize.php admin
 * page or when any request includes a _customize=on param or a customize_changeset
 ...
 * @global _Customize_Manager $_customize
 */
function __customize_include() {

	$is_customize_admin_page = ( is_admin() && 'customize.php' === basename( $_SERVER['PHP_SELF'] ) );
	$should_include          = (
		$is_customize_admin_page
		||
		( isset( $_REQUEST['_customize'] ) && 'on' === $_REQUEST['_customize'] )
		||
	...
	/*
	 * Note that _unslash() is not being used on the input vars because it is
	 * called before _magic_quotes() gets called. Besides this fact, none of
	...
	$input_vars = array_merge(
		_array_slice_assoc( $_GET, $keys ),
		_array_slice_assoc( $_POST, $keys )
	);
	...
	// Note that theme will be sanitized via _Theme.
	 ...
	 * here in the case of a customize_save action because this will cause  to think
	...
	$is_customize_save_action = (
		...
		&&
		'customize_save' === _unslash( $_REQUEST['action'] )
	);
	$settings_previewed       = ! $is_customize_save_action;

	require_once ABSPATH . INC . '/class-customize-manager.php';
	$GLOBALS['_customize'] = new _Customize_Manager(
		compact(
			'changeset_uuid',
			'theme',
			'messenger_channel',
 ...
 * @global _Customize_Manager $_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param _Post $changeset_post Changeset post object.
 */
function __customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	global $_customize, $db;
	...
	if ( empty( $_customize ) ) {
		require_once ABSPATH . INC . '/class-customize-manager.php';
		$_customize = new _Customize_Manager(
			array(
	...
	if ( ! did_action( 'customize_register' ) ) {
		 ...
		 * will have a $_customize with those settings registered since they
		 ...
		 */
		remove_action( 'customize_register', array( $_customize, 'register_controls' ) );
		$_customize->register_controls();

		/** This filter is documented in /includes/class-customize-manager.php */
		do_action( 'customize_register', $_customize );
	}
	$_customize->_publish_changeset_values( $changeset_post->ID );
	 ...
	 */
	if ( ! _revisions_enabled( $changeset_post ) ) {
		$_customize->trash_changeset_post( $changeset_post->ID );
	}
}
 ...
 */
function __customize_changeset_filter_insert_post_data( $post_data, $supplied_post_data ) {
 ...
 * @since 3.4.0
 */
function __customize_loader_settings() {
	...
	$browser = array(
		'mobile' => _is_mobile(),
		'ios'    => _is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _CustomizeLoaderSettings = ' . _json_encode( $settings ) . ';';

	$_scripts = _scripts();
	$data       = $_scripts->get_data( 'customize-loader', 'data' );
	if ( $data ) {
		$script = "$data\n$script";
	}

	$_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function _customize_url( $stylesheet = '' ) {
 ...
 */
function _customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin['host'] ) != strtolower( $home_origin['host'] ) );
...
function is_customize_preview() {
	...
	return ( $_customize instanceof _Customize_Manager ) && $_customize->is_preview();
}
 ...
 * garbage-collected at the same time by `_delete_auto_drafts()`. Otherwise,
 ...
 *
 * @link https://core.trac..org/ticket/39752
 *
 * @since 4.8.0
 * @access private
 * @see _delete_auto_drafts()
 *
 * @global db $db  database abstraction object.
 *
 ...
 */
function __keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	global $db;
	unset( $old_status );
	 ...
	 * are then visible in the  Admin. We cannot wait for the deletion of the changeset in which
	 * __delete_customize_changeset_dependent_auto_drafts() will be called, since they need to be
	...
	if ( 'auto-draft' === $new_status ) {
		 ...
		 */
		$post_args['post_date'] = $post->post_date; // Note _delete_auto_drafts() only looks at this date.
	} else {
		/*
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default  block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
 ...
 */
function _is_block_theme() {
	return _get_theme()->is_block_theme();
}
