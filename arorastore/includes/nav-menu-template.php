...
require_once ABSPATH . INC . '/class-walker-nav-menu.php';
 ...
 *
 *     @type int|string|_Term $menu                 Desired menu. Accepts a menu ID, slug, name, or object.
 *                                                    Default empty.
 ...
 *                                                    Default is '_page_menu'. Set to false for no fallback.
...
function _nav_menu( $args = array() ) {
	 ...
	 * @see _nav_menu()
	 *
	 * @param array $args Array of _nav_menu() arguments.
	 */
	$args = apply_filters( '_nav_menu_args', $args );
	$args = (object) $args;

	/**
	 * Filters whether to short-circuit the _nav_menu() output.
	 *
	 * Returning a non-null value from the filter will short-circuit _nav_menu(),
	 ...
	 * @param stdClass    $args   An object containing _nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'pre__nav_menu', null, $args );
	...
	$menu = _get_nav_menu_object( $args->menu );

	// Get the nav menu based on the theme_location.
	$locations = get_nav_menu_locations();
	if ( ! $menu && $args->theme_location && $locations && isset( $locations[ $args->theme_location ] ) ) {
		$menu = _get_nav_menu_object( $locations[ $args->theme_location ] );
	}

	// Get the first menu that has items if we still can't find a menu.
	if ( ! $menu && ! $args->theme_location ) {
		$menus = _get_nav_menus();
		foreach ( $menus as $menu_maybe ) {
			$menu_items = _get_nav_menu_items( $menu_maybe->term_id, array( 'update_post_term_cache' => false ) );
			if ( $menu_items ) {
				$menu = $menu_maybe;
				break;
			}
	...
	if ( $menu && ! is__error( $menu ) && ! isset( $menu_items ) ) {
		$menu_items = _get_nav_menu_items( $menu->term_id, array( 'update_post_term_cache' => false ) );
	}
	 ...
	 *  - Otherwise, bail.
	 */
	if ( ( ! $menu || is__error( $menu ) || ( isset( $menu_items ) && empty( $menu_items ) && ! $args->theme_location ) )
		&& isset( $args->fallback_cb ) && $args->fallback_cb && is_callable( $args->fallback_cb ) ) {
			return call_user_func( $args->fallback_cb, (array) $args );
	}

	if ( ! $menu || is__error( $menu ) ) {
		return false;
	}
	...
	if ( $args->container ) {
		 ...
		 */
		$allowed_tags = apply_filters( '_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
	...
	__menu_item_classes_by_context( $menu_items );
	 ...
	 * @param stdClass $args              An object containing _nav_menu() arguments.
	 */
	$sorted_menu_items = apply_filters( '_nav_menu_objects', $sorted_menu_items, $args );
	 ...
	 * @param stdClass $args  An object containing _nav_menu() arguments.
	 */
	$items = apply_filters( '_nav_menu_items', $items, $args );
	/**
	 ...
	 * @param stdClass $args  An object containing _nav_menu() arguments.
	 */
	$items = apply_filters( "_nav_menu_{$menu->slug}_items", $items, $args );
	 ...
	 * @param stdClass $args     An object containing _nav_menu() arguments.
	 */
	$nav_menu = apply_filters( '_nav_menu', $nav_menu, $args );
 ...
 */
function __menu_item_classes_by_context( &$menu_items ) {
	global $_query, $_rewrite;

	$queried_object    = $_query->get_queried_object();
	$queried_object_id = (int) $_query->queried_object_id;
	...
	if ( $_query->is_singular && ! empty( $queried_object->post_type ) && ! is_post_type_hierarchical( $queried_object->post_type ) ) {
		foreach ( (array) get_object_taxonomies( $queried_object->post_type ) as $taxonomy ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				$term_hierarchy = _get_term_hierarchy( $taxonomy );
				$terms          = _get_object_terms( $queried_object_id, $taxonomy, array( 'fields' => 'ids' ) );
				if ( is_array( $terms ) ) {
	...
	$privacy_policy_page_id = (int) get_option( '_page_for_privacy_policy' );
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		if ( $_query->is_singular && 'taxonomy' === $menu_item->type
			...
			&& (
				( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
					&& $_query->is_home && $home_page_id == $menu_item->object_id )
				|| ( 'post_type' === $menu_item->type && $_query->is_singular )
				|| ( 'taxonomy' === $menu_item->type
					&& ( $_query->is_category || $_query->is_tag || $_query->is_tax )
			...
			if ( 'post_type' === $menu_item->type && 'page' === $menu_item->object ) {
				// Back compat classes for pages to match _page_menu().
				$classes[] = 'page_item';
			...
			$_indexless_current = untrailingslashit( preg_replace( '/' . preg_quote( $_rewrite->index, '/' ) . '$/', '', $current_url ) );
			...
			if ( $raw_item_url && in_array( $item_url, $matches, true ) ) {
				...
				if ( in_array( home_url(), array( untrailingslashit( $current_url ), untrailingslashit( $_indexless_current ) ), true ) ) {
					// Back compat for home link to match _page_menu().
					$classes[] = 'current_page_item';
				}
				$active_parent_item_ids[]   = (int) $menu_item->menu_item_parent;
				$active_parent_object_ids[] = (int) $menu_item->post_parent;
		...
		// Back-compat with _page_menu(): add "current_page_parent" to static home page link for any non-page query.
		if ( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
			&& empty( $_query->is_page ) && $home_page_id == $menu_item->object_id
		) {
