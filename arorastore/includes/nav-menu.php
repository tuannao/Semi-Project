 ...
 * @since 3.0.0
 *
 * @param int|string|_Term $menu Menu ID, slug, name, or object.
 * @return _Term|false Menu object on success, false if $menu param isn't supplied or term does not exist.
 */
function _get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is__error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filters the nav_menu term retrieved for _get_nav_menu_object().
	 *
	 * @since 4.3.0
	 *
	 * @param _Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|_Term $menu     The menu ID, slug, name, or object passed to _get_nav_menu_object().
	 */
	return apply_filters( '_get_nav_menu_object', $menu_obj, $menu );
}
 ...
 * @since 3.0.0
 *
 * @param int|string|_Term $menu Menu ID, slug, name, or object of menu to check.
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = _get_nav_menu_object( $menu );
...
function register_nav_menus( $locations = array() ) {
	...
	$__registered_nav_menus = array_merge( (array) $__registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $__registered_nav_menus ) && isset( $__registered_nav_menus[ $location ] ) ) {
		unset( $__registered_nav_menus[ $location ] );
		if ( empty( $__registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $__registered_nav_menus;
	if ( isset( $__registered_nav_menus ) ) {
		return $__registered_nav_menus;
	}
	return array();
}
 ...
 */
function _get_nav_menu_name( $location ) {
	$menu_name = '';

	$locations = get_nav_menu_locations();

	if ( isset( $locations[ $location ] ) ) {
		$menu = _get_nav_menu_object( $locations[ $location ] );
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( '_get_nav_menu_name', $menu_name, $location );
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is__error( $menu_item_id ) && ( 'nav_menu_item' === get_post_type( $menu_item_id ) ) );
}
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|_Error Menu ID on success, _Error object on failure.
 */
function _create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return _update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 *
 * @param int|string|_Term $menu Menu ID, slug, name, or object.
 * @return bool|_Error True on success, false or _Error object on failure.
 */
function _delete_nav_menu( $menu ) {
	$menu = _get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = _delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is__error( $result ) ) {
		 ...
		 */
		do_action( '_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|_Error Menu ID on success, _Error object on failure.
 */
function _update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = _get_nav_menu_object( $menu_id );
	...
	if ( ! $_menu || is__error( $_menu ) ) {
		...
		$_menu = _insert_term( $menu_data['menu-name'], 'nav_menu', $args );
		 ...
		 */
		do_action( '_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = _update_term( $menu_id, 'nav_menu', $args );

	if ( is__error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( '_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * @return int|_Error The menu item's database ID or _Error object on failure.
 */
function _update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// Make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects.
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new _Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = _get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new _Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	$args = _parse_args( $menu_item_data, $defaults );

	if ( 0 == $menu_id ) {
		$args['menu-item-position'] = 1;
	} elseif ( 0 == (int) $args['menu-item-position'] ) {
		$menu_items                 = 0 == $menu_id ? array() : (array) _get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( _unslash( $args['menu-item-title'] ) === _specialchars_decode( $original_title ) ) {
			$args['menu-item-title'] = '';
		}

		// Hack to get  to create a post object when too many properties are empty.
	...
	$post_date = _resolve_post_date( $args['menu-item-post-date'], $args['menu-item-post-date-gmt'] );
	if ( $post_date ) {
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = _insert_post( $post, true, $fire_after_hooks );
		if ( ! $menu_item_db_id || is__error( $menu_item_db_id ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @see _update_nav_menu_item()
		 *
		 * @param int   $menu_id         ID of the updated menu.
		 * @param int   $menu_item_db_id ID of the new menu item.
		 * @param array $args            An array of arguments used to update/add the menu item.
		 */
		do_action( '_add_nav_menu_item', $menu_id, $menu_item_db_id, $args );
	}

	// Associate the menu item with the menu term.
	// Only set the menu term if it isn't set to avoid unnecessary _get_object_terms().
	if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, 'nav_menu', (int) $menu->term_id ) ) ) {
		$update_terms = _set_object_terms( $menu_item_db_id, array( $menu->term_id ), 'nav_menu' );
		if ( is__error( $update_terms ) ) {
			return $update_terms;
		}
	}
	...
	if ( $update ) {
		...
		$update_post = _update_post( $post, true );
		if ( is__error( $update_post ) ) {
			return $update_post;
		}
	}
	 ...
	 */
	do_action( '_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 *                    Default empty array.
 * @return _Term[] An array of menu objects.
 */
function _get_nav_menus( $args = array() ) {
	$defaults = array(
		'taxonomy'   => 'nav_menu',
		'hide_empty' => false,
		'orderby'    => 'name',
	 ...
	 * @see get_terms()
	 *
	 * @param _Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 */
	return apply_filters( '_get_nav_menus', get_terms( $args ), $args );
}

/**
 * Determines whether a menu item is valid.
 *
 * @link https://core.trac..org/ticket/13958
 *
 * @since 3.2.0
 * @access private
 *
 ...
 */
function _get_nav_menu_items( $menu, $args = array() ) {
	$menu = _get_nav_menu_object( $menu );
	...
	$items = array_map( '_setup_nav_menu_item', $items );
	 ...
	 */
	return apply_filters( '_get_nav_menu_items', $items, $menu, $args );
}
 ...
 */
function _setup_nav_menu_item( $menu_item ) {
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $object ) {
					...
					if ( function_exists( 'get_post_states' ) ) {
						...
						if ( $post_states ) {
							$menu_item->type_label = _strip_all_tags( implode( ', ', $post_states ) );
						}
					}
				} else {
				...
				$post_content          = _trim_words( $menu_item->post_content, 200 );
				...
				if ( $original_object && ! is__error( $original_object ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', _trim_words( $menu_item->post_content, 200 ) );
			}
	 ...
	 */
	return apply_filters( '_setup_nav_menu_item', $menu_item );
}
 ...
 */
function _get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function __delete_post_menu_item( $object_id ) {
	$object_id = (int) $object_id;

	$menu_item_ids = _get_associated_nav_menu_items( $object_id, 'post_type' );

	foreach ( (array) $menu_item_ids as $menu_item_id ) {
		_delete_post( $menu_item_id, true );
	}
}
 ...
 */
function __delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = _get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 * @param _Post $post       The post object being transitioned from one status to another.
 */
function __auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = _get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		foreach ( $items as $item ) {
		...
		_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function __delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '__delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			_trash_post( $stub_post_id );
			delete_post_meta( $stub_post_id, '_customize_changeset_uuid' );
		}
	}
	add_action( 'delete_post', '__delete_customize_changeset_dependent_auto_drafts' );
}
 ...
 * @since 4.9.0
 */
function __menus_changed() {
	$old_nav_menu_locations    = get_option( 'theme_switch_menu_locations', array() );
	$new_nav_menu_locations    = get_nav_menu_locations();
	$mapped_nav_menu_locations = _map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function _map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
