...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof _Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
...
function locate_template( $template_names, $load = false, $require_once = true, $args = array() ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists( STYLESHEETPATH . '/' . $template_name ) ) {
			...
			break;
		} elseif ( file_exists( ABSPATH . INC . '/theme-compat/' . $template_name ) ) {
			$located = ABSPATH . INC . '/theme-compat/' . $template_name;
			break;
		}
	}
 ...
 * @global int        $id
 * @global _Comment $comment       Global comment object.
 * @global int        $user_ID
 *
...
function load_template( $_template_file, $require_once = true, $args = array() ) {
	global $posts, $post, $_did_header, $_query, $_rewrite, $db, $_version, $, $id, $comment, $user_ID;

	if ( is_array( $_query->query_vars ) ) {
		/*
		 ...
		 */
		// phpcs:ignore .PHP.DontExtract.extract_extract
		extract( $_query->query_vars, EXTR_SKIP );
	}
