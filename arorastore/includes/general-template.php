...
function get_search_form( $args = array() ) {
	 ...
	 * @since 5.5.0 The `$args` parameter was added.
	 *
	 * @link https://core.trac..org/ticket/19321
	 *
 ...
 *     @type string $id_submit      ID attribute value for the submit button. Default 'submit'.
	...
	$form =
		...
		sprintf(
			'<p class="login-submit">
				<input type="submit" name="submit" id="%1$s" class="button button-primary" value="%2$s" />
 ...
 * @since 1.5.0
 *
 * @link https://core.trac..org/ticket/1458 Explanation of 'wp_meta' action.
 */
function wp_meta() {
	/**
...
function get_calendar( $initial = true, $echo = true ) {
	...
	$calendar_output  = '<table id="calendar" class="calendar-table">
	<caption>' . sprintf(
		$calendar_caption,
	...
	$calendar_output .= '<nav aria-label="' . __( 'Previous and next months' ) . '" class="calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . '<span class="calendar-nav-prev"><a href="' . get_month_link( $previous->year, $previous->month ) . '">&laquo; ' .
			$wp_locale->get_month_abbrev( $wp_locale->get_month( $previous->month ) ) .
		'</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="calendar-nav-prev">&nbsp;</span>';
	}

	$calendar_output .= "\n\t\t" . '<span class="pad">&nbsp;</span>';

	if ( $next ) {
		$calendar_output .= "\n\t\t" . '<span class="calendar-nav-next"><a href="' . get_month_link( $next->year, $next->month ) . '">' .
			$wp_locale->get_month_abbrev( $wp_locale->get_month( $next->month ) ) .
		' &raquo;</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="calendar-nav-next">&nbsp;</span>';
	}
 ...
 *
 * The object will be set to the timezone from  settings.
 *
 ...
 * _WP_Editors should not be used directly. See https://core.trac..org/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running wp_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac..org/ticket/19173 for more information.
 *
...
function wp_get_code_editor_settings( $args ) {
	$settings = array(
		...
		'jshint'     => array(
			// The following are copied from <https://github.com//-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
...
/**
 * Displays the URL of a  admin CSS file.
 *
...
function wp_admin_css_uri( $file = 'admin' ) {
	...
	/**
	 * Filters the URI of a  admin CSS file.
	 *
	 * @since 2.3.0
	 *
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		...
		case 'xhtml':
			...
			break;
		case 'atom':
			$gen = '<generator uri="https://.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '"></generator>';
			break;
		case 'rss2':
			$gen = '<generator>' . esc_url_raw( 'https://.org/?v=' . get_bloginfo_rss( 'version' ) ) . '</generator>';
			break;
		case 'rdf':
			$gen = '<admin:generatorAgent rdf:resource="' . esc_url_raw( 'https://.org/?v=' . get_bloginfo_rss( 'version' ) ) . '" />';
			break;
		case 'comment':
			$gen = '<!-- generator="/' . esc_attr( get_bloginfo( 'version' ) ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $echo, $type ) { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
