...
function shortcode_unautop( $text ) {
	...
	// phpcs:disable Squiz.Strings.ConcatenationSpacing.PaddingFound,.WhiteSpace.PrecisionAlignment.Found -- don't remove regex indentation
	$pattern =
		'/'
		. '<p>'                              // Opening paragraph.
...
function wp_check_invalid_utf8( $string, $strip = false ) {
	...
	// phpcs:ignore .PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $string.
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://.org/support/article/using-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
...
// phpcs:disable .WP.CapitalPDangit.Misspelled, .NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
 * Forever eliminate "" from the planet (or at least the little bit we can influence).
 *
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			...
			'wpemoji' => apply_filters( 'script_loader_src', includes_url( "js/emoji.js?$version" ), 'wpemoji' ),
			/** This filter is documented in includes/class.scripts.php */
		...
		$settings['source'] = array(
			/** This filter is documented in includes/class.scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/emoji-release.min.js?$version" ), 'concatemoji' ),
		);
	}

	wp_print_inline_script_tag(
		sprintf( 'window._wpemojiSettings = %s;', wp_json_encode( $settings ) ) . "\n" .
			file_get_contents( sprintf( ABSPATH . WPINC . '/js/emoji-loader' . wp_scripts_get_suffix() . '.js' ) )
	);
}
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] && false !== strpos( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 * @return array An array to match all emoji that  recognises.
 */
