 ...
 *
 * @since 2.9.0
 * @since 4.4.0 `$post` can be a post ID or _Post object.
 *
...
function has_post_thumbnail( $post = null ) {
	 ...
	 * @param int|_Post|null $post          Post ID or _Post object. Default is global `$post`.
 ...
 *
 * @param _Query $_query Optional. A _Query instance. Defaults to the $_query global.
 */
function update_post_thumbnail_cache( $_query = null ) {
	if ( ! $_query ) {
		$_query = $GLOBALS['_query'];
	}

	if ( $_query->thumbnails_cached ) {
		return;
	}
...
function get_the_post_thumbnail( $post = null, $size = 'post-thumbnail', $attr = '' ) {
	...
	if ( $post_thumbnail_id ) {
		 ...
		 *
		 * Provides "just in time" filtering of all filters in _get_attachment_image().
		 *
		 * @since 2.9.0
		 *
		 * @param int          $post_id           The post ID.
		...
		$loading = _get_loading_attr_default( 'the_post_thumbnail' );
		...
		$html = _get_attachment_image( $post_thumbnail_id, $size, false, $attr );
...
function get_the_post_thumbnail_url( $post = null, $size = 'post-thumbnail' ) {
	...
	$thumbnail_url = _get_attachment_image_url( $post_thumbnail_id, $size );
...
function get_the_post_thumbnail_caption( $post = null ) {
	...
	$caption = _get_attachment_caption( $post_thumbnail_id );
