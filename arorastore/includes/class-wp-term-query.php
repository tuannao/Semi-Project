...
class WP_Term_Query {
	...
	public function get_terms() {
		 ...
		 *
		 * Return a non-null value to bypass ' default term queries.
		 *
		 * @since 5.3.0
		 *
		...
		if ( 'count' === $_fields ) {
			$count = $wpdb->get_var( $this->request ); // phpcs:ignore .DB.PreparedSQL.NotPrepared
			wp_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}

		$terms = $wpdb->get_results( $this->request ); // phpcs:ignore .DB.PreparedSQL.NotPrepared
