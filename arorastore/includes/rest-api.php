...
function register_rest_route( $namespace, $route, $args = array(), $override = false ) {
	if ( empty( $namespace ) ) {
		 ...
		 * non-namespaced route, call `_REST_Server::register_route` directly.
		 */
 ...
 * @since 4.7.0
 *
 * @global array $_rest_additional_fields Holds registered fields, organized
 *                                          by object type.
 *
...
function register_rest_field( $object_type, $attribute, $args = array() ) {
	...
	foreach ( $object_types as $object_type ) {
		$_rest_additional_fields[ $object_type ][ $attribute ] = $args;
	}
}
...
function rest_api_register_rewrites() {
	...
	add_rewrite_rule( '^' . $_rewrite->index . '/' . rest_get_url_prefix() . '/?$', 'index.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . $_rewrite->index . '/' . rest_get_url_prefix() . '/(.*)?', 'index.php?rest_route=/$matches[1]', 'top' );
}
...
function create_initial_rest_routes() {
	foreach ( get_post_types( array( 'show_in_rest' => true ), 'objects' ) as $post_type ) {
		...
		if ( post_type_supports( $post_type->name, 'revisions' ) ) {
			$revisions_controller = new _REST_Revisions_Controller( $post_type->name );
			$revisions_controller->register_routes();
		}

		if ( 'attachment' !== $post_type->name ) {
			$autosaves_controller = new _REST_Autosaves_Controller( $post_type->name );
			$autosaves_controller->register_routes();
		}
	}

	// Post types.
	$controller = new _REST_Post_Types_Controller;
	$controller->register_routes();

	// Post statuses.
	$controller = new _REST_Post_Statuses_Controller;
	$controller->register_routes();

	// Taxonomies.
	$controller = new _REST_Taxonomies_Controller;
	$controller->register_routes();
	...
	// Users.
	$controller = new _REST_Users_Controller;
	$controller->register_routes();

	// Application Passwords
	$controller = new _REST_Application_Passwords_Controller();
	$controller->register_routes();

	// Comments.
	$controller = new _REST_Comments_Controller;
	$controller->register_routes();

	$search_handlers = array(
		new _REST_Post_Search_Handler(),
		new _REST_Term_Search_Handler(),
		new _REST_Post_Format_Search_Handler(),
	);
	 ...
	 *                               handler instance must extend the `_REST_Search_Handler` class.
	 *                               Default is only a handler for posts.
	 */
	$search_handlers = apply_filters( '_rest_search_handlers', $search_handlers );

	$controller = new _REST_Search_Controller( $search_handlers );
	$controller->register_routes();

	// Block Renderer.
	$controller = new _REST_Block_Renderer_Controller();
	$controller->register_routes();

	// Block Types.
	$controller = new _REST_Block_Types_Controller();
	$controller->register_routes();

	// Global Styles.
	$controller = new _REST_Global_Styles_Controller;
	$controller->register_routes();

	// Settings.
	$controller = new _REST_Settings_Controller;
	$controller->register_routes();

	// Themes.
	$controller = new _REST_Themes_Controller;
	$controller->register_routes();

	// Plugins.
	$controller = new _REST_Plugins_Controller();
	$controller->register_routes();

	// Sidebars.
	$controller = new _REST_Sidebars_Controller();
	$controller->register_routes();

	// Widget Types.
	$controller = new _REST_Widget_Types_Controller();
	$controller->register_routes();

	// Widgets.
	$controller = new _REST_Widgets_Controller();
	$controller->register_routes();

	// Block Directory.
	$controller = new _REST_Block_Directory_Controller();
	$controller->register_routes();

	// Pattern Directory.
	$controller = new _REST_Pattern_Directory_Controller();
	$controller->register_routes();

	// Block Patterns.
	$controller = new _REST_Block_Patterns_Controller();
	$controller->register_routes();

	// Block Pattern Categories.
	$controller = new _REST_Block_Pattern_Categories_Controller();
	$controller->register_routes();

	// Site Health.
	$site_health = _Site_Health::get_instance();
	$controller  = new _REST_Site_Health_Controller( $site_health );
	$controller->register_routes();

	// URL Details.
	$controller = new _REST_URL_Details_Controller();
	$controller->register_routes();

	// Menu Locations.
	$controller = new _REST_Menu_Locations_Controller();
	$controller->register_routes();

	// Site Editor Export.
	$controller = new _REST_Edit_Site_Export_Controller();
	$controller->register_routes();
}
...
function rest_api_loaded() {
	if ( empty( $GLOBALS['']->query_vars['rest_route'] ) ) {
		return;
	}
	...
	$route = untrailingslashit( $GLOBALS['']->query_vars['rest_route'] );
	if ( empty( $route ) ) {
		$route = '/';
	}
...
function rest_get_url_prefix() {
	 ...
	 * @since 4.4.0
	 *
	 * @param string $prefix URL prefix. Default 'json'.
	 */
	return apply_filters( 'rest_url_prefix', 'json' );
}
...
function get_rest_url( $blog_id = null, $path = '/', $scheme = 'rest' ) {
	...
	if ( is_multisite() && get_blog_option( $blog_id, 'permalink_structure' ) || get_option( 'permalink_structure' ) ) {
		...
		if ( $_rewrite->using_index_permalinks() ) {
			$url = get_home_url( $blog_id, $_rewrite->index . '/' . rest_get_url_prefix(), $scheme );
		} else {
 ...
 * Do a REST request.
 *
 * Used primarily to route internal requests through _REST_Server.
 *
 * @since 4.4.0
 *
 * @param _REST_Request|string $request Request.
 * @return _REST_Response REST response.
 */
 ...
 * @since 4.5.0
 *
 * @global _REST_Server $_rest_server REST server instance.
 *
 * @return _REST_Server REST server instance.
 */
function rest_get_server() {
	/* @var _REST_Server $_rest_server */
	global $_rest_server;

	if ( empty( $_rest_server ) ) {
		/**
		 ...
		 * @since 4.4.0
		 *
		 * @param string $class_name The name of the server class. Default '_REST_Server'.
		 */
		$_rest_server_class = apply_filters( '_rest_server_class', '_REST_Server' );
		$_rest_server       = new $_rest_server_class;
		 ...
		 * @since 4.4.0
		 *
		 * @param _REST_Server $_rest_server Server object.
		 */
		do_action( 'rest_api_init', $_rest_server );
	}
 ...
 *
 * @param array|string|_REST_Request $request Request to check.
 * @return _REST_Request REST request instance.
 */
function rest_ensure_request( $request ) {
	if ( $request instanceof _REST_Request ) {
		return $request;
	}

	if ( is_string( $request ) ) {
		return new _REST_Request( 'GET', $request );
	}
 ...
 *
 * This implements _REST_Response, allowing usage of `set_status`/`header`/etc
 * without needing to double-check the object. Will also allow _Error to indicate error
 * responses, so users should immediately check for this value.
 *
 * @since 4.4.0
 *
 * @param _REST_Response|_Error|_HTTP_Response|mixed $response Response to check.
 * @return _REST_Response|_Error If response generated an error, _Error, if response
 *                                   is already an instance, _REST_Response, otherwise
 *                                   returns a new _REST_Response instance.
 */
function rest_ensure_response( $response ) {
	...
	if ( $response instanceof _REST_Response ) {
		return $response;
	}

	// While _HTTP_Response is the base class of _REST_Response, it doesn't provide
	// all the required methods used in _REST_Server::dispatch().
	if ( $response instanceof _HTTP_Response ) {
 ...
 * @param string|null $version  The version of  where the message was added.
 */
function rest_handle_doing_it_wrong( $function, $message, $version ) {
	...
	if ( $version ) {
		/* translators: Developer debugging message. 1: PHP function name, 2:  version number, 3: Explanatory message. */
	...
	header( sprintf( 'X-DoingItWrong: %s', $string ) );
}
 ...
 * @param _REST_Server  $handler  ResponseHandler instance (usually _REST_Server).
 * @param _REST_Request $request  The request that was used to make current response.
 * @return _REST_Response Modified response, either response or `null` to indicate pass-through.
 */
 ...
 * @since 4.4.0
 *
 * @param _REST_Response $response Current response being served.
 * @param _REST_Server   $server   ResponseHandler instance (usually _REST_Server).
 * @param _REST_Request  $request  The request that was used to make current response.
 * @return _REST_Response Response to be served, with "Allow" header if route has allowed methods.
 */
 ...
 * @param _REST_Server   $server   ResponseHandler instance (usually _REST_Server).
 * @param _REST_Request  $request  The request that was used to make current response.
 * @return _REST_Response Response to be served, trimmed down to contain a subset of fields.
 */
function rest_filter_response_fields( $response, $server, $request ) {
	...
	$fields = _parse_list( $request['_fields'] );
...
/**
 * Adds the REST API URL to the  RSD endpoint.
 *
 * @since 4.4.0
 *
 * @see get_rest_url()
 ...
 * @see get_rest_url()
 */
function rest_output_link__head() {
	$api_root = get_rest_url();
 ...
 * @since 4.4.0
 *
 * @global mixed          $_rest_auth_cookie
 *
 * @param _Error|mixed $result Error from another authentication handler,
 *                               null if we should handle it, or another value if not.
 * @return _Error|mixed|bool _Error if the cookie is invalid, the $result, otherwise true.
 */
function rest_cookie_check_errors( $result ) {
	 ...
	 * must have been used).
	 */
	if ( true !== $_rest_auth_cookie && is_user_logged_in() ) {
		return $result;
	}
	...
	} elseif ( isset( $_SERVER['HTTP_X__NONCE'] ) ) {
		$nonce = $_SERVER['HTTP_X__NONCE'];
	}
	...
	// Check the nonce.
	$result = _verify_nonce( $nonce, '_rest' );

	if ( ! $result ) {
		return new _Error( 'rest_cookie_invalid_nonce', __( 'Cookie check failed' ), array( 'status' => 403 ) );
	}

	// Send a refreshed nonce in header.
	rest_get_server()->send_header( 'X-Nonce', _create_nonce( '_rest' ) );
 ...
 *
 * Collects errors from _validate_auth_cookie for use by rest_cookie_check_errors.
 *
 * @since 4.4.0
 *
 * @see current_action()
...
function rest_cookie_collect_status() {
	...
	if ( 'auth_cookie_valid' !== $status_type ) {
		$_rest_auth_cookie = substr( $status_type, 12 );
		return;
	}

	$_rest_auth_cookie = true;
}
 ...
 *
 * @global _User|_Error|null $_rest_application_password_status
 * @global string|null $_rest_application_password_uuid
 *
 * @param _Error $user_or_error The authenticated user or error instance.
...
function rest_application_password_collect_status( $user_or_error, $app_password = array() ) {
	global $_rest_application_password_status, $_rest_application_password_uuid;

	$_rest_application_password_status = $user_or_error;

	if ( empty( $app_password['uuid'] ) ) {
		$_rest_application_password_uuid = null;
	} else {
		$_rest_application_password_uuid = $app_password['uuid'];
	}
}
...
function rest_get_authenticated_app_password() {
	global $_rest_application_password_uuid;

	return $_rest_application_password_uuid;
}
 ...
 * @global _User|_Error|null $_rest_application_password_status
 *
 * @param _Error|null|true $result Error from another authentication handler,
 *                                   null if we should handle it, or another value if not.
 * @return _Error|null|true _Error if the application password is invalid, the $result, otherwise true.
 */
function rest_application_password_check_errors( $result ) {
	global $_rest_application_password_status;

	if ( ! empty( $result ) ) {
		return $result;
	}

	if ( is__error( $_rest_application_password_status ) ) {
		$data = $_rest_application_password_status->get_error_data();

		if ( ! isset( $data['status'] ) ) {
			$data['status'] = 401;
		}

		$_rest_application_password_status->add_data( $data );

		return $_rest_application_password_status;
	}

	if ( $_rest_application_password_status instanceof _User ) {
		return true;
	}
 ...
 * @since 5.6.0
 *
 * @param _REST_Response $response The index response object.
...
function rest_is_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		$maybe_array = _parse_list( $maybe_array );
	}

	return _is_numeric_array( $maybe_array );
}
...
function rest_sanitize_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		return _parse_list( $maybe_array );
	}
...
function rest_handle_multi_type_schema( $value, $args, $param = '' ) {
	...
	if ( $invalid_types ) {
		_doing_it_wrong(
			...
			_sprintf( __( 'The "type" schema keyword for %1$s can only contain the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
...
/**
 * Formats a combining operation error into a _Error object.
 *
 * @since 5.6.0
 *
 ...
 * @return _Error      The combining operation error.
 */
function rest_get_combining_operation_error( $value, $param, $errors ) {
	...
	if ( count( $schema_titles ) === count( $errors ) ) {
		...
		return new _Error( 'rest_no_matching_schema', _sprintf( __( '%1$s is not a valid %2$l.' ), $param, $schema_titles ) );
	}

	/* translators: %s: Parameter. */
	return new _Error( 'rest_no_matching_schema', sprintf( __( '%s does not match any of the expected formats.' ), $param ) );
}
 ...
 * @param array  $args    The schema array to use.
 * @param string $param   The parameter name, used in error messages.
 * @return array|_Error The matching schema or _Error instance if all schemas do not match.
 */
 ...
 * @return array|_Error                The matching schema or _Error instance if the number of matching schemas is not equal to one.
 */
	...
	if ( count( $matching_schemas ) > 1 ) {
		...
		if ( count( $schema_titles ) === count( $matching_schemas ) ) {
			return new _Error(
				...
				/* translators: 1: Parameter, 2: Schema titles. */
				_sprintf( __( '%1$s matches %2$l, but should match only one.' ), $param, $schema_titles ),
 ...
 * @param array  $args   The schema array to use.
 * @param string $param  The parameter name, used in error messages.
 * @return true|_Error True if the "enum" contains the value or a _Error instance otherwise.
 */
function rest_validate_enum( $value, $args, $param ) {
	$sanitized_value = rest_sanitize_value_from_schema( $value, $args, $param );
	if ( is__error( $sanitized_value ) ) {
	...
	foreach ( $args['enum'] as $enum_value ) {
		$encoded_enum_values[] = is_scalar( $enum_value ) ? $enum_value : _json_encode( $enum_value );
	}

	if ( count( $encoded_enum_values ) === 1 ) {
		/* translators: 1: Parameter, 2: Valid values. */
		return new _Error( 'rest_not_in_enum', _sprintf( __( '%1$s is not %2$s.' ), $param, $encoded_enum_values[0] ) );
	}

	/* translators: 1: Parameter, 2: List of valid values. */
	return new _Error( 'rest_not_in_enum', _sprintf( __( '%1$s is not one of %2$l.' ), $param, $encoded_enum_values ) );
}
...
function rest_validate_value_from_schema( $value, $args, $param = '' ) {
	...
	if ( ! in_array( $args['type'], $allowed_types, true ) ) {
		_doing_it_wrong(
			...
			_sprintf( __( 'The "type" schema keyword for %1$s can only be one of the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
	...
	if ( ! empty( $args['enum'] ) ) {
		...
		if ( is__error( $enum_contains_value ) ) {
		...
		switch ( $args['format'] ) {
			case 'hex-color':
				if ( ! rest_parse_hex_color( $value ) ) {
					return new _Error( 'rest_invalid_hex_color', __( 'Invalid hex color.' ) );
				}
				break;

			case 'date-time':
				if ( ! rest_parse_date( $value ) ) {
					return new _Error( 'rest_invalid_date', __( 'Invalid date.' ) );
				}
				break;

			case 'email':
				if ( ! is_email( $value ) ) {
					return new _Error( 'rest_invalid_email', __( 'Invalid email address.' ) );
				}
				break;
			case 'ip':
				if ( ! rest_is_ip_address( $value ) ) {
					/* translators: %s: IP address. */
					return new _Error( 'rest_invalid_ip', sprintf( __( '%s is not a valid IP address.' ), $param ) );
				}
				break;
			case 'uuid':
				if ( ! _is_uuid( $value ) ) {
					/* translators: %s: The name of a JSON field expecting a valid UUID. */
					return new _Error( 'rest_invalid_uuid', sprintf( __( '%s is not a valid UUID.' ), $param ) );
				}
				break;
		}
	}
...
function rest_validate_array_value_from_schema( $value, $args, $param ) {
	...
	if ( ! empty( $args['uniqueItems'] ) && ! rest_validate_array_contains_unique_items( $value ) ) {
		/* translators: %s: Parameter. */
		return new _Error( 'rest_duplicate_items', sprintf( __( '%s has duplicate items.' ), $param ) );
	}
 ...
 * @param string $param The parameter name, used in error messages.
 * @return mixed|_Error The sanitized value or a _Error instance if the value cannot be safely sanitized.
 */
...
function rest_preload_api_request( $memo, $path ) {
	...
	$request = new _REST_Request( $method, $path_parts['path'] );
	...
	if ( 200 === $response->status ) {
		$server = rest_get_server();
		/** This filter is documented in includes/rest-api/class-rest-server.php */
...
function rest_get_route_for_post_type_items( $post_type ) {
	...
	$namespace = ! empty( $post_type->rest_namespace ) ? $post_type->rest_namespace : '/v2';
	 ...
	 *
	 * @param string       $route      The route path.
	 * @param _Post_Type $post_type  The post type object.
	 */
 ...
 * @since 5.5.0
 *
 * @param int|_Term $term Term ID or term object.
...
function rest_get_route_for_taxonomy_items( $taxonomy ) {
	...
	$namespace = ! empty( $taxonomy->rest_namespace ) ? $taxonomy->rest_namespace : '/v2';
	 ...
	 *
	 * @param string      $route    The route path.
	 * @param _Taxonomy $taxonomy The taxonomy object.
	 */
...
function rest_get_queried_resource_route() {
	if ( is_singular() ) {
		...
		$route = '//v2/users/' . get_queried_object_id();
	} else {
		$route = '';
	}
 ...
 *                       on `EDITABLE` endpoints. Default _REST_Server::CREATABLE.
 * @return array The endpoint arguments.
 */
function rest_get_endpoint_args_for_schema( $schema, $method = _REST_Server::CREATABLE ) {
	...
	foreach ( $schema_properties as $field_id => $params ) {
		...
		if ( _REST_Server::CREATABLE === $method && isset( $params['default'] ) ) {
			$endpoint_args[ $field_id ]['default'] = $params['default'];
		}

		if ( _REST_Server::CREATABLE === $method && ! empty( $params['required'] ) ) {
		...
		if ( isset( $params['arg_options'] ) ) {
			...
			if ( _REST_Server::CREATABLE !== $method ) {
 ...
 * @since 5.7.0
 *
 * @param _Error $error _Error instance.
 *
 * @return _REST_Response List of associative arrays with code and message keys.
 */
function rest_convert_error_to_response( $error ) {
	...
	return new _REST_Response( $data, $status );
}
