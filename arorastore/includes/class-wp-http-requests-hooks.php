...
class WP_HTTP_Requests_Hooks extends Requests_Hooks {
	...
	public function dispatch( $hook, $parameters = array() ) {
		...
		/**
		 * Transforms a native Request hook to a  action.
		 *
		 * This action maps Requests internal hook to a native  action.
		 *
		 * @see https://github.com//Requests/blob/master/docs/hooks.md
		 *
		 * @since 4.7.0
		 *
		 ...
		 * @param string $url URL to request.
		 */
		do_action_ref_array( "requests-{$hook}", $parameters, $this->request, $this->url ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
