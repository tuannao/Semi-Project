...
/**
 * Displays the ID of the current item in the  Loop.
 *
 * @since 0.71
 */
function the_ID() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
	echo get_the_ID();
}

/**
 * Retrieves the ID of the current item in the  Loop.
 *
 * @since 2.1.0
 *
 * @return int|false The ID of the current item in the  Loop. False if $post is not set.
 */
function get_the_ID() { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
	$post = get_post();
 ...
 *     @type _Post $post   Current post object to retrieve the title for.
 * }
...
function get_the_title( $post = 0 ) {
	...
	if ( ! is_admin() ) {
		if ( ! empty( $post->post_password ) ) {
			 ...
			 *                         Default 'Protected: %s'.
			 * @param _Post $post    Current post object.
			 */
 ...
 * @since 1.5.0
 *
 * @param int|_Post $post Optional. Post ID or post object. Default is global $post.
 */
 ...
 * @param string             $more_link_text Optional. Content for when there is more text.
 * @param bool               $strip_teaser   Optional. Strip teaser content before the more text. Default false.
 * @param _Post|object|int $post           Optional. _Post instance or Post ID/object. Default null.
 * @return string
 */
function get_the_content( $more_link_text = null, $strip_teaser = false, $post = null ) {
	...
	if ( preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
		...
		if ( ! empty( $matches[1] ) && ! empty( $more_link_text ) ) {
			$more_link_text = strip_tags( _kses_no_null( trim( $matches[1] ) ) );
		}
 ...
 * @param int|_Post     $post_id Optional. Post ID or post object. Defaults to the global `$post`.
 */
 ...
 * @param int|_Post     $post_id Optional. Post ID or post object.
...
function get_post_class( $class = '', $post_id = null ) {
	...
	if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
		...
		if ( $post_format && ! is__error( $post_format ) ) {
...
function get_body_class( $class = '' ) {
	...
	if ( is_search() ) {
		$classes[] = 'search';
		$classes[] = $_query->posts ? 'search-results' : 'search-no-results';
	}
	if ( is_paged() ) {
		$classes[] = 'paged';
	}
	...
	if ( is_singular() ) {
		$post_id   = $_query->get_queried_object_id();
		$post      = $_query->get_queried_object();
		...
		if ( is_attachment() ) {
			...
			$page_id = $_query->get_queried_object_id();
		...
		if ( is_post_type_archive() ) {
			...
			$author    = $_query->get_queried_object();
			$classes[] = 'author';
			...
			$term = $_query->get_queried_object();
	...
	$page = $_query->get( 'page' );

	if ( ! $page || $page < 2 ) {
		$page = $_query->get( 'paged' );
	}
 ...
 * @since 2.7.0
 *
 * @param int|_Post|null $post An optional post. Global $post used if not provided.
...
function post_password_required( $post = null ) {
	...
	if ( ! isset( $_COOKIE[ 'postpass_' . COOKIEHASH ] ) ) {
		/** This filter is documented in includes/post-template.php */
	...
	$hash = _unslash( $_COOKIE[ 'postpass_' . COOKIEHASH ] );
...
function _link_pages( $args = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param array $parsed_args An array of page link arguments. See _link_pages()
	 *                           for information on accepted arguments.
	 */
	$parsed_args = apply_filters( '_link_pages_args', $parsed_args );
	...
	if ( $multipage ) {
		if ( 'number' === $parsed_args['next_or_number'] ) {
			...
			for ( $i = 1; $i <= $numpages; $i++ ) {
				 ...
				 */
				$link = apply_filters( '_link_pages_link', $link, $i );
			...
			$prev    = $page - 1;
			if ( $prev > 0 ) {
				$link = __link_page( $prev ) . $parsed_args['link_before'] . $parsed_args['previouspagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in includes/post-template.php */
				$output .= apply_filters( '_link_pages_link', $link, $prev );
			}
			$next = $page + 1;
			if ( $next <= $numpages ) {
				if ( $prev ) {
				...
				$link = __link_page( $next ) . $parsed_args['link_before'] . $parsed_args['nextpagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in includes/post-template.php */
				$output .= apply_filters( '_link_pages_link', $link, $next );
			}
			$output .= $parsed_args['after'];
		}
	}
	 ...
	 * @param array|string $args   An array or query string of arguments. See _link_pages()
	 *                             for information on accepted arguments.
	 */
	$html = apply_filters( '_link_pages', $output, $args );
...
/**
 * Helper function for _link_pages().
 *
 * @since 3.1.0
 * @access private
 *
...
function __link_page( $i ) {
	...
	if ( 1 == $i ) {
		...
		if ( ! get_option( 'permalink_structure' ) || in_array( $post->post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$url = trailingslashit( get_permalink() ) . user_trailingslashit( "$_rewrite->pagination_base/" . $i, 'single_paged' );
		} else {
	...
	if ( is_preview() ) {
		...
		if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
			$query_args['preview_id']    = _unslash( $_GET['preview_id'] );
			$query_args['preview_nonce'] = _unslash( $_GET['preview_nonce'] );
		}
...
function _dropdown_pages( $args = '' ) {
	 ...
	 * @param array     $parsed_args The parsed arguments array. See _dropdown_pages()
	 *                               for information on accepted arguments.
	 * @param _Post[] $pages       Array of the page objects.
	 */
	$html = apply_filters( '_dropdown_pages', $output, $parsed_args, $pages );
...
function _list_pages( $args = '' ) {
	 ...
	 */
	$parsed_args['exclude'] = implode( ',', apply_filters( '_list_pages_excludes', $exclude_array ) );
	...
	if ( ! empty( $pages ) ) {
		...
		global $_query;
		if ( is_page() || is_attachment() || $_query->is_posts_page ) {
	 ...
	 * @param array     $parsed_args An array of page-listing arguments. See _list_pages()
	 *                               for information on accepted arguments.
	 * @param _Post[] $pages       Array of the page objects.
	 */
	$html = apply_filters( '_list_pages', $output, $parsed_args, $pages );
 ...
 *
 * The arguments are listed below and part of the arguments are for _list_pages() function.
 ...
 *
 * @param array|string $args {
 *     Optional. Array or string of arguments to generate a page menu. See `_list_pages()` for additional arguments.
 *
 ...
 */
function _page_menu( $args = array() ) {
	$defaults = array(
		'sort_column'  => 'menu_order, post_title',
		'menu_id'      => '',
		'menu_class'   => 'menu',
	 ...
	 * @see _page_menu()
	 *
	 * @param array $args An array of page menu arguments. See _page_menu()
	 *                    for information on accepted arguments.
	 */
	$args = apply_filters( '_page_menu_args', $args );
	...
	$list_args['title_li'] = '';
	$menu                 .= _list_pages( $list_args );

	$container = sanitize_text_field( $args['container'] );

	// Fallback in case `_nav_menu()` was called without a container.
	if ( empty( $container ) ) {
		$container = 'div';
	}
	...
	if ( $menu ) {
		...
		// _nav_menu() doesn't set before and after.
		if ( isset( $args['fallback_cb'] ) &&
			'_page_menu' === $args['fallback_cb'] &&
			'ul' !== $container ) {
			$args['before'] = "<ul>{$n}";
			$args['after']  = '</ul>';
		}
	 ...
	 * @param array  $args An array of arguments. See _page_menu()
	 *                     for information on accepted arguments.
	 */
	$menu = apply_filters( '_page_menu', $menu, $args );
 ...
 * @since 2.0.0
 *
 * @param int|_Post $id Optional. Post ID or post object.
...
function the_attachment_link( $id = 0, $fullsize = false, $deprecated = false, $permalink = false ) {
	...
	if ( $fullsize ) {
		echo _get_attachment_link( $id, 'full', $permalink );
	} else {
		echo _get_attachment_link( $id, 'thumbnail', $permalink );
	}
}
 ...
 *
 * @since 2.5.0
 * @since 4.4.0 The `$id` parameter can now accept either a post ID or `_Post` object.
 *
 * @param int|_Post  $id        Optional. Post ID or post object.
 ...
 */
function _get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false, $attr = '' ) {
	$_post = get_post( $id );

	if ( empty( $_post ) || ( 'attachment' !== $_post->post_type ) || ! _get_attachment_url( $_post->ID ) ) {
		return __( 'Missing Attachment' );
	}

	$url = _get_attachment_url( $_post->ID );
	...
	if ( $text ) {
		...
		$link_text = _get_attachment_image( $_post->ID, $size, $icon, $attr );
	} else {
		$link_text = '';
	}
	 ...
	 * @param int|_Post  $id        Post ID or object. Can be 0 for the current global post.
	 ...
	 */
	return apply_filters( '_get_attachment_link', "<a href='" . esc_url( $url ) . "'>$link_text</a>", $id, $size, $permalink, $icon, $text, $attr );
}
...
function prepend_attachment( $content ) {
	...
	if ( _attachment_is( 'video', $post ) ) {
		$meta = _get_attachment_metadata( get_the_ID() );
		$atts = array( 'src' => _get_attachment_url() );
		if ( ! empty( $meta['width'] ) && ! empty( $meta['height'] ) ) {
			$atts['width']  = (int) $meta['width'];
			$atts['height'] = (int) $meta['height'];
		}
		if ( has_post_thumbnail() ) {
			$atts['poster'] = _get_attachment_url( get_post_thumbnail_id() );
		}
		$p = _video_shortcode( $atts );
	} elseif ( _attachment_is( 'audio', $post ) ) {
		$p = _audio_shortcode( array( 'src' => _get_attachment_url() ) );
	} else {
		$p = '<p class="attachment">';
		// Show the medium sized image representation of the attachment if available, and link to the raw file.
		$p .= _get_attachment_link( 0, 'medium', false );
		$p .= '</p>';
	}
...
function get_page_template_slug( $post = null ) {
	...
	$template = get_post_meta( $post->ID, '__page_template', true );
 ...
 */
function _post_revision_title( $revision, $link = true ) {
	...
	if ( ! _is_post_revision( $revision ) ) {
		$date = sprintf( $currentf, $date );
	} elseif ( _is_post_autosave( $revision ) ) {
 ...
 */
function _post_revision_title_expanded( $revision, $link = true ) {
	$revision = get_post( $revision );
	if ( ! $revision ) {
		return $revision;
	}
	 ...
	 * @param _Post $revision             The revision object.
	 * @param bool    $link                 Whether to link to the revisions page, as passed into
	 *                                      _post_revision_title_expanded().
	 */
	return apply_filters( '_post_revision_title_expanded', $revision_date_author, $revision, $link );
}
 ...
 * @since 2.6.0
 *
 * @param int|_Post $post_id Optional. Post ID or _Post object. Default is global $post.
 * @param string      $type    'all' (default), 'revision' or 'autosave'
 */
function _list_post_revisions( $post_id = 0, $type = 'all' ) {
	$post = get_post( $post_id );
	if ( ! $post ) {
		return;
	}
	...
	$revisions = _get_post_revisions( $post->ID );
	if ( ! $revisions ) {
		return;
	}
	...
	foreach ( $revisions as $revision ) {
		...
		$is_autosave = _is_post_autosave( $revision );
		if ( ( 'revision' === $type && $is_autosave ) || ( 'autosave' === $type && ! $is_autosave ) ) {
			continue;
		}

		$rows .= "\t<li>" . _post_revision_title_expanded( $revision ) . "</li>\n";
	}
 ...
 * @since 5.7.0
 *
 * @param int|_Post|null $post Optional. Post ID or _Post object. Default is global $post.
 * @return _Post|null Parent post object, or null if there isn't one.
 */
function get_post_parent( $post = null ) {
	$_post = get_post( $post );
	return ! empty( $_post->post_parent ) ? get_post( $_post->post_parent ) : null;
}
