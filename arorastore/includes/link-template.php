 ...
 *
 * @param int|_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|_Post $post      Post ID, _Post object, or 0. Default 0.
	 */
...
function user_trailingslashit( $string, $type_of_url = '' ) {
	global $_rewrite;
	if ( $_rewrite->use_trailing_slashes ) {
 ...
 * @since 5.7.0
 *
 * @param _Post|int|null $post   Optional. Post ID or post object. Defaults to global $post.
 ...
 */
function _force_plain_post_permalink( $post = null, $sample = null ) {
	if (
		null === $sample &&
		is_object( $post ) &&
		isset( $post->filter ) &&
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param _Post $post      The post in question.
	...
	if (
		$permalink &&
		! _force_plain_post_permalink( $post )
	) {
		...
		if ( strpos( $permalink, '%category%' ) !== false ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param _Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (_Term objects) associated with the post.
				 * @param _Post  $post The post in question.
				 */
			...
			if ( empty( $category ) ) {
				...
				if ( $default_category && ! is__error( $default_category ) ) {
 ...
 * @global _Rewrite $_rewrite  rewrite component.
 *
 * @param int|_Post $id        Optional. Post ID or post object. Default is the global `$post`.
 * @param bool        $leavename Optional. Whether to keep post name. Default false.
 * @param bool        $sample    Optional. Is it a sample permalink. Default false.
 * @return string|_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $_rewrite->get_extra_permastruct( $post->post_type );

	$slug = $post->post_name;

	$force_plain_link = _force_plain_post_permalink( $post );
 ...
 * @since 1.5.0
 *
 * @param int|_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $_rewrite->get_page_permastruct();
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $force_plain_link || ! $parent_valid ) {
		$link = false;
	} elseif ( $_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $_rewrite->get_year_permastruct();
	if ( ! empty( $yearlink ) ) {
...
function get_month_link( $year, $month ) {
	...
	$monthlink = $_rewrite->get_month_permastruct();
	if ( ! empty( $monthlink ) ) {
...
function get_day_link( $year, $month, $day ) {
	...
	$daylink = $_rewrite->get_day_permastruct();
	if ( ! empty( $daylink ) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $_rewrite->get_feed_permastruct();

	if ( $permalink ) {
		if ( false !== strpos( $feed, 'comments_' ) ) {
			$feed      = str_replace( 'comments_', '', $feed );
			$permalink = $_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @since 2.5.0
 *
 * @param int|_Term|object $cat  The ID or category object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 3.0.0
 *
 * @param int|_Term|object $term     The ID or term object whose feed link will be retrieved.
 ...
 * @since 2.3.0
 *
 * @param int|_Term|object $tag  The ID or term object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 2.7.0
 *
 * @param int|_Term|object $tag      The ID or term object whose edit link will be retrieved.
 ...
 * @param _Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
 ...
 *
 * @param int|_Term|object $term        The ID or term object whose edit link will be retrieved.
 ...
 * @param string           $before Optional. Display before edit link. Default empty.
 * @param string           $after  Optional. Display after edit link. Default empty.
 * @param int|_Term|null $term   Optional. Term ID or object. If null, the queried object will be inspected. Default null.
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] ) {
			$struct = $_rewrite->front . $struct;
		} else {
			$struct = $_rewrite->root . $struct;
		}
 ...
 * @since 4.4.0
 *
 * @param int|_Post $post         Optional. Post ID or `_Post` object. Defaults to global `$post`.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', _nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @since 2.3.0
 *
 * @param int|_Comment $comment_id Optional. Comment ID or _Comment object.
 ...
 * @return _Post|null|string Post object if successful. Null if global $post is not set. Empty string if no
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		if ( $in_same_term ) {
			$join  .= " INNER JOIN $db->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $db->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$where .= $db->prepare( 'AND tt.taxonomy = %s', $taxonomy );

			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) ) {
				return '';
			}
			$term_array = _get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is__error( $term_array ) ) {
				return '';
			}
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $db->term_relationships tr LEFT JOIN $db->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( ',', array_map( 'intval', $excluded_terms ) ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $db->prepare( ' OR p.post_status = %s', $state );
			} else {
				$where .= $db->prepare( ' OR (p.post_author = %d AND p.post_status = %s)', $user_id, $state );
			}
		}
		$where .= ' )';
	} else {
	 ...
	 * @param _Post $post           _Post object.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $db->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms, $taxonomy, $post );
	...
	$query     = "SELECT p.ID FROM $db->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5( $query );
	$result    = _cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result ) {
	...
	_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like '_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 ...
 */
function adjacent_posts_rel_link__head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param _Post $post     The adjacent post.
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	if ( ! $_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = preg_replace( '|^' . preg_quote( $_rewrite->index, '|' ) . '|i', '', $request );
		$request = ltrim( $request, '/' );

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' !== $request ) ) {
			$base .= $_rewrite->index . '/';
		}

		if ( $pagenum > 1 ) {
			$request = ( ( ! empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $_rewrite->pagination_base . '/' . $pagenum, 'paged' );
		}
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['_query']->max_num_pages > 1 ) {
		// Make sure the nav element has an aria-label attribute: fallback to the screen reader text.
		if ( ! empty( $args['screen_reader_text'] ) && empty( $args['aria_label'] ) ) {
			$args['aria_label'] = $args['screen_reader_text'];
		}
...
function get_comments_pagenum_link( $pagenum = 1, $max_page = 0 ) {
	...
	if ( 'newest' === get_option( 'default_comments_page' ) ) {
		if ( $pagenum != $max_page ) {
			if ( $_rewrite->using_permalinks() ) {
				$result = user_trailingslashit( trailingslashit( $result ) . $_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged' );
			} else {
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty( $max_page ) ) {
		$max_page = $_query->max_num_comment_pages;
	}
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $_rewrite->using_permalinks() ) {
		$defaults['base'] = user_trailingslashit( trailingslashit( get_permalink() ) . $_rewrite->comments_pagination_base . '-%#%', 'commentpaged' );
	}
...
function content_url( $path = '' ) {
	$url = set_url_scheme( _CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$plugin        = _normalize_path( $plugin );
	$mu_plugin_dir = _normalize_path( MU_PLUGIN_DIR );
 ...
 * @since 4.6.0
 *
 * @param int|_Post $post Optional. Post ID or object. Default is global `$post`.
 * @return string|false The canonical URL, or false if the post does not exist or has not
 *                      been published yet.
 */
function _get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use `_get_canonical_url()`.
 */
 ...
 *                are not enabled.
 */
function _get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see '_head'} action.
 *
 * @since 3.0.0
 */
function _shortlink__head() {
	$shortlink = _get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the {@see ''} action.
 *
 * @since 3.0.0
 */
function _shortlink_header() {
	if ( headers_sent() ) {
		return;
	}
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = _get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof _User ) {
		// User object.
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof _Post ) {
		// Post object.
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof _Comment ) {
		...
		if ( ( ! $user || is__error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
...
function get_privacy_policy_url() {
	$url            = '';
	$policy_page_id = (int) get_option( '_page_for_privacy_policy' );
