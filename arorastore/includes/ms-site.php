 ...
 *                                Passed to the `_initialize_site` hook.
 * }
 * @return int|_Error The new site's ID on success, or error object on failure.
 */
function _insert_site( array $data ) {
	global $db;
	...
	$prepared_data = _prepare_site_data( $data, $defaults );
	if ( is__error( $prepared_data ) ) {
		return $prepared_data;
	}

	if ( false === $db->insert( $db->blogs, $prepared_data ) ) {
		return new _Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $db->last_error );
	}
	...
	if ( ! $new_site ) {
		return new _Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param _Site $new_site New site object.
	 */
	do_action( '_insert_site', $new_site );
	 ...
	 */
	do_action( '_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'mu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// LANG was passed with `$meta` to the `mu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'LANG', $meta ) ) {
			$meta['LANG'] = get_network_option( $new_site->network_id, 'LANG' );
		}

		// Rebuild the data expected by the `mu_new_blog` hook prior to 5.1.0 using allowed keys.
		// The `$allowed_data_fields` matches the one used in `mu_create_blog()`.
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see '_initialize_site'} instead.
		 *
 ...
 * @param array $data    Site data to update. See {@see _insert_site()} for the list of supported keys.
 * @return int|_Error The updated site's ID on success, or error object on failure.
 */
function _update_site( $site_id, array $data ) {
	global $db;

	if ( empty( $site_id ) ) {
		return new _Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new _Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = _prepare_site_data( $data, $defaults, $old_site );
	if ( is__error( $data ) ) {
		return $data;
	}

	if ( false === $db->update( $db->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new _Error( 'db_update_error', __( 'Could not update site in the database.' ), $db->last_error );
	}
	 ...
	 *
	 * @param _Site $new_site New site object.
	 * @param _Site $old_site Old site object.
	 */
	do_action( '_update_site', $new_site, $old_site );
 ...
 * @return _Site|_Error The deleted site object on success, or error object on failure.
 */
function _delete_site( $site_id ) {
	global $db;
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param _Error $errors   Error object to add validation errors to.
	 * @param _Site  $old_site The site object to be deleted.
	 */
	do_action( '_validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param _Site $old_site Deleted site object.
	 */
	do_action( '_uninitialize_site', $old_site );

	if ( is_site_meta_supported() ) {
		$blog_meta_ids = $db->get_col( $db->prepare( "SELECT meta_id FROM $db->blogmeta WHERE blog_id = %d ", $old_site->id ) );
		foreach ( $blog_meta_ids as $mid ) {
			delete_metadata_by_mid( 'blog', $mid );
		}
	}

	if ( false === $db->delete( $db->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new _Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $db->last_error );
	}
	 ...
	 * @param _Site $old_site Deleted site object.
	 */
	do_action( '_delete_site', $old_site );
 ...
 * @since 4.6.0
 *
 * @param _Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return _Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof _Site ) {
		...
		$_site = new _Site( $site );
	} else {
		$_site = _Site::get_instance( $site );
	}
...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $db->get_results( sprintf( "SELECT * FROM $db->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore .DB.PreparedSQL.NotPrepared
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	_cache_add_multiple( $site_data, 'sites' );
	_cache_add_multiple( $blog_details_data, 'blog-details' );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', '_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', '_check_site_meta_support_prefilter' );
	}
 ...
 *
 * @see _Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See _Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return array|int List of _Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                   or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new _Site_Query();
 ...
 *                               See {@see _insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param _Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|_Error Site data ready for a database transaction, or _Error in case a validation
 *                        error occurred.
 */
function _prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see _insert_site()} for the possibly included data.
	 */
	$data = apply_filters( '_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( _parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param _Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see _insert_site()}
	 *                               for the included data.
	 * @param _Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( '_validate_site_data', $errors, $data, $old_site );
 ...
 */
function _normalize_site_data( $data ) {
 ...
 * @since 5.1.0
 *
 * @param _Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see _insert_site()}
 *                               for the included data.
 * @param _Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function _validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = _checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
 ...
 *
 * @param int|_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|_Error True on success, or error object on failure.
 */
function _initialize_site( $site_id, array $args = array() ) {
	global $db, $_roles;
	...
	if ( ! $site ) {
		return new _Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( _is_site_initialized( $site ) ) {
		return new _Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param _Site    $site    Site that is being initialized.
	 * @param _Network $network Network that the site belongs to.
	 */
	$args = apply_filters( '_initialize_site_args', $args, $site, $network );

	$orig_installing = _installing();
	if ( ! $orig_installing ) {
		_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'LANG'      => get_network_option( $network->id, 'LANG' ),
			),
			$args['options']
		)
	);
	...
	_install_defaults( $args['user_id'] );
 ...
 * @return true|_Error True on success, or error object on failure.
 */
function _uninitialize_site( $site_id ) {
	global $db;
	...
	if ( ! _is_site_initialized( $site ) ) {
		return new _Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 */
	$drop_tables = apply_filters( 'mu_drop_tables', $tables, $site->id );

	foreach ( (array) $drop_tables as $table ) {
		$db->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore .DB.PreparedSQL.NotPrepared
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. @see _upload_dir()
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'mu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable .PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable .PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 */
function _is_site_initialized( $site_id ) {
	global $db;
	 ...
	 */
	$pre = apply_filters( 'pre__is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', '_switch_roles_and_user', 1 );
		switch_to_blog( $site_id );
	}

	$suppress = $db->suppress_errors();
	$result   = (bool) $db->get_results( "DESCRIBE {$db->posts}" );
	$db->suppress_errors( $suppress );

	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', '_switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param _Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a _Site object exists even when the site has been deleted.
		$blog = new _Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	...
	_cache_delete( $blog_id, 'sites' );
	_cache_delete( $blog_id, 'site-details' );
	_cache_delete( $blog_id, 'blog-details' );
	_cache_delete( $blog_id . 'short', 'blog-details' );
	_cache_delete( $domain_path_key, 'blog-lookup' );
	_cache_delete( $domain_path_key, 'blog-id-cache' );
	_cache_delete( $blog_id, 'blog_meta' );
 ...
 * @since 5.1.0
 *
 * @param _Site      $new_site The site object that has been inserted, updated or deleted.
 * @param _Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function _maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id != $old_site->network_id ) {
		_maybe_update_network_site_counts( $new_site->network_id );
		_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param _Site      $new_site The site object after the update.
 * @param _Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function _maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new _Site( new stdClass() );
	}
 ...
 *
 * @param _Site $new_site The site object after the update.
 * @param _Site $old_site The site obejct prior to the update.
 */
function _maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 */
function _update_blog_public_option_on_site_update( $site_id, $public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! _is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function _cache_set_sites_last_changed() {
	_cache_set( 'last_changed', microtime(), 'sites' );
}
 ...
 */
function _check_site_meta_support_prefilter( $check ) {
	if ( ! is_site_meta_supported() ) {
		/* translators: %s: Database table name. */
		_doing_it_wrong( __FUNCTION__, sprintf( __( 'The %s table is not installed. Please run the network database upgrade.' ), $GLOBALS['db']->blogmeta ), '5.1.0' );
		return false;
	}
