 ...
 * @since 3.9.0
 */
function _underscore_audio_template() {
	$audio_types = _get_audio_extensions();
	?>
...
>
	...
	<source src="{{ data.model.src }}" type="{{ .media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>
	...
	<source src="{{ data.model.<?php echo $type; ?> }}" type="{{ .media.view.settings.embedMimes[ '<?php echo $type; ?>' ] }}" />
	<# } #>
		<?php
	endforeach;
	?>
 ...
 * @since 3.9.0
 */
function _underscore_video_template() {
	$video_types = _get_video_extensions();
	?>
<#  var w_rule = '', classes = [],
		w, h, settings = .media.view.settings,
...
#>
<div style="{{ w_rule }}" class="video">
<video controls
	class="video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
	$props = array(
 ...
 * @since 3.5.0
 */
function _print_media_templates() {
	$class = 'media-modal core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<?php else : ?>
			...
			<div class="post-upload-ui" id="post-upload-info">
				<?php
				/** This action is documented in admin/includes/media.php */
				do_action( 'pre-upload-ui' ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
				/** This action is documented in admin/includes/media.php */
				do_action( 'pre-plupload-upload-ui' ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores

				if ( 10 === remove_action( 'post-plupload-upload-ui', 'media_upload_flash_bypass' ) ) {
					/** This action is documented in admin/includes/media.php */
					do_action( 'post-plupload-upload-ui' ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
				...
				<?php
				/** This action is documented in admin/includes/media.php */
				do_action( 'post-upload-ui' ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
				?>
			</div>
		<?php endif; ?>
		</div>
		...
		<div class="attachment-media-view {{ data.orientation }}">
			...
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<div class="media-wrapper audio">
					<audio style="visibility: hidden" controls class="audio-shortcode" width="100%" preload="none">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( .media.view.settings.contentWidth ) {
						w_rule = 'width: ' + .media.view.settings.contentWidth + 'px;';
					}
				#>
				<div style="{{ w_rule }}" class="media-wrapper video">
					<video controls="controls" class="video-shortcode" preload="metadata"
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<span class="setting">
			...
			<select id="gallery-settings-link-to" class="link-to"
				...
				<option value="post" <# if ( ! .media.galleryDefaults.link || 'post' === .media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_html_e( 'Attachment Page' ); ?>
				</option>
				<option value="file" <# if ( 'file' === .media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_html_e( 'Media File' ); ?>
				</option>
				<option value="none" <# if ( 'none' === .media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_html_e( 'None' ); ?>
				</option>
			</select>
		</span>
			...
			<select id="gallery-settings-columns" class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i; ?> == .media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
			...
			<div class="embed-media-settings">
				<div class="column-settings">
					...
					<# if ( data.attachment ) { #>
						...
						<# } #>
							<div class="custom-size clearfix<# if ( data.model.size !== 'custom' ) { #> hidden<# } #>">
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: .media.view.settings.embedMimes.mp3,
			ogg: .media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = _get_audio_extensions(); ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: .media.view.settings.embedMimes.mp4,
			ogv: .media.view.settings.embedMimes.ogv,
			webm: .media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = _get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
