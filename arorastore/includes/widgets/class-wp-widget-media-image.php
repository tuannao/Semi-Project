<?php
/**
 * Widget API: _Widget_Media_Image class
 *
 * @package 
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see _Widget
 */
class _Widget_Media_Image extends _Widget_Media {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment ) {
			...
			$image_attributes = array(
				'class' => sprintf( 'image image-%d %s', $attachment->ID, $instance['image_classes'] ),
			...
			if ( 'custom' === $size || ! in_array( $size, array_merge( get_intermediate_image_sizes(), array( 'full' ) ), true ) ) {
				...
				$caption_size = __get_image_size_from_meta( $instance['size'], _get_attachment_metadata( $attachment->ID ) );
			...
			$image = _get_attachment_image( $attachment->ID, $size, false, $image_attributes );
		...
		if ( 'file' === $instance['link_type'] ) {
			$url = $attachment ? _get_attachment_url( $attachment->ID ) : $instance['url'];
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-media-widget-image-fields">
		...
		<script type="text/html" id="tmpl-media-widget-image-preview">
