<?php
/**
 * Widget API: _Widget_Custom_HTML class
 *
 * @package 
 * @subpackage Widgets
 * @since 4.8.1
 ...
 * @see _Widget
 */
class _Widget_Custom_HTML extends _Widget {
	...
	public function _register_one( $number = -1 ) {
		...
		_add_inline_script( 'custom-html-widgets', sprintf( '.customHtmlWidgets.idBases.push( %s );', _json_encode( $this->id_base ) ) );

		// Note that the widgets component in the customizer will also do
		// the 'admin_print_scripts-widgets.php' action in _Customize_Widgets::print_scripts().
		add_action( 'admin_print_scripts-widgets.php', array( $this, 'enqueue_admin_scripts' ) );

		// Note that the widgets component in the customizer will also do
		// the 'admin_footer-widgets.php' action in _Customize_Widgets::print_footer_scripts().
		add_action( 'admin_footer-widgets.php', array( '_Widget_Custom_HTML', 'render_control_template_scripts' ) );

		// Note this action is used to ensure the help text is added to the end.
		add_action( 'admin_head-widgets.php', array( '_Widget_Custom_HTML', 'add_help_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in includes/widgets/class-widget-pages.php */
		...
		$simulated_text_widget_instance = array_merge(
			...
			array(
				...
				'filter' => false, // Because autop is not applied.
				'visual' => false, // Because it wasn't created in TinyMCE.
			)
		);
		unset( $simulated_text_widget_instance['content'] ); // Was moved to 'text' prop.

		/** This filter is documented in includes/widgets/class-widget-text.php */
		$content = apply_filters( 'widget_text', $instance['content'], $simulated_text_widget_instance, $this );

		// Adds 'noopener' relationship, without duplicating values, to all HTML A elements that have a target.
		$content = _targeted_link_rel( $content );
		 ...
		 * @param _Widget_Custom_HTML $widget   Current Custom HTML widget instance.
		 */
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['content'] = _kses_post( $new_instance['content'] );
		}
		return $instance;
	}
	...
	public function enqueue_admin_scripts() {
		...
		_enqueue_script( 'custom-html-widgets' );
		if ( empty( $settings ) ) {
			$settings = array(
				'disabled' => true,
			);
		}
		_add_inline_script( 'custom-html-widgets', sprintf( '.customHtmlWidgets.init( %s );', _json_encode( $settings ) ), 'after' );
		...
		_add_inline_script( 'custom-html-widgets', sprintf( 'jQuery.extend( .customHtmlWidgets.l10n, %s );', _json_encode( $l10n ) ), 'after' );
	}
	 ...
	 *
	 * @since 4.8.1
	 * @since 4.9.0 The form contains only hidden sync inputs. For the control UI, see `_Widget_Custom_HTML::render_control_template_scripts()`.
	 *
	 * @see _Widget_Custom_HTML::render_control_template_scripts()
	 *
	 * @param array $instance Current instance.
	 */
	public function form( $instance ) {
		$instance = _parse_args( (array) $instance, $this->default_instance );
		?>
	...
	public static function add_help_text() {
		...
		if ( 'false' !== _get_current_user()->syntax_highlighting ) {
			$content .= '<p>';
			$content .= sprintf(
