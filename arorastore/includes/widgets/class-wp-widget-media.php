<?php
/**
 * Widget API: _Media_Widget class
 *
 * @package 
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see _Widget
 */
abstract class _Widget_Media extends _Widget {
	 ...
	 * @param array  $widget_options  Optional. Widget options. See _register_sidebar_widget() for
	 *                                information on accepted arguments. Default empty array.
	 * @param array  $control_options Optional. Widget control options. See _register_widget_control()
	 *                                for information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = _parse_args(
			$widget_options,
			array(
		...
		$control_opts = _parse_args( $control_options, array() );
	...
	public function _register_one( $number = -1 ) {
		...
		if ( $this->is_preview() ) {
			add_action( '_enqueue_scripts', array( $this, 'enqueue_preview_scripts' ) );
		}
	...
	public function get_instance_schema() {
		 ...
		 * @param _Widget_Media $widget Widget object.
		 */
	 ...
	 * @since 4.8.0
	 *
	 * @param int|_Post $attachment Attachment post ID or object.
	...
	public function is_attachment_with_mime_type( $attachment, $mime_type ) {
		...
		return _attachment_is( $mime_type, $attachment );
	}
	...
	public function widget( $args, $instance ) {
		$instance = _parse_args( $instance, _list_pluck( $this->get_instance_schema(), 'default' ) );
	 ...
	 *
	 * @see _Widget::update()
	 * @see _REST_Request::has_valid_params()
	 * @see _REST_Request::sanitize_params()
	 *
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $schema as $field => $field_schema ) {
			...
			if ( is__error( $value ) ) {
				continue; // Handle case when rest_sanitize_value_from_schema() ever returns _Error as its phpdoc @return tag indicates.
			}
	 ...
	 * @since 4.8.0
	 *
	 * @see \_Widget_Media::render_control_template_scripts() Where the JS template is located.
	 *
	 * @param array $instance Current settings.
	 */
	final public function form( $instance ) {
		$instance_schema = $this->get_instance_schema();
		$instance        = _array_slice_assoc(
			_parse_args( (array) $instance, _list_pluck( $instance_schema, 'default' ) ),
		...
		foreach ( $instance as $name => $value ) : ?>
			<input
				...
				id="<?php echo esc_attr( $this->get_field_id( $name ) ); // Needed specifically by Widgets.appendTitle(). ?>"
	 ...
	 * @param _Post $post   The current attachment object.
	 * @return array
	 */
