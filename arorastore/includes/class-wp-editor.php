...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				...
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="switch-editor switch-tmce"' .
					' data-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="switch-editor switch-html"' .
					' data-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'core-ui editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="' . $editor_id_attr . '-editor-tools" class="editor-tools hide-if-no-js">';
			...
			if ( $set['media_buttons'] ) {
				...
				echo '<div id="' . $editor_id_attr . '-media-buttons" class="media-buttons">';
			...
			echo '<div class="editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor',
			'<div id="' . $editor_id_attr . '-editor-container" class="editor-container">' .
			$quicktags_toolbar .
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			/*
			 * In production all plugins are loaded (they are in editor.js.gz).
			 * The 'wpview', 'wpdialogs', and 'media' TinyMCE plugins are not initialized by default.
			 * Can be added from js by using the 'before-tinymce-init' event.
			 */
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins//content.css?$version" );
	...
	public static function print_tinymce_scripts() {
		...
		wp_print_scripts( array( 'tinymce' ) );
	...
	public static function editor_js() {
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/langs-en.js?$version'></script>\n";
			}
		}
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
				} else {
	...
	public static function wp_link_dialog() {
		...
		?>
		<div id="link-backdrop" style="display: none"></div>
		<div id="link-wrap" class="core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
		<form id="link" tabindex="-1">
		<?php wp_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="wplink-enter-url"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span>
					<input id="link-url" type="text" aria-describedby="wplink-enter-url" /></label>
				</div>
				<div class="link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			...
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						...
						<input type="search" id="link-search" class="link-search-field" autocomplete="off" aria-describedby="wplink-link-existing-content" />
		...
		<div class="submitbox">
			...
			<div id="link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="link-submit" name="link-submit">
			</div>
		</div>
		</form>
		</div>
