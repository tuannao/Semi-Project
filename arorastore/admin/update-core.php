...
function list_core_update( $update ) {
	...
	if ( 'development' === $update->response ) {
		...
		if ( $current ) {
			...
			$version_url = sprintf(
				/* translators: %s:  version. */
				esc_url( __( 'https://.org/support/-version/version-%s/' ) ),
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				...
				$message = sprintf(
					/* translators: 1: Installed  version number, 2: URL to  release notes, 3: New  version number, including locale if necessary. */
					__( 'You can update from  %1$s to <a href="%2$s"> %3$s</a> manually:' ),
					$wp_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $wp_local_package ) || $wp_local_package != $update->locale ) ) {
		...
		echo '<p class="hint">' . sprintf(
			/* translators: %s:  version. */
			__( 'You are about to install  %s <strong>in English (US).</strong> There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
function core_upgrade_preamble() {
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		printf(
			/* translators: 1: Documentation on  backups, 2: Documentation on updating . */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating </a> documentation page.' ),
			__( 'https://.org/support/article/-backups/' ),
			__( 'https://.org/support/article/updating-/' )
		);
		echo '</p></div>';
	} elseif ( $is_development_version ) {
		echo '<h2 class="response">' . __( 'You are using a development version of .' ) . '</h2>';
	} else {
		echo '<h2 class="response">' . __( 'You have the latest version of .' ) . '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2:  version. */
			__( '<a href="%1$s">Learn more about  %2$s</a>.' ),
...
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all  versions have been enabled. Thank you!' );
			...
			$notice_text = __( ' will only receive automatic security and maintenance releases from now on.' );
			echo '<div class="notice notice-success is-dismissible"><p>' . $notice_text . '</p></div>';
		}
	}
	...
	<p class="auto-update-status">
		...
		if ( $updater->is_vcs_checkout( ABSPATH ) ) {
			...
			_e( 'This site is automatically kept up to date with each new version of .' );
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of  only.' );
			...
			if ( $can_set_update_option ) {
				...
				printf(
					...
					__( 'Enable automatic updates for all new versions of .' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of .' );
		}
		?>
	</p>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s:  version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with  %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s:  version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with  %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of .
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s:  version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with  %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s:  version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with  %s: Unknown' ), $core_update_version );
			}
		}
...
<p>
	...
	printf(
		...
		__( 'https://developer..org/themes/advanced-topics/child-themes/' )
	);
	?>
</p>
...
<table class="widefat updates-table" id="update-themes-table">
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br>' . __( 'This update does not work with your versions of  and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to  Updates screen, 2: URL to Update PHP page. */
					__( '<a href="%1$s">Please update </a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$compat .= sprintf(
					/* translators: %s: URL to  Updates screen. */
					__( '<a href="%s">Please update </a>.' ),
					self_admin_url( 'update-core.php' )
				);
			...
			$compat .= '<br>' . __( 'This update does not work with your version of .' ) . '&nbsp;';
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for  major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the WP-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override  scheduling.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://.org/support/article/plugins-themes-auto-updates/">Learn more: Auto-updates documentation</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://.org/support/article/dashboard-updates-screen/">Documentation on Updating </a>' ) . '</p>' .
	$help_sidebar_autoupdates .
	'<p>' . __( '<a href="https://.org/support/">Support</a>' ) . '</p>'
);
...
if ( 'upgrade-core' === $action ) {
	...
	/* translators: Current version of . */
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
}
