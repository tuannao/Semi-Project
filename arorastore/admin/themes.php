...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf(
			/* translators: %s: https://.org/themes/ */
			__( 'If you would like to see more themes to choose from, click on the &#8220;Add New&#8221; button and you will be able to browse or search for additional themes from the <a href="%s"> Theme Directory</a>. Themes in the  Theme Directory are designed and developed by third parties, and are compatible with the license  uses. Oh, and they&#8217;re free!' ),
			__( 'https://.org/themes/' )
		) . '</p>';
	}
...
if ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		...
		'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override  scheduling.' ) . '</p>';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://.org/support/article/using-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://.org/support/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<div class="theme<?php echo $active_class; ?>">
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		...
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			_e( 'This theme does not work with your versions of  and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				printf(
					/* translators: 1: URL to  Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update </a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			_e( 'This theme does not work with your version of .' );
			if ( current_user_can( 'update_core' ) ) {
				printf(
					/* translators: %s: URL to  Updates screen. */
					' ' . __( '<a href="%s">Please update </a>.' ),
