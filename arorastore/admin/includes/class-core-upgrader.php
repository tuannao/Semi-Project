 ...
 *
 * It allows for  to upgrade itself in combination with
...
class Core_Upgrader extends WP_Upgrader {
	...
	public function upgrade_strings() {
		$this->strings['up_to_date'] = __( ' is at the latest version.' );
		...
		$this->strings['rollback_was_required'] = __( 'Due to an error during updating,  has rolled back to your previous version.' );
	}
	 ...
	 *
	 * @param object $current Response object for whether  is current.
	 * @param array  $args {
	 *     Optional. Arguments for upgrading  core. Default empty array.
	 *
	 ...
	 *                                  Default false.
	 * }
	 * @return string|false|WP_Error New  version on success, false or WP_Error on failure.
	 */
	public function upgrade( $current, $args = array() ) {
		...
		if ( is_wp_error( $download ) && $download->get_error_data( 'softfail-filename' ) ) {
			...
			// Report this failure back to .org for debugging purposes.
			wp_version_check(
				array(
	...
	/**
	 * Determines if this  Core version should update to an offered version or not.
	 *
	 * @since 3.7.0
	 *
