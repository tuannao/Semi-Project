<?php
/**
 * Class for looking up a site's health based on a user's  environment.
 *
 * @package 
...
class WP_Site_Health {
	...
	private function prepare_sql_data() {
		...
		if ( $wpdb->use_mysqli ) {
			// phpcs:ignore .DB.RestrictedFunctions.mysql_mysqli_get_server_info
			$mysql_server_type = mysqli_get_server_info( $wpdb->dbh );
		} else {
			// phpcs:ignore .DB.RestrictedFunctions.mysql_mysql_get_server_info,PHPCompatibility.Extensions.RemovedExtensions.mysql_DeprecatedRemoved
	...
	/**
	 * Tests for  version and outputs it.
	 *
	 ...
	 */
	public function get_test__version() {
		$result = array(
			'label'       => '',
			'status'      => '',
			'badge'       => array(
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: Your current version of . */
				__( ' version %s' ),
			...
			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'Unable to check if any new versions of  are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						/* translators: %s: The latest version of  available. */
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of ' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of  is available.' )
						);
					} else {
					...
					$result['status'] = 'good';
					$result['label']  = sprintf(
						/* translators: %s: The current version of  installed on this site. */
						__( 'Your version of  (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of  available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
			...
			if ( $active_theme->parent() ) {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							/* translators: 1: The default theme for . 2: The currently active theme. 3: The active theme's parent theme. */
							__( 'To enhance your site&#8217;s security, you should consider removing any themes you are not using. You should keep %1$s, the default  theme, %2$s, your active theme, and %3$s, its parent theme.' ),
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						'<p>%s %s</p>',
						sprintf(
							/* translators: 1: The amount of inactive themes. 2: The default theme for . 3: The currently active theme. */
							_n(
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by  automatically if anything is wrong with your chosen theme.' )
			);
		}
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is the programming language used to build and maintain . Newer versions of PHP are created with increased performance in mind, so you may see a positive effect on your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
	 ...
	 * as seen at https://make..org/hosting/handbook/handbook/server-environment/#php-extensions
	 *
	 * @since 5.2.0
	 *
	 * @return array
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'The  Hosting Team maintains a list of those modules, both recommended and required, in <a href="%1$s" %2$s>the team handbook%3$s</a>.' ),
					/* translators: Localized team handbook, if one exists. */
					esc_url( __( 'https://make..org/hosting/handbook/handbook/server-environment/#php-extensions' ) ),
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by  on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Localized version of  requirements if one exists. */
				esc_url( __( 'https://.org/about/requirements/' ) ),
				__( 'Learn more about what  requires to run.' ),
				/* translators: Accessibility text. */
				__( '(opens in a new tab)' )
			),
			'test'        => 'sql_server',
	...
	public function get_test_utf8mb4_support() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'UTF8MB4 is the character set  prefers for database storage because it safely supports the widest set of characters and encodings, including Emoji, enabling better support for non-English languages.' )
			),
			'actions'     => '',
		...
		if ( ! $this->is_mariadb ) {
			if ( version_compare( $this->mysql_server_version, '5.5.3', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( '&#8217; utf8mb4 support requires MySQL version %s or greater. Please contact your server administrator.' ),
						'5.5.3'
					)
				);
			} else {
			...
			if ( version_compare( $this->mysql_server_version, '5.5.0', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						/* translators: %s: Version number. */
						__( '&#8217; utf8mb4 support requires MariaDB version %s or greater. Please contact your server administrator.' ),
						'5.5.0'
					)
				);
			} else {
		...
		if ( $wpdb->use_mysqli ) {
			// phpcs:ignore .DB.RestrictedFunctions.mysql_mysqli_get_client_info
			$mysql_client_version = mysqli_get_client_info();
		} else {
			// phpcs:ignore .DB.RestrictedFunctions.mysql_mysql_get_client_info,PHPCompatibility.Extensions.RemovedExtensions.mysql_DeprecatedRemoved
			...
			if ( version_compare( $mysql_client_version, '5.0.9', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( '&#8217; utf8mb4 support requires MySQL client library (%1$s) version %2$s or newer. Please contact your server administrator.' ),
						'mysqlnd',
						'5.0.9'
					)
				);
	...
	/**
	 * Test if the site can communicate with .org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_dotorg_communication() {
		$result = array(
			'label'       => __( 'Can communicate with .org' ),
			'status'      => '',
			'badge'       => array(
				'label' => __( 'Security' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the  servers is used to check for new versions, and to both install and update  core, themes or plugins.' )
			),
			'actions'     => '',
		...
		if ( ! is_wp_error( $wp_dotorg ) ) {
			...
			$result['label'] = __( 'Could not reach .org' );
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'Error' ),
					sprintf(
						/* translators: 1: The IP address .org resolves to. 2: The error returned by the lookup. */
						__( 'Your site is unable to reach .org at %1$s, and returned the error: %2$s' ),
						gethostbyname( 'api..org' ),
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Documentation explaining debugging in . */
				esc_url( __( 'https://.org/support/article/debugging-in-/' ) ),
				__( 'Learn more about debugging in .' ),
				/* translators: Accessibility text. */
				__( '(opens in a new tab)' )
			),
			...
			if ( wp_is_site_url_using_https() ) {
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: URL to Settings > General >  Address, 2: URL to Settings > General > Site Address. */
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s"> Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General >  Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s"> Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
				...
				if ( defined( 'WP_HOME' ) || defined( 'WP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							__( 'However, your  Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and  itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
	...
	/**
	 * Test if  can run automated background updates.
	 *
	 * Background updates in  are primarily used for minor releases and security updates.
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that  can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
	 ...
	 *
	 * A loopback is when  queries itself, for example to start a new WP_Cron instance,
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way , and other applications, communicate with the server. One example is the block editor screen, which relies on this to display, and save, your posts and pages.' )
			),
			'actions'     => '',
		...
		if ( is_wp_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					sprintf(
						/* translators: 1: The  error message. 2: The  error code. */
						__( 'Error: %1$s (%2$s)' ),
						$r->get_error_message(),
						$r->get_error_code()
					)
	...
	public function get_test_authorization_header() {
		...
		if ( got_mod_rewrite() ) {
			...
			$result['actions'] .= sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				__( 'https://developer..org/rest-api/frequently-asked-questions/#why-is-authentication-not-working' ),
	...
	public static function get_tests() {
		$tests = array(
			...
			'async'  => array(
				'dotorg_communication' => array(
					'label'             => __( 'Communication with .org' ),
		 ...
		 * both the  Hosting Team and web standards in general.
		 *
	...
	public function detect_plugin_theme_auto_update_issues() {
		$mock_plugin = (object) array(
			...
			'new_version'   => '9.9',
			'url'           => 'https://.org/plugins/a-fake-plugin/',
			'package'       => 'https://downloads..org/plugin/a-fake-plugin.9.9.zip',
			'icons'         => array(
		...
		$mock_theme = (object) array(
			'theme'        => 'a-fake-theme',
			'new_version'  => '9.9',
			'url'          => 'https://.org/themes/a-fake-theme/',
			'package'      => 'https://downloads..org/theme/a-fake-theme.9.9.zip',
			'requires'     => '5.0.0',
			'requires_php' => '5.6.20',
		);
	 ...
	 *
	 * Loopbacks are what  uses to communicate with itself to start up WP_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		 ...
		 * GET request  uses for wp-cron.php loopback requests but is close
		 * enough. See https://core.trac..org/ticket/52547
		 */
