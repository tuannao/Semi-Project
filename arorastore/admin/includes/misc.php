	...
	switch ( $map_option ) {
		...
		default:
			...
			if ( '_page' === substr( $option, -5 ) || 'layout_columns' === $option ) {
				 ...
				 */
				$screen_option = apply_filters( 'set-screen-option', $screen_option, $option, $value ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
			}
...
/**
 * Checks if rewrite rule for  already exists in the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
...
function iis7_rewrite_rule_exists( $filename ) {
	...
	$rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'\')]' );
...
/**
 * Deletes  rewrite rule from web.config file if it exists there.
 *
 * @since 2.8.0
 *
...
/**
 * Adds  rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename     The file path to the configuration file.
...
function iis7_add_rewrite_rule( $filename, $rewrite_rule ) {
	...
	$_rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'\')]' );
 ...
 */
function saveDomDocument( $doc, $filename ) { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
	$config = $doc->saveXML();
...
function wp_check_php_version() {
	...
	if ( false === $response ) {
		$url = 'http://api..org/core/serve-happy/1.0/';
		 ...
		 *  'is_acceptable' - boolean - Whether the PHP version is still acceptable for .
		 */
	...
	if ( isset( $response['is_acceptable'] ) && $response['is_acceptable'] ) {
		 ...
		 *
		 * This filter is only run if the .org Serve Happy API considers the PHP version acceptable, ensuring
