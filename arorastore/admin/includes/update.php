...
function get_core_checksums( $version, $locale ) {
	$http_url = 'http://api..org/core/checksums/1.0/?' . http_build_query( compact( 'version', 'locale' ), '', '&' );
	$url      = $http_url;
...
/**
 * Finds the available update for  core.
 *
 * @since 2.7.0
 *
...
function core_update_footer( $msg = '' ) {
	...
	if ( $is_development_version ) {
		return sprintf(
			/* translators: 1:  version number, 2: URL to  Updates screen. */
...
function update_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: 1: URL to  release notes, 2: New  version, 3: URL to network admin, 4: Accessibility text. */
			__( '<a href="%1$s"> %2$s</a> is available! <a href="%3$s" aria-label="%4$s">Please update now</a>.' ),
			$version_url,
			$cur->current,
			network_admin_url( 'update-core.php' ),
		...
		$msg = sprintf(
			/* translators: 1: URL to  release notes, 2: New  version. */
			__( '<a href="%1$s"> %2$s</a> is available! Please notify the site administrator.' ),
			$version_url,
			$cur->current
		);
	}
...
/**
 * Displays  version and active theme in the 'At a Glance' dashboard widget.
 *
 * @since 2.5.0
 */
	...
	if ( current_user_can( 'update_core' ) ) {
		...
		if ( isset( $cur->response ) && 'upgrade' === $cur->response ) {
			$msg .= sprintf(
				...
				network_admin_url( 'update-core.php' ),
				/* translators: %s:  version number, or 'Latest' string. */
...
function wp_plugin_update_row( $file, $plugin_data ) {
	...
	if ( is_network_admin() || ! is_multisite() ) {
		 ...
		 *     @type string   $requires     The version of  which the plugin requires.
		 *     @type string   $tested       The version of  the plugin is tested against.
		 *     @type string   $requires_php The version of PHP which the plugin requires.
		 * }
		 */
		do_action( "in_plugin_update_message-{$file}", $plugin_data, $response ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
...
function wp_theme_update_row( $theme_key, $theme ) {
	...
	if ( $compatible_wp && $compatible_php ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			printf(
				/* translators: %s: Theme name. */
				__( 'There is a new version of %s available, but it does not work with your versions of  and PHP.' ),
				$theme['Name']
			);
			...
			printf(
				/* translators: %s: Theme name. */
				__( 'There is a new version of %s available, but it does not work with your version of .' ),
				$theme['Name']
			);
			if ( current_user_can( 'update_core' ) ) {
				printf(
	 ...
	 * }
	 */
	do_action( "in_theme_update_message-{$theme_key}", $theme, $response ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
...
function maintenance_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: %s: URL to  Updates screen. */
			__( 'An automated  update has failed to complete - <a href="%s">please attempt the update again now</a>.' ),
			'update-core.php'
		);
	} else {
