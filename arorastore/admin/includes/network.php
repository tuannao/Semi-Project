...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of  sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( $is_www ) :
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your  network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	<ol>
		...
		printf(
			 ...
			 * https://i18n.svn..org/<locale code>/branches/<wp version>/dist/wp-config-sample.php
			 */
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
		...
		if ( ! empty( $keys_salts ) ) {
			$keys_salts_str = '';
			$from_api       = wp_remote_get( 'https://api..org/secret-key/1.1/salt/' );
...
';
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( 'https://.org/support/article/nginx/' )
		);
		echo '</p></li>';
