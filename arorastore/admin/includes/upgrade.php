...
if ( ! function_exists( 'wp_install_defaults' ) ) :
	...
	function wp_install_defaults( $user_id ) {
		...
		if ( is_multisite() ) {
			...
			$first_post = "<!-- wp:paragraph -->\n<p>" .
			/* translators: First post content. %s: Site link. */
			__( 'Welcome to . This is your first post. Edit or delete it, then start writing!' ) .
			"</p>\n<!-- /wp:paragraph -->";
		}
		...
		$first_comment_author = ! empty( $first_comment_author ) ? $first_comment_author : __( 'A  Commenter' );
		$first_comment_email  = ! empty( $first_comment_email ) ? $first_comment_email : 'wapuu@.example';
		$first_comment_url    = ! empty( $first_comment_url ) ? $first_comment_url : esc_url( __( 'https://.org/' ) );
...
Commenter avatars come from <a href="%s">Gravatar</a>.'
		...
		if ( empty( $first_page ) ) {
			...
			$first_page .= sprintf(
				...
				__( 'As a new  user, you should go to <a href="%s">your dashboard</a> to delete this page and create new pages for your content. Have fun!' ),
				admin_url()
			);
			$first_page .= "</p>\n<!-- /wp:paragraph -->";
		}
...
if ( ! function_exists( 'wp_new_blog_notification' ) ) :
	/**
	 * Notifies the site admin that the installation of  is complete.
	 *
...
'
		...
		/**
		 * Filters the contents of the email sent to the site administrator when  is installed.
		 *
		 * @since 5.6.0
		 *
...
/**
 * Executes changes made in  4.6.0.
 *
 * @ignore
 * @since 4.6.0
 *
...
/**
 * Executes changes made in  5.0.0.
 *
 * @ignore
 * @since 5.0.0
 * @deprecated 5.1.0
...
/**
 * Executes changes made in  5.1.0.
 *
 * @ignore
 * @since 5.1.0
 */
...
/**
 * Executes changes made in  5.3.0.
 *
 * @ignore
 * @since 5.3.0
 */
...
/**
 * Executes changes made in  5.5.0.
 *
 * @ignore
 * @since 5.5.0
 */
...
/**
 * Executes changes made in  5.6.0.
 *
 * @ignore
 * @since 5.6.0
 */
function upgrade_560() {
	...
	if ( $wp_current_db_version < 49572 ) {
		 ...
		 * See https://core.trac..org/ticket/51742.
		 */
	...
	if ( $wp_current_db_version < 49632 ) {
		 ...
		 * See https://core.trac..org/ticket/51723.
		 */
...
/**
 * Executes changes made in  5.9.0.
 *
 * @ignore
 * @since 5.9.0
 *
...
/**
 * Executes changes made in  6.0.0.
 *
 * @ignore
 * @since 6.0.0
 *
 ...
 * @return mixed
 */
function __get_option( $setting ) { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	global $wpdb;
 ...
 */
function dbDelta( $queries = '', $execute = true ) { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wpdb;
