 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the .org
 ...
 *         {@link https://developer..org/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the .org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current .org Plugin Installation API request.
	 *
	 * Returning a non-false value will effectively short-circuit the .org API request.
	 *
	...
	if ( false === $res ) {
		...
		$url = 'http://api..org/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on .org, you can browse them here.' ); ?></p>
	<form method="get">
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'https://make..org/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
		case 'install_plugins_featured':
			printf(
				/* translators: %s: https://.org/plugins/ */
				'<p>' . __( 'Plugins extend and expand the functionality of . You may automatically install plugins from the <a href="%s"> Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
				__( 'https://.org/plugins/' )
			);
			break;
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires  Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://.org/plugins/' ) . $api->slug ); ?>/"><?php _e( '.org Plugin Page &#187;' ); ?></a></li>
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://.org/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	if ( ! $tested_wp ) {
		...
		_e( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of .' );
		echo '</p></div>';
	} elseif ( ! $compatible_wp ) {
		echo '<div class="notice notice-error notice-alt"><p>';
		_e( '<strong>Error:</strong> This plugin <strong>requires a newer version of </strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			printf(
				/* translators: %s: URL to  Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update </a>.' ),
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://.org/plugins/' . $api->slug . '/' );
