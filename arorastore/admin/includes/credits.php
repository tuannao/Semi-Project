...
function wp_credits( $version = '', $locale = '' ) {
	...
	$results = get_site_transient( '_credits_' . $locale );

	if ( ! is_array( $results )
		|| false !== strpos( $version, '-' )
		|| ( isset( $results['data']['version'] ) && strpos( $version, $results['data']['version'] ) !== 0 )
	) {
		$url     = "http://api..org/core/credits/1.1/?version={$version}&locale={$locale}";
		...
		set_site_transient( '_credits_' . $locale, $results, DAY_IN_SECONDS );
	}
