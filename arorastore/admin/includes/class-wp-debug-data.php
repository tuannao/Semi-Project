<?php
/**
 * Class for providing debug data based on a users  environment.
 *
 * @package 
...
class WP_Debug_Data {
	...
	public static function debug_data() {
		...
		$info['wp-dropins'] = array(
			...
			'description' => sprintf(
				...
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance  features in ways that are not possible for traditional plugins.' ),
		...
		$info['wp-constants'] = array(
			'label'       => __( ' Constants' ),
			'description' => __( 'These settings alter where and how parts of  are loaded.' ),
			'fields'      => array(
				'ABSPATH'             => array(
					'label'   => 'ABSPATH',
					'value'   => ABSPATH,
		...
		$info['wp-filesystem'] = array(
			...
			'description' => __( 'Shows whether  is able to write to the directories it needs access to.' ),
			'fields'      => array(
				''  => array(
		...
		//  features requiring processing.
		$wp_dotorg = wp_remote_get( 'https://.org', array( 'timeout' => 10 ) );

		if ( ! is_wp_error( $wp_dotorg ) ) {
			$info['wp-core']['fields']['dotorg_communication'] = array(
				'label' => __( 'Communication with .org' ),
				'value' => __( '.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$info['wp-core']['fields']['dotorg_communication'] = array(
				...
				'value' => sprintf(
					...
					__( 'Unable to reach .org at %1$s: %2$s' ),
					gethostbyname( '.org' ),
		...
		if ( ! $is_multisite ) {
			...
			$info['wp-paths-sizes']['fields'] = array(
				'_path' => array(
					'label' => __( ' directory location' ),
					'value' => untrailingslashit( ABSPATH ),
				),
				'_size' => array(
					'label' => __( ' directory size' ),
					'value' => $loading,
					'debug' => 'loading...',
				),
				'uploads_path'   => array(
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core  rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN [\s\S]+?# END /si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core  features.' ), '.htaccess' );
			}
	...
	/**
	 * Fetch the sizes of the  directories: `` (ABSPATH), `plugins`, `themes`, and `uploads`.
	...
	public static function get_sizes() {
		...
		// No trailing slashes.
		$paths = array(
			'_size' => untrailingslashit( ABSPATH ),
