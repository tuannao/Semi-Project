	...
	<p class="community-events-footer">
		<?php
			printf(
				...
				'https://make..org/community/meetups-landing-page',
				__( 'Meetups' ),
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text">%3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				/* translators: If a Rosetta site exists (e.g. https://es..org/news/), then use that. Otherwise, leave untranslated. */
				esc_url( _x( 'https://.org/news/', 'Events and News dashboard widget' ) ),
				__( 'News' ),
				/* translators: Accessibility text. */
				__( '(opens in a new tab)' )
			);
		...
		<# if ( data.events.length <= 2 ) { #>
			<li class="event-none">
				...
				printf(
					...
					__( 'https://make..org/community/organize-event-landing-page/' )
				);
				?>
			</li>
		<# } #>
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'There are no events scheduled near %1$s at the moment. Would you like to <a href="%2$s">organize a  event</a>?' ),
					'{{ data.location.description }}',
					__( 'https://make..org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			...
			<# } else { #>
				...
				printf(
					...
					__( 'There are no events scheduled near you at the moment. Would you like to <a href="%s">organize a  event</a>?' ),
					__( 'https://make..org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			<# } #>
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'https://.org/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'https://.org/news/feed/' ) ),
		...
		'planet' => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_secondary_link', __( 'https://planet..org/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_secondary_feed', __( 'https://planet..org/feed/' ) ),
...
/**
 * Displays the  events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best  experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best  experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best  experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore .NamingConventions.ValidHookName.UseUnderscores
}
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		...
		$url     = 'http://api..org/core/browse-happy/1.1/';
		$options = array(
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<a href="<?php echo esc_url( __( 'https://.org/support/article/block-themes/' ) ); ?>"><?php _e( 'Learn about block themes' ); ?></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
