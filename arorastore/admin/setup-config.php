...
if ( file_exists( ABSPATH . 'wp-config-sample.php' ) ) {
	...
	wp_die(
		sprintf(
			...
			__( 'Sorry, I need a %s file to work from. Please re-upload this file to your  installation.' ),
...
<p><?php _e( 'Welcome to . Before getting started, you will need to know the following items.' ); ?></p>
<ol>
...
<p>
	...
	<strong>
		...
		printf(
			...
			__( 'https://.org/support/article/editing-wp-config-php/' ),
			'<code>wp-config.php</code>'
		);
		?>
</p>
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" aria-describedby="dbname-desc" size="25" value=""<?php echo $autofocus; ?>/></td>
			<td id="dbname-desc"><?php _e( 'The name of the database you want to use with .' ); ?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td id="prefix-desc"><?php _e( 'If you want to run multiple  installations in a single database, change this.' ); ?></td>
		</tr>
	</table>
		<?php
		if ( isset( $_GET['noapi'] ) ) {
	...
	case 2:
		...
		*/
		...
		try {
			...
			if ( ! $no_api ) {
				$secret_keys = wp_remote_get( 'https://api..org/secret-key/1.1/salt/' );
			}
