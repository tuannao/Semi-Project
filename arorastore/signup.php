...
if ( is_array( get_site_option( 'illegal_names' ) ) && isset( $_GET['new'] ) && in_array( $_GET['new'], get_site_option( 'illegal_names' ), true ) ) {
	_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via _head.
 *
 * @since MU (3.0.0)
 */
...
add_action( '_head', 'do_signup_header' );

if ( ! is_multisite() ) {
	_redirect( _registration_url() );
	die();
}

if ( ! is_main_site() ) {
	_redirect( network_site_url( 'signup.php' ) );
	die();
}
...
add_action( '_head', 'mu_signup_stylesheet' );
get_header( 'signup' );
...
<div class="mu_register signup-container" role="main">
<?php
/**
 ...
 * @param _Error|string $errors     A _Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param _Error $errors A _Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
 ...
 *               See mu_validate_blog_signup() for details.
 */
function validate_blog_form() {
	...
	return mu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'], $user );
}
...
function show_user_form( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param _Error $errors A _Error object containing 'user_name' or 'user_email' errors.
	 */
 ...
 *               See mu_validate_user_signup() for details.
 */
function validate_user_form() {
	return mu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type _Error $errors     A _Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
 ...
 * @global _Error $errors     Existing errors in the global scope.
...
function validate_another_blog_signup() {
	...
	if ( ! empty( $_POST['LANG'] ) ) {
		...
		if ( in_array( $_POST['LANG'], $languages, true ) ) {
			$language = _unslash( sanitize_text_field( $_POST['LANG'] ) );

			if ( $language ) {
				$blog_meta_defaults['LANG'] = $language;
			}
		}
	}
	...
	$blog_id = mu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, get_current_network_id() );
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type _Error $errors     A _Error object with possible errors relevant to the sign-up user.
	 * }
	 */
...
function validate_user_signup() {
	...
	mu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function signup_blog( $user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type _Error $errors     A _Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
function validate_blog_signup() {
	...
	$user_result = mu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result     = mu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain     = $result['domain'];
	$path       = $result['path'];
	$blogname   = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	mu_signup_blog( $domain, $path, $blog_title, $user_name, $user_email, $meta );
 ...
 *                              'all', 'none', 'blog', or 'user'.
 */
$active_signup = apply_filters( 'mu_active_signup', $active_signup );
...
if ( 'none' === $active_signup ) {
	...
	$login_url = _login_url( network_site_url( 'signup.php' ) );
...
<?php
/**
 * Fires after the sign-up forms, before _footer.
 *
 * @since 3.0.0
 */
